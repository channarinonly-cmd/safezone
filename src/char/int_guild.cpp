	...
	if (flag&GS_MEMBER)
	{
		...
		for(i=0;i<g.max_member;i++){
			...
			if (!m->modified)
				continue;
			if(m->account_id) {
				// ป้องกันตัวอักษรพิเศษในชื่อ
				char esc_membername[NAME_LENGTH*2+1];
				Sql_EscapeStringLen(sql_handle, esc_membername, m->name, strnlen(m->name, NAME_LENGTH));

				// สั่งบันทึกข้อมูลแบบเต็มรูปแบบ
				if( SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `%s` "
					"(`guild_id`, `account_id`, `char_id`, `hair`, `hair_color`, `gender`, `class`, `lv`, `exp`, `online`, `position`, `name`) "
					"VALUES ('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%" PRIu64 "', '%d', '%d', '%s')",
					schema_config.guild_member_db, g.guild_id, m->account_id, m->char_id, m->hair, m->hair_color, m->gender, m->class_, m->lv, m->exp, m->online, m->position, esc_membername ) )
					Sql_ShowDebug(sql_handle);
...
int guild_calcinfo( std::shared_ptr<CharGuild> g ){
	...
	g->guild.max_member = 16 + guild_checkskill(g, GD_EXTENSION) * 1;
	if(g->guild.max_member > MAX_GUILD)
	{
