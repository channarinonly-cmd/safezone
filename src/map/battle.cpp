...
#include "pet.hpp"
// ==========================================
// Advanced Status Reflect from TXT DB
// ==========================================
std::vector<s_class_penalty_yaml> class_penalty_db;

void battle_read_custom_yaml() { 
	class_penalty_db.clear();
	const char* filepath = "db/custom/class_penalty.txt";
	...
	// ในฟังก์ชัน battle_read_custom_yaml()
	char line[1024];
	while (fgets(line, sizeof(line), fp)) {
		if (line[0] == '/' && line[1] == '/') continue;
		if (line[0] == '\n' || line[0] == '\r') continue;

		int job, sc, skill, type;
		float rate; // <--- เปลี่ยน Rate เป็น float
		
		// เปลี่ยนตัวที่ 4 จาก %d เป็น %f
		if (sscanf(line, "%d,%d,%d,%f,%d", &job, &sc, &skill, &rate, &type) == 5) {
			s_class_penalty_yaml entry = {job, sc, skill, rate, type};
			class_penalty_db.push_back(entry);
		}
	}
	fclose(fp);
	ShowStatus("Done reading '" CL_WHITE "%zu" CL_RESET "' entries in '" CL_WHITE "%s" CL_RESET "'.\n", class_penalty_db.size(), filepath);
}

int city_faction_war = 1;
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	nullpo_ret(bl);
		...
		// ==========================================
		// [Fix] Absolute Protector Shield (กำแพงด่านสุดท้าย)
		// ป้องกัน แทรปฮัน, โฮมุนครุส และสกิลเวทลงพื้นที่พยายามมุดข้ามกฎล็อคเป้าหมาย
		// ==========================================
		struct block_list *s_master = battle_get_master(src);
		struct block_list *t_master = battle_get_master(bl);
		
		if (s_master && t_master && s_master->type == BL_PC && t_master->type == BL_PC && s_master->id != t_master->id) {
			map_session_data *s_sd = (map_session_data*)s_master;
			map_session_data *t_sd = (map_session_data*)t_master;
			
			// ถ้าไม่ได้อยู่ปาร์ตี้หรือกิลด์เดียวกัน
			if (!(s_sd->status.party_id > 0 && s_sd->status.party_id == t_sd->status.party_id) &&
				!(s_sd->status.guild_id > 0 && s_sd->status.guild_id == t_sd->status.guild_id)) 
			{
				struct map_data *mapdata = map_getmapdata(bl->m);
				if (!mapdata_flag_gvg(mapdata) && !mapdata->getMapFlag(MF_BATTLEGROUND) && !mapdata->getMapFlag(MF_PVP)) {
					if (!(map_getcell(s_sd->bl.m, s_sd->bl.x, s_sd->bl.y, CELL_CHKPVP) && map_getcell(t_sd->bl.m, t_sd->bl.x, t_sd->bl.y, CELL_CHKPVP))) {
						
						// ถ้ามีคนกางโล่รักสงบ ให้ลบดาเมจทิ้งเป็น 0 (ยกเว้นสกิล Heal/Buff)
						if (s_sd->protector || t_sd->protector) {
							if (skill_id == 0 || !(skill_get_inf(skill_id) & (INF_SUPPORT_SKILL))) {
								return 0; 
							}
						}
					}
	...
	// Advanced Status Reflect from TXT DB
	// ==========================================
	map_session_data *attacker_sd = BL_CAST(BL_PC, src); // เช็กว่าคนตีคือผู้เล่นไหม
	if (attacker_sd) {
		status_change *sc = status_get_sc(src);
		for (const auto& penalty : class_penalty_db) {
			// 1. เช็กอาชีพ
			if (attacker_sd->status.class_ != penalty.job_id) continue;
			
			// 2. เช็กสถานะ SC (ถ้าในไฟล์ตั้งเป็น 0 แปลว่าไม่ต้องมีบัฟ/ดีบัฟ ก็สะท้อน)
			if (penalty.sc_id != 0) {
				if (!sc || !sc->getSCE((sc_type)penalty.sc_id)) continue;
			}
			
			// 3. เช็กสกิล (ถ้าตั้งเป็น 0 คือการโจมตีธรรมดา)
			if (skill_id != penalty.skill_id) continue;

			// 4. เช็กประเภท (1=กายภาพ, 2=เวท, 3=ทั้งหมด)
			bool can_reflect = false;
			if (penalty.type == 3) can_reflect = true;
			else if (penalty.type == 1 && (d->flag & BF_WEAPON)) can_reflect = true;
			else if (penalty.type == 2 && (d->flag & BF_MAGIC)) can_reflect = true;

			// สุ่มโอกาสสะท้อน
			if (can_reflect && ((rnd() % 10000) / 100.0f < penalty.rate)) {
				// สร้างดาเมจใส่ตัวผู้เล่น (src) ที่มาจากมอนสเตอร์ (bl)
				status_fix_damage(bl, src, damage, 0, 0);
				// โชว์เลขดาเมจสีแดงกระเด้งบนหัวผู้เล่น
				clif_damage(bl, src, gettick(), 0, 0, damage, 0, DMG_NORMAL, 0, false);
				
				// เคลียร์ดาเมจทิ้ง มอนสเตอร์จะไม่โดนดาเมจ
				damage = 0;
				return 0;
			}
		}
	...
	// ==========================================================
	// Custom Mapflag: ระบบลดดาเมจทำมือ (ทำใส่เฉพาะมอนสเตอร์)
	// ==========================================================
	if (bl != NULL && bl->type == BL_MOB) {
		bool applied_custom_skill = false;
		struct map_data *mapdata = map_getmapdata(bl->m);
		
		if (mapdata) {
			// 1. เช็คว่าแมพนี้มีการตั้งค่า "ลดดาเมจเฉพาะสกิล" ไหม?
			if (skill_id > 0) {
				auto it = mapdata->custom_skill_dmg.find(skill_id);
				if (it != mapdata->custom_skill_dmg.end()) {
					damage = damage * it->second / 100;
					applied_custom_skill = true; // มาร์คไว้ว่าคิดดาเมจของสกิลนี้ไปแล้ว
				}
			}
			
			// 2. ถ้าไม่ได้ใช้สกิลที่ดักไว้ ให้ไปเช็ค "การลดดาเมจแบบเหมารวม" แทน
			if (!applied_custom_skill && mapdata->mob_damagerate > 0) {
				damage = damage * mapdata->mob_damagerate / 100;
			}
		}
	}
	// ==========================================================
...
void battle_consume_ammo(map_session_data*sd, int skill, int lv)
{
	...
	int item_id = pc_get_itemid(sd, sd->equip_index[EQI_AMMO]);
	int item_index = pc_search_inventory(sd, item_id);
	int ammo_count = sd->inventory.u.items_inventory[item_index].amount;
	...
	if (ammo_count > 1) {
		...
		int price = itemdb_search(item_id)->value_buy;
		price = price * qty;
...
static bool is_attack_critical(struct Damage* wd, struct block_list *src, struct block_list *target, int skill_id, int skill_lv, bool first_call)
{
	...
	if (skill_id == NPC_CRITICALSLASH || skill_id == LG_PINPOINTATTACK) //Always critical skills
		return true;
#endif
	if( skill_id && !skill_get_nk(skill_id,NK_CRITICAL) && skill_id != GC_ROLLINGCUTTER )
		return false;
...
//				wd.div_ = 2; // ดาบ 2 มือ = 2 hit
//			else
//				wd.div_ = 1; // ดาบ 1 มือ (หรืออาวุธอื่น) = 1 hit
//		}
//		break;
//#endif
//
...
//#endif
			...
			case KN_BOWLINGBASH:
				if (sd && sd->status.weapon == W_2HSWORD) {
					if (wd.miscflag >= 1 && wd.miscflag <= 2)
						wd.div_ = 1;
					else if (wd.miscflag >= 3)
						wd.div_ = 2;
				}
				break;
#endif
...
// Skill damage adjustment
	...
	// ==========================================
	// 🔥 ระบบดูด HP/SP ด้วยเวทมนตร์
	// ==========================================
	if (sd && ad.damage > 0 && src != target && target->type == BL_MOB) {
		int64 hp_gain = 0;
		int64 sp_gain = 0;

		if (sd->bonus.magic_hp_drain_per > 0 && (sd->bonus.magic_hp_drain_rate >= 10000 || rnd() % 10000 < sd->bonus.magic_hp_drain_rate)) {
			hp_gain = (ad.damage * sd->bonus.magic_hp_drain_per) / 100;
			if (hp_gain == 0) hp_gain = 1;
		}
		
		if (sd->bonus.magic_sp_drain_per > 0 && (sd->bonus.magic_sp_drain_rate >= 10000 || rnd() % 10000 < sd->bonus.magic_sp_drain_rate)) {
			sp_gain = (ad.damage * sd->bonus.magic_sp_drain_per) / 100;
			if (sp_gain == 0) sp_gain = 1;
		}

		if (hp_gain > 0 || sp_gain > 0) {
			status_heal(src, (int)hp_gain, (int)sp_gain, 1);
		}
	}
	// ==========================================
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	// ==========================================
	// [Fix] จัดการสกิลนกฮันเตอร์ (Auto Bird, Blitz Beat, Falcon Assault)
	// ป้องกันนกบินไปโดนผู้เล่นอื่นตอนปล่อยบอท และห้ามเจาะโล่ Protect
	// ==========================================
	if (sd && tsd && (skill_id == HT_BLITZBEAT || skill_id == SN_FALCONASSAULT)) {
		struct map_data *mapdata = map_getmapdata(target->m);
		
		// ถ้านอกเขตสงครามหลัก และนอกลานประลอง
		if (!mapdata_flag_gvg(mapdata) && !mapdata->getMapFlag(MF_BATTLEGROUND) && !mapdata->getMapFlag(MF_PVP)) {
			if (!(map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) && map_getcell(tsd->bl.m, tsd->bl.x, tsd->bl.y, CELL_CHKPVP))) {
				
				// 1. ถ้ามีคนกางโล่ Protect = บังคับดาเมจ 0 (Miss)
				if (sd->protector || tsd->protector) {
					md.damage = 0;
					md.dmg_lv = ATK_MISS;
					return md;
				}
				
				// 2. ถ้าคนยิงไม่ได้เปิดโหมดนักฆ่า PK = บังคับดาเมจ 0 (Miss) เพื่อกันนกบินสุ่มโดนคน
				if (!(battle_config.pk_pass && sd->attacker)) {
					md.damage = 0;
					md.dmg_lv = ATK_MISS;
					return md;
				}
		...
		case SN_FALCONASSAULT:
			{
				...
				if(!sd || !(skill = pc_checkskill(sd,HT_STEELCROW)))
					skill = 0;
#ifdef RENEWAL
				md.damage = (sstatus->dex / 5 + sstatus->agi / 1 + skill * 3 + 40) * 2;
				RE_LVL_MDMOD(100);
#else
...
struct Damage battle_calc_attack(int attack_type,struct block_list *bl,struct block_list *target,uint16 skill_id,uint16 skill_lv,int flag)
{
	...
	if (sd && bl->type == BL_PC && target->type == BL_PC && mapdata->getMapFlag(MF_REDHEADPK)) {
		status_change *tsc = status_get_sc(target);

		if (tsc != nullptr && !sd->sc.getSCE(SC_REDHEADPK) && !sd->sc.getSCE(SC_PURPLEHEADPK) && !tsc->getSCE(SC_REDHEADPK) && !tsc->getSCE(SC_PURPLEHEADPK)) {
			sc_start(NULL, bl, SC_PURPLEHEADPK, 100, 1, 60000);
		}
	}
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	struct block_list *s_master = battle_get_master(src);
	struct block_list *t_master = battle_get_master(target);
	...
	// ===================================================
	// 🔥 4. กฎเหล็กศาลสูงสุด! ตัดขาดระบบ Auto Attack ล็อคเป้า 100%
	// ===================================================
	if (target->type == BL_MOB) {
		struct mob_data *t_md = (struct mob_data *)target;
		if (t_md->npc_event[0] != '\0' && (strstr(t_md->npc_event, "OnFieldDead") || strstr(t_md->npc_event, "FPCtrl"))) {
			// 🔥 หัวใจหลัก: ถ้าคนที่กำลังจะตี "ไม่ใช่มอนสเตอร์ด้วยกัน" 
			// (แปลว่าต้องเป็น ผู้เล่น, ระบบ Auto-Attack, สัตว์เลี้ยง ฯลฯ)
			if (src->type != BL_MOB) return -1; // -1 = ไม่อนุญาตให้ล็อคเป้าเด็ดขาด!
		}
	}

	if (src->type == BL_MOB && target->type == BL_MOB) {
		struct mob_data *s_md = (struct mob_data *)src;
		struct mob_data *t_md = (struct mob_data *)target;
		bool s_is_clone = (s_md->npc_event[0] != '\0' && (strstr(s_md->npc_event, "OnFieldDead") || strstr(s_md->npc_event, "FPCtrl")));
		bool t_is_clone = (t_md->npc_event[0] != '\0' && (strstr(t_md->npc_event, "OnFieldDead") || strstr(t_md->npc_event, "FPCtrl")));

		// มอนสเตอร์ป่า กับ โคลน เป็นศัตรูกัน (ให้ป่ารุมกัดโคลนได้)
		if (!s_is_clone && t_is_clone) { if (flag & BCT_ENEMY) return 1; }
		if (s_is_clone && !t_is_clone) { if (flag & BCT_ENEMY) return 1; }
		
		// โคลน กับ โคลน ห้ามตีกันเอง!
		if (s_is_clone && t_is_clone) { if (flag & BCT_ENEMY) return -1; }
	}
	// ===================================================
	...
	if( (s_bl = battle_get_master(src)) == NULL )
		s_bl = src;
	// ==========================================
	// [Fix] Cell PVP & PK PASS & City Faction War (ป้องกันดาเมจลูกหลง)
	// ลำดับ: ปาร์ตี้/กิลด์ > ลานประลอง > โหมดรักสงบ > PK PASS > สงครามเมือง
	// ==========================================
	if (s_bl->type == BL_PC && t_bl->type == BL_PC) {
		map_session_data *sd = BL_CAST(BL_PC, s_bl);
		map_session_data *tsd = BL_CAST(BL_PC, t_bl);

		if (sd && tsd && sd->bl.id != tsd->bl.id) {
			
			// [Priority 1] ปาร์ตี้เดียวกัน บังคับห้ามโจมตี
			if (sd->status.party_id > 0 && sd->status.party_id == tsd->status.party_id) {
				if (flag & BCT_ENEMY) return -1;
			}

			// [Priority 2] กิลด์เดียวกัน บังคับห้ามโจมตี
			if (sd->status.guild_id > 0 && sd->status.guild_id == tsd->status.guild_id) {
				if (flag & BCT_ENEMY) return -1;
			}

			struct map_data *mapdata = map_getmapdata(sd->bl.m);

			// [Priority 3] ลานประลอง (Cell PVP)
			if (map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) && 
				map_getcell(tsd->bl.m, tsd->bl.x, tsd->bl.y, CELL_CHKPVP)) {
				state |= BCT_ENEMY;
				strip_enemy = 0;
				return (flag & state) ? 1 : -1;
			}
			
			// ตรวจสอบในพื้นที่ทั่วไป (นอก GvG, PVP, BG)
			if (!mapdata_flag_gvg(mapdata) && !mapdata->getMapFlag(MF_BATTLEGROUND) && !mapdata->getMapFlag(MF_PVP)) {
				
				// [Priority 4] โหมดรักสงบ (Protector Cash Shop)
				if (sd->protector || tsd->protector) {
					if (flag & BCT_ENEMY) return -1; 
				}
				
				// [Priority 5] โหมดนักฆ่า (PK PASS Attacker)
				else if (battle_config.pk_pass && sd->attacker) {
					state |= BCT_ENEMY;
					strip_enemy = 0;
					return (flag & state) ? 1 : -1;
				}
				else if (battle_config.pk_pass && tsd->attacker) {
					state |= BCT_ENEMY;
					strip_enemy = 0;
					return (flag & state) ? 1 : -1;
				}

				// [Priority 6] สงครามเมือง (City Faction War) & ป้องกันโดนลูกหลง
				else if (city_faction_war == 1) {
					int s_faction = (sd->fd == 0) ? 0 : pc_readaccountreg(sd, add_str("#city_faction"));
					int t_faction = (tsd->fd == 0) ? 0 : pc_readaccountreg(tsd, add_str("#city_faction"));;

					if (s_faction > 0 && t_faction > 0) {
						if (!mapdata->getMapFlag(MF_TOWN) && !mapdata->getMapFlag(MF_NOFACTIONWAR)) {
							if (s_faction != t_faction) {
								
								// ถ้าคลิกล็อคเป้าที่ตัวผู้เล่นคนนี้โดยตรง ถึงจะอนุญาตให้ตีเข้า
								if (sd->ud.target == tsd->bl.id || sd->ud.skilltarget == tsd->bl.id) {
									state |= BCT_ENEMY;
									strip_enemy = 0;
									return (flag & state) ? 1 : -1; 
								} else {
									// ถ้าโดนลูกหลงจากสกิลหมู่ หรือกดยิงลงพื้น ให้ดาเมจว่าวไปเลย!
									state &= ~BCT_ENEMY; 
									if (flag & BCT_ENEMY) return -1; 
								}
								...
								state &= ~BCT_ENEMY; 
								if (flag & BCT_ENEMY) return -1; // เมืองเดียวกันห้ามตี
							}
						}
					}
				}
	...
	if ( s_bl->type == BL_PC ) {
		switch( t_bl->type ) {
			case BL_MOB: // Source => PC, Target => MOB
				...
				// ===================================================
				// 🔥 [Custom AI] ป้องกันระบบ Auto-Attack ล็อคเป้าโคลนแบบเด็ดขาด 100%
				// ===================================================
				if (target->type == BL_MOB) {
					struct mob_data *md = (struct mob_data *)target;
					if (md->npc_event[0] != '\0' && (strstr(md->npc_event, "FPCtrl") || strstr(md->npc_event, "OnFieldDead"))) {
						return 0; // 🔥 เปลี่ยนจาก -1 เป็น 0! แปลว่า "เป้าหมายนี้ไม่มีตัวตน"
						          // ระบบ Auto-Attack จะข้ามตัวนี้ไปเลย ไม่มีการเดินไปหาและไม่ยืนเอ๋อแน่นอน!
					}
				}
				// ===================================================
				break;
	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			if( s_bl != t_bl )
			{
				...
				else if( map_getcell( s_bl->m, s_bl->x, s_bl->y, CELL_CHKPVP ) && map_getcell( t_bl->m, t_bl->x, t_bl->y, CELL_CHKPVP ) ) // Addon Cell PVP
				{
					 // เมื่อยืนบนพื้น PVP ทั้งคู่ ให้ถือว่าเป็นศัตรูกันทันที (ข้ามกฎ Mapflag Town)
					 state |= BCT_ENEMY;
					 strip_enemy = 0; // บังคับให้เป็นศัตรูถาวรในโซนนี้
					 return BCT_ENEMY; // ส่งค่ากลับทันทีเพื่อให้ "คลิกซ้ายแล้ววิ่งไปตี" ทำงาน (ไม่ต้องกด Shift)
				}
			}
		...
		case BL_MOB:
		{
			...
			// ===================================================
			// 🔥 [Custom] มอนสเตอร์ป่าปกติ (Aggressive) จะโจมตีร่างโคลนของเรา!
			// ===================================================
			if (target->type == BL_MOB) {
				struct mob_data *t_md = BL_CAST(BL_MOB, target);
				bool s_is_clone = (md->npc_event[0] != '\0' && strstr(md->npc_event, "OnFieldDead"));
				bool t_is_clone = (t_md->npc_event[0] != '\0' && strstr(t_md->npc_event, "OnFieldDead"));

				// ถ้าผู้โจมตีคือ "มอนสเตอร์ป่า (ไม่ใช่โคลน)" และเป้าหมายคือ "โคลน" ให้ถือว่าเป็นศัตรูกันทันที!
				if (!s_is_clone && t_is_clone) {
					state |= BCT_ENEMY;
					strip_enemy = 0;
					break; 
				}
				
				// โคลนไม่ตีโคลนด้วยกันเอง (ป้องกันแลค) ปล่อยให้มันข้ามไป
			}
			// ===================================================
	...
	else if(mapdata->getMapFlag(MF_GREENHEADPK) && s_bl->type == BL_PC)
	{
		if( flag&BCT_ENEMY )
		{
			TBL_PC *sd = (TBL_PC*)s_bl, *sd2 = (TBL_PC*)t_bl;
			...
			int s_guild = status_get_guild_id(s_bl);
			int t_guild = status_get_guild_id(t_bl);
			if(sd2 && sd2->sc.getSCE(SC_REDHEADPK)) {
				if(s_guild && s_guild == t_guild)
					state |= BCT_GUILD;
				else if(s_party && s_party == t_party)
					state |= BCT_PARTY;
				else
					state |= BCT_ENEMY;
			}
	...
	{ //Non pvp/gvg, check party/guild settings.
		...
		{
			int s_party = status_get_party_id(s_bl);
			// ถ้ามีปาร์ตี้เดียวกัน และ config สั่งว่า "ห้ามตีกันเองใน CellPVP" ให้มองเป็นพวกเดียวกัน
		...
		if( flag&BCT_GUILD || state&BCT_ENEMY )
		{
			...
			// ถ้าอยู่กิลด์เดียวกัน/พันธมิตร และ config สั่งว่า "ห้ามตีกันเองใน CellPVP" ให้มองเป็นพวกเดียวกัน
	...
	//PK_PASS
	if( battle_config.pk_pass && s_bl->type == BL_PC && t_bl->type == BL_PC && !mapdata_flag_gvg(mapdata) && !mapdata->getMapFlag(MF_PVP) && !mapdata->getMapFlag(MF_TOWN) && !mapdata->getMapFlag(MF_BATTLEGROUND))
	{
		TBL_PC *sd = (TBL_PC*)s_bl, *sd2 = (TBL_PC*)t_bl;
		if(sd->attacker){
			if ( sd2->protector ){
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "faction_vending_block",              &battle_config.faction_vending_block,           1,      0,      1,              },
...
// rAthena
	...
	{ "city_faction_war", 				  &battle_config.city_faction_war, 				  1, 	  0,	  1, 			  },
	...
	{ "penalty_mob_type",                   &battle_config.penalty_mob_type,                3,      1,      4,              },
	{ "penalty_drop_rate",                  &battle_config.penalty_drop_rate,               100,    0,      100,            },
	{ "minion_hate_range",                  &battle_config.minion_hate_range,               0,      0,      50,             },
	...
	/* HNOMKENG */
	{ "trade_tax",       	                &battle_config.trade_tax,  	 	                0,      0,      1,              },
	...
	//Prevent Block IP [KIRA]
	{ "ip_block_limit",       				&battle_config.ip_block_limit,    			 	0,      0,      INT_MAX,		},	
	{ "prevent_change_ip",       			&battle_config.prevent_change_ip,    			0,      0,      1,				},	
	...
	/* HNOMKENG Item Type */
	{ "itemtype_healing",                   &battle_config.itemtype_healing,                0,      0,      1,              },
	{ "itemtype_usable",                    &battle_config.itemtype_usable,                 0,      0,      1,              },
	{ "itemtype_etc",                       &battle_config.itemtype_etc,                    0,      0,      1,              },
	{ "itemtype_armor",                     &battle_config.itemtype_armor,                  0,      0,      1,              },
	{ "itemtype_weapon",                    &battle_config.itemtype_weapon,                 0,      0,      1,              },
	{ "itemtype_card",                      &battle_config.itemtype_card,                   0,      0,      1,              },
	{ "card_insert_allow_armor",            &battle_config.card_insert_allow_armor,          1,      0,      1,              },
	{ "card_insert_allow_head_top",         &battle_config.card_insert_allow_head_top,       1,      0,      1,              },
	{ "card_insert_allow_hand_l",           &battle_config.card_insert_allow_hand_l,         1,      0,      1,              },
	{ "card_insert_allow_shoes",            &battle_config.card_insert_allow_shoes,          1,      0,      1,              },
	{ "card_insert_allow_garment",          &battle_config.card_insert_allow_garment,        1,      0,      1,              },
	{ "card_insert_allow_hand_r",           &battle_config.card_insert_allow_hand_r,         1,      0,      1,              },
	{ "card_insert_allow_acc_l",           &battle_config.card_insert_allow_acc_l,          1,      0,      1,              },
	{ "card_insert_allow_acc_r",           &battle_config.card_insert_allow_acc_r,          1,      0,      1,              },

	{ "itemtype_petegg",                    &battle_config.itemtype_petegg,                 0,      0,      1,              },
	{ "itemtype_petarmor",                  &battle_config.itemtype_petarmor,               0,      0,      1,              },
	{ "itemtype_ammo",                      &battle_config.itemtype_ammo,                   0,      0,      1,              },
	{ "itemtype_delayconsume",              &battle_config.itemtype_delayconsume,           0,      0,      1,              },
	{ "itemtype_shadowgear",                &battle_config.itemtype_shadowgear,             0,      0,      1,              },
	{ "itemtype_cash",                      &battle_config.itemtype_cash,                   0,      0,      1,              },

	{ "cashpoint_nameid",                   &battle_config.cashpoint_nameid,                0,  	0,   	INT_MAX,        },
	{ "buy_vat",                            &battle_config.buy_vat,                         0,      0,      100,            },
	{ "stall_cash_to_zeny_tax_rate",        &battle_config.stall_cash_to_zeny_tax_rate,     10000,  0,      INT_MAX,        }, // <--- เพิ่มบรรทัดนี้เข้าไปครับ

	{ "stammina_enable",					&battle_config.stamina_enable,					0,		0,		1,				},
	...
	{ "max_sale_card",						&battle_config.max_sale_card,                	1000,   0,   	INT_MAX,        },

	{ "disable_drop_type",					&battle_config.disable_drop_type,				0,		0,		1|2|4|8|16|32,  },
	{ "disable_mob_type",					&battle_config.disable_mob_type,				0,		0,		1|2|4,			},
	...
	{ "autoattack_max_moblist",				&battle_config.autoattack_max_moblist,			0,		0,		INT_MAX,		},
	{ "autoattack_max_itemlist",			&battle_config.autoattack_max_itemlist,			0,		0,		INT_MAX,		},
	{ "autoattack_steal_reduce_rate",		&battle_config.autoattack_steal_reduce_rate,	0,		0,		100,			},
	{ "autoattack_max_time_per_mob",		&battle_config.autoattack_max_time_per_mob,		0,		0,		INT_MAX,		},
	{ "autoattack_mob_struck",				&battle_config.autoattack_mob_struck,			0,		0,		INT_MAX,		},
	{ "autoattack_quiver_check",			&battle_config.autoattack_quiver_check,			0,		0,		INT_MAX,		},
	{ "autoattack_max_flee_mob",			&battle_config.autoattack_max_flee_mob,			0,		0,		INT_MAX,		},
	{ "autoattack_gunslinger_conflip_fix",	&battle_config.autoattack_gunslinger_conflip_fix,	0,		0,		1,			},
	{ "autoattack_coinflip_coin",			&battle_config.autoattack_coinflip_coin,		0,		0,		INT_MAX,		},
	{ "char_bonus_debug",					&battle_config.char_bonus_debug,        		0,      0,      1,              },
	//m_cri
	{ "magic_critical",           			&battle_config.magic_critical,             		1,      0,      1,              },
	{ "magic_critical_option",           	&battle_config.magic_critical_option,           1,      0,      2,              },
	{ "magic_critical_flag",           		&battle_config.magic_critical_flag,             0,		0,		1|2|4, 		    },
	{ "pk_pass",                   			&battle_config.pk_pass,              		    0,      0,      2,              },
	{ "pk_pass_attack_time",				&battle_config.pk_pass_attack_time, 	 		0,      0,      INT_MAX,		},
	{ "pk_pass_protect_time",				&battle_config.pk_pass_protect_time, 	 		0,      0,      INT_MAX,		},		
	{ "pk_pass_attack_effect",				&battle_config.pk_pass_attack_effect, 	 		0,      0,      INT_MAX,		},		
	{ "pk_pass_protect_effect",				&battle_config.pk_pass_protect_effect, 	 		0,      0,      INT_MAX,		},		

	{ "sm_per_limit", 						&battle_config.sm_per_limit, 					500, 	0, 		INT_MAX, 		},
	{ "sm_server_limit", 					&battle_config.sm_server_limit, 				5000, 	0, 		INT_MAX, 		},
	{ "mob_max_level_cap",                  &battle_config.mob_max_level_cap,               0,      0,      INT_MAX,        },
	{ "disable_pet_drops",                  &battle_config.disable_pet_drops,               0,      0,      1,              },
...
int battle_set_value(const char* w1, const char* w2)
{
	...
	//Block IP AND Prevent Change IP SYSTEM [KIRA]
	if (strcmpi(w1, "roplay_ip_whitelist") == 0) {
		safestrncpy(battle_config.roplay_ip_whitelist, w2, sizeof(battle_config.roplay_ip_whitelist));
		return 0;
	}

	// --- แทรกระบบ Advanced Class Penalty ตรงนี้ (ดักจับตัวแปรที่เป็นข้อความ) ---
	if (strcmpi(w1, "penalty_job_list") == 0) {
		memset(battle_config.penalty_job_list, 0, sizeof(battle_config.penalty_job_list));
		char* p = (char*)w2;
		while (p && *p) {
			int job_id = atoi(p);
			if (job_id >= 0 && job_id < 6000) {
				battle_config.penalty_job_list[job_id] = true;
			}
			p = strchr(p, ',');
			if (p) p++;
		}
		return 1; // ส่งค่า 1 (สำเร็จ) เพื่อไม่ให้ระบบไปทำงานต่อข้างล่าง
	}
	// --------------------------------------------------------
...
void battle_set_defaults()
{
	...
	// เพิ่มบรรทัดนี้ลงไปเพื่อเคลียร์ค่า IP Whitelist
	memset(battle_config.roplay_ip_whitelist, 0, sizeof(battle_config.roplay_ip_whitelist));
}
...
{
	// โหลดไฟล์ตั้งค่า Advanced Status Reflect ตอนเปิดรันเซิร์ฟ
	battle_read_custom_yaml();

	// =====================================
	// สูตรโกงให้เซิร์ฟบอกเลข SC_AUTOATTACK
	ShowStatus("===================================\n");
	ShowStatus("HEY! SC_AUTOATTACK ID is: %d\n", SC_AUTOATTACK);
	ShowStatus("===================================\n");
	// =====================================
