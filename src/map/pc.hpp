...
struct s_teleport {
	...
	bool swarm_enable; // teleport/flywing when swarmed (configurable)
	uint16 swarm_count; // minimum mobs targeting to trigger swarm teleport
};
...
struct s_autoattack {
	t_tick last_teleport;
	// Teleport lock: allow at most one teleport per server tick (prevents multi-condition warp spam)
	t_tick teleport_lock_tick;
	t_tick last_move;
	t_tick last_attack;
	t_tick last_pickup;
	// anti-spam for "stuck idle" recovery teleport
	t_tick last_stuck_tp_try;
	t_tick skill_cd;
	t_tick last_hit;
	int attack_target_id;
	int target_id;
	int itempick_id;
	t_tick last_warp_time;
	t_tick last_buff_time;
	// --- AutoAttack extensions ---
	t_tick start_tick;          // when SC_AUTOATTACK started
	...
	int stay_mode;  
  	...
  	int heal_hp;   
	int skill_use_rate;
	int skill_swarm_min = 0; // 0=off, >=1 require mobs targeting player >= X to allow attack skills
...
public:
	...
	struct s_state {
		...
		unsigned int pvp : 1;		// Addon Cell PVP
	...
	// รายการข้อความสรุปบัฟที่ถูก apply แล้ว (ให้ @charbonus แสดงผล)
	std::vector<std::string> active_char_bonuses;
	...
	t_tick pvpcan_walkout_tick;		// Addon Cell PVP
	...
	struct s_bonus {
		...
		// 🔥 ระบบดูดเลือดและมานาด้วยเวทมนตร์
		short magic_hp_drain_rate;
		short magic_hp_drain_per;
		short magic_sp_drain_rate;
		short magic_sp_drain_per;
	...
	// --- Anti-Macro Heuristic System ---
	t_tick macro_last_tick;
	int macro_last_diff;
	int macro_spam_count;
	// -----------------------------------
...
// int pc_count_item_premium_storage(map_session_data *sd, t_itemid id);
