...
enum e_disable_mob : uint8 {
	DIS_MOB_NORMAL	= 0x01,
	DIS_MOB_MINI	= 0x02,
	DIS_MOB_BOSS	= 0x04,
};

extern std::vector<t_itemid> allow_drops;
...
class GradeDropDatabase : public TypesafeYamlDatabase<uint16, s_grade_drop> {
public:
	GradeDropDatabase() : TypesafeYamlDatabase("GRADE_DROP_DB", 1 ){
...
extern GradeDropDatabase grade_drop_db;

struct s_grade_bonus {
...
struct s_grade_effect {
	uint16 grade;
	std::unordered_map<equip_pos, std::shared_ptr<s_grade_bonus>> position;	
};

class GradeEffectDatabase : public TypesafeYamlDatabase<uint16, s_grade_effect> {
public:
	GradeEffectDatabase() : TypesafeYamlDatabase("GRADE_EFFECT_DB", 1 ){
...
extern GradeEffectDatabase grade_effect_db;
...
extern std::vector<t_itemid> no_grade_drops;
extern std::vector<t_itemid> no_grade_effect;
...
#define ITEMID_CASH battle_config.cashpoint_nameid

struct s_global_drop_item {
	t_itemid itemid;
	int rate;
	e_race race;
	e_aegis_monsterclass classes;
	e_item_drop_effect dropeffect;
	bool is_announce;
	bool is_autoloot;
	bool is_identify;
...
struct s_global_drops {
	...
	std::vector<int16> no_mobs;
	std::map<uint16,std::shared_ptr<s_global_drop_map>> ReduceRateMaps;
};
