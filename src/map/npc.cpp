	...
	if( this->nodeExists( node, "Items" ) ){
		for( const ryml::NodeRef& itemNode : node["Items"] ){
			...
			if( !item_exists ){
				...
				item->index = index;
				item->success_rate = 100; // default = 100%
			...
			if( this->nodeExists( itemNode, "SuccessRate" ) ){
				...
				if( !this->asUInt32( itemNode, "SuccessRate", rate ) ){
					return 0;
				}

				if( rate > 100 ){
					this->invalidWarning( itemNode["SuccessRate"], "barter_parseBodyNode: SuccessRate %u is above 100, capping...\n", rate );
					rate = 100;
				}
...
e_purchase_result npc_buylist( map_session_data* sd, std::vector<s_npc_buy_list>& item_list ){
	...
	// process entries in buy list
	for( int i = 0; i < item_list.size(); ++i ){
		t_itemid nameid;
		unsigned short amount;
		int value;

		ARR_FIND( 0, nd->u.shop.count, j, item_list[i].nameid == shop[j].nameid || item_list[i].nameid == itemdb_viewid(shop[j].nameid) );
		if( j == nd->u.shop.count ) return e_purchase_result::PURCHASE_FAIL_COUNT;

		amount = item_list[i].qty;
		nameid = item_list[i].nameid = shop[j].nameid;
		value = shop[j].value;

		std::shared_ptr<item_data> id = item_db.find(nameid);
		if( !id ) return e_purchase_result::PURCHASE_FAIL_COUNT;

		if( !itemdb_isstackable2(id.get()) && amount > 1 ) amount = item_list[i].qty = 1;
		if( nd->master_nd ) continue;

		switch( pc_checkadditem(sd,nameid,amount) ) {
			case CHKADDITEM_NEW: new_ += id->inventorySlotNeeded(amount); break;
			case CHKADDITEM_OVERAMOUNT: return e_purchase_result::PURCHASE_FAIL_WEIGHT;
		}

		if (npc_shop_discount(nd)) value = pc_modifybuyvalue(sd,value);
	...
	if (Sql_Query(mmysql_handle, "SELECT `tax_rate` FROM `mayor_towns` WHERE `town_map` = '%s' AND `is_active` = 1", map_mapid2mapname(sd->bl.m)) == SQL_SUCCESS) {
		if (Sql_NumRows(mmysql_handle) > 0 && Sql_NextRow(mmysql_handle) == SQL_SUCCESS) {
			char* data_ptr = NULL; 
			Sql_GetData(mmysql_handle, 0, &data_ptr, NULL);
			if (data_ptr != NULL) {
				int town_tax = atoi(data_ptr);
				if (town_tax > 0) total_tax_rate = town_tax;
			}
			is_town = 1;
		}
		Sql_FreeResult(mmysql_handle);
	...
	if (z >= 100 && total_tax_rate > 0) {
		tax = (z * (double)total_tax_rate) / 100.0;
		z = z + tax;
	}

	if (z > (double)sd->status.zeny) return e_purchase_result::PURCHASE_FAIL_MONEY;
	if (w + sd->weight > sd->max_weight) return e_purchase_result::PURCHASE_FAIL_WEIGHT;
	...
	if(z >= 100 && tax > 0) {
        ...
        if (is_town) {
            if (Sql_Query(mmysql_handle, "UPDATE `mayor_towns` SET `treasury` = `treasury` + %d WHERE `town_map` = '%s'", (int)tax, map_mapid2mapname(sd->bl.m)) != SQL_SUCCESS) 
                Sql_ShowDebug(mmysql_handle);
        }
...
// ===== ?????????? Dummy Shop (OnSellItem) ????? =====
	...
	int per_limit = battle_config.sm_per_limit; 
	if (per_limit <= 0) per_limit = 500; 

	struct s_sm_item { t_itemid id; int64 price; int limit, sold, pending; };
	std::vector<s_sm_item> sm_cache;
	if (Sql_Query(mmysql_handle, "SELECT item_id, price, server_quota, sold_count FROM `stock_market_data` WHERE status = '2'") != SQL_ERROR) {
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			char* data; s_sm_item entry;
			Sql_GetData(mmysql_handle, 0, &data, NULL); entry.id = (t_itemid)atoll(data);
			Sql_GetData(mmysql_handle, 1, &data, NULL); entry.price = atoll(data);
			Sql_GetData(mmysql_handle, 2, &data, NULL); entry.limit = atoi(data);
			Sql_GetData(mmysql_handle, 3, &data, NULL); entry.sold = atoi(data);
			entry.pending = 0; 
			sm_cache.push_back(entry);
		}
		Sql_FreeResult(mmysql_handle);
	}
	...
	for( i = 0; i < list_length; i++ ) {
		...
		if( idx >= MAX_INVENTORY || idx < 0 || original_amount <= 0 ) continue;
		
		t_itemid nameid = sd->inventory.u.items_inventory[idx].nameid;

		for(auto& sm : sm_cache) { 
			if(sm.id == nameid) {
				char reg_key[32];
				safesnprintf(reg_key, sizeof(reg_key), "#SM_MY_SOLD_%u", nameid);
				int my_sold = pc_readaccountreg(sd, add_str(reg_key));

				int can_p = per_limit - (my_sold + sm.pending);
				int can_s = sm.limit - (sm.sold + sm.pending);
				int can_sell = (can_p < can_s) ? can_p : can_s;
				if (can_sell < 0) can_sell = 0;
	...
	for( i = 0; i < list_length; i++ ) {
		...
		if( idx >= MAX_INVENTORY || idx < 0 || amount <= 0 ) continue;

		t_itemid nameid = sd->inventory.u.items_inventory[idx].nameid;
		if( !nameid || !sd->inventory_data[idx] || sd->inventory.u.items_inventory[idx].amount < amount ) continue;
		if (!pc_can_sell_item(sd, &sd->inventory.u.items_inventory[idx], nd->subtype)) continue;

		int64 item_final_zeny = 0;
		bool found_stock = false;

		for(const auto& sm : sm_cache) {
			if(sm.id == nameid) {
				char reg_key[32];
				safesnprintf(reg_key, sizeof(reg_key), "#SM_MY_SOLD_%u", nameid);
				int my_sold = pc_readaccountreg(sd, add_str(reg_key));

				item_final_zeny = (int64)amount * sm.price;
				
				pc_setaccountreg(sd, add_str(reg_key), my_sold + amount);
				Sql_Query(mmysql_handle, "UPDATE `stock_market_data` SET sold_count = sold_count + %d WHERE item_id = '%u'", amount, nameid);
				found_stock = true; break;
			}
		}

		if(!found_stock) {
			if (battle_config.rental_item_novalue && sd->inventory.u.items_inventory[idx].expire_time) value = 0;
			else value = pc_modifysellvalue(sd, sd->inventory_data[idx]->value_sell);
			item_final_zeny = (int64)value * amount;
		}
		
		z += (double)item_final_zeny;
	...
	if (z >= 100 && battle_config.trade_tax) {
		...
		if (sd != nullptr && sd->bl.m >= 0) {
			if (Sql_Query(mmysql_handle, "SELECT `tax_rate` FROM `mayor_towns` WHERE `town_map` = '%s' AND `is_active` = 1", map_mapid2mapname(sd->bl.m)) == SQL_SUCCESS) {
				if (Sql_NumRows(mmysql_handle) > 0 && Sql_NextRow(mmysql_handle) == SQL_SUCCESS) {
					char* data; Sql_GetData(mmysql_handle, 0, &data, NULL);
					if (data) {
						int town_tax = atoi(data);
		...
		if (total_tax_rate > 0) {
			double tax = (z * total_tax_rate) / 100.0;
			if (tax > 0) {
				z -= tax;
				if (Sql_Query(mmysql_handle, "UPDATE `castle_money` SET `money` = `money` + '%d' WHERE `castle` = 0", (int)tax) != SQL_SUCCESS) Sql_ShowDebug(mmysql_handle);
				if (is_town && sd != nullptr && sd->bl.m >= 0) {
					if (Sql_Query(mmysql_handle, "UPDATE `mayor_towns` SET `treasury` = `treasury` + %d WHERE `town_map` = '%s'", (int)tax, map_mapid2mapname(sd->bl.m)) != SQL_SUCCESS) Sql_ShowDebug(mmysql_handle);
				}
				char out[256]; 
				snprintf(out, sizeof(out), msg_txt(sd, 1535), (int)(z + tax), (int)tax);
	...
	if( battle_config.shop_exp > 0 && z > 0 && ( skill = pc_checkskill(sd,MC_OVERCHARGE) ) > 0)
	{
		...
		if( sd->status.skill[sk_idx].flag >= SKILL_FLAG_REPLACED_LV_0 )
			skill = sd->status.skill[sk_idx].flag - SKILL_FLAG_REPLACED_LV_0;
		if( skill > 0 ) {
			double exp_gain = z * (double)skill * (double)battle_config.shop_exp / 10000.;
			if( exp_gain < 1 ) exp_gain = 1;
			pc_gainexp(sd, NULL, 0, (int)exp_gain, 0);
		}
	}
							...
							for( refine = requirement->refine + 1; refine <= MAX_REFINE; refine++ ){
								for( j = 0; j < MAX_INVENTORY; j++ ){
									if( sd.inventory.u.items_inventory[j].nameid == requirement->nameid ){
										...
										if( battle_config.hide_fav_sell && sd.inventory.u.items_inventory[j].favorite != 0 )
											continue;
										if( sd.inventory.u.items_inventory[j].refine != refine )
											continue;
	...
	for( s_barter_purchase& purchase : purchases ){
		...
		if( purchase.item->stockLimited ){
			...
			if( Sql_Query( mmysql_handle,
						   "REPLACE INTO `%s` (`name`,`index`,`amount`) VALUES ( '%s', '%hu', '%hu' )",
						   barter_table, barter->name.c_str(), purchase.item->index, purchase.item->stock ) != SQL_SUCCESS ){
				Sql_ShowDebug( mmysql_handle );
				return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
			}
		}
	...
	switch( mapflag ){
		...
		case MF_INVALID:
			if (!strcmpi(w3, "mob_damagerate")) {
				if (state) map_getmapdata(m)->mob_damagerate = atoi(w4);
				else map_getmapdata(m)->mob_damagerate = 0;
				break;
			}
			if (!strcmpi(w3, "custom_skill_dmg")) {
				if (state) {
					...
					map_getmapdata(m)->custom_skill_dmg.clear();
				}
				break;
			}
...
const char *npc_get_script_event_name(int npce_index)
{
	...
	case NPCE_EQUIPITEM: return "OnPCEquipItemEvent";
	case NPCE_UNEQUIPITEM: return "OnPCUnequipItemEvent";
	case NPCE_BASELVUP:
