...
static inline unsigned char clif_bl_type(struct block_list *bl, bool walking) {
	...
	case BL_MOB:
		// ===================================================
		// 🔥 [Custom] หลอก Client ว่าร่างโคลนคือ "ผู้เล่น (PC)" ดาบจะได้ไม่ขึ้น
		// ===================================================
		if (((mob_data*)bl)->npc_event[0] != '\0' && 
		   (strstr(((mob_data*)bl)->npc_event, "FPCtrl") || strstr(((mob_data*)bl)->npc_event, "OnFieldDead"))) {
			return 0x0; // 0x0 คือ PC_TYPE หน้าจอจะมองเป็นคนปกติ เมาส์ไม่ขึ้นรูปดาบ!
		}
		// ===================================================
...
{
	// 🌟 ป้องกันจอฟ้าเวลาเตะบอทออกจากแมพ!
	...
	case BL_PC:
		{
			...
			//PK PASS & Cell PVP     
			// ถ้ามีบัฟ PK PASS หรือ ยืนอยู่บนพิกัดลานประลอง ให้เปิดออร่าแดงทันที
			if(sd->attacker || map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP))
			    pk_pass_attack_effect(bl);
			    
			if(sd->protector)
			    pk_pass_protect_effect(bl);	

			// ==========================================
			// [Addon] Faction War Aura (โชว์เฉพาะอยู่นอกเมือง และไม่อยู่ในกิลวอ)
			// ==========================================
			if (city_faction_war == 1) {
				struct map_data *mapdata = map_getmapdata(bl->m);
				int my_faction = pc_readaccountreg(sd, add_str("#city_faction"));
				
				// เช็กว่าต้องไม่ใช่แมพ Town, ไม่ใช่แมพ Nofactionwar และ **ไม่ใช่แมพกิลวอ**
				if (mapdata != nullptr && !mapdata->getMapFlag(MF_TOWN) && !mapdata->getMapFlag(MF_NOFACTIONWAR) && 
				    !mapdata_flag_gvg(mapdata) && !mapdata_flag_gvg2(mapdata) && !mapdata->getMapFlag(MF_GVG_TE)) {
					if (my_faction == 1) {
						clif_hat_effect_single(sd, 228, true); // เปิดออร่าแดง Morocc
					} else if (my_faction == 2) {
						clif_hat_effect_single(sd, 229, true); // เปิดออร่าฟ้า Prontera
					}
				} else {
					// วาร์ปเข้าเมือง หรือเข้ากิลวอปุ๊บ ต้องสั่ง "ปิดออร่า" ทันที!
					if (my_faction == 1) {
						clif_hat_effect_single(sd, 228, false); // ปิดออร่าแดง
					} else if (my_faction == 2) {
						clif_hat_effect_single(sd, 229, false); // ปิดออร่าฟ้า
					}
				}
			}
			// ==========================================
...
void clif_walkok(map_session_data *sd)
{
	...
	// 🌟 ป้องกันจอฟ้า! ถ้านี่คือบอท (ไม่มี socket) ห้ามส่งแพ็กเก็ตเดิน
	if (!session_isActive(fd)) return; 
	...
	case BL_PC:
		{
			...
			clif_autoattack_effect(bl);
			//PK PASS & Cell PVP     
			if(tsd->attacker || tsd->state.pvp)
			    pk_pass_attack_effect(bl);
			if(tsd->protector)
			    pk_pass_protect_effect(bl);

			// ==========================================
			// [Addon] Faction War Aura (โชว์ให้คนที่เดินเข้ามาในจอเห็น และไม่โชว์ในเมือง/กิลวอ)
			// ==========================================
			if (city_faction_war == 1) {
				struct map_data *mapdata = map_getmapdata(bl->m);
				int t_faction = pc_readaccountreg(tsd, add_str("#city_faction"));

				// เช็กว่าต้องไม่ใช่แมพ Town, ไม่ใช่แมพ Nofactionwar และ **ไม่ใช่แมพกิลวอ**
				if (mapdata != nullptr && !mapdata->getMapFlag(MF_TOWN) && !mapdata->getMapFlag(MF_NOFACTIONWAR) && 
				    !mapdata_flag_gvg(mapdata) && !mapdata_flag_gvg2(mapdata) && !mapdata->getMapFlag(MF_GVG_TE)) {
					if (t_faction == 1) {
						clif_hat_effect_single(tsd, 228, true); // เปิดออร่าแดง
					} else if (t_faction == 2) {
						clif_hat_effect_single(tsd, 229, true); // เปิดออร่าฟ้า
					}
				} else {
					// วาร์ปเข้าเมือง หรือเข้ากิลวอปุ๊บ ต้องสั่งให้คนอื่นเห็นว่าออร่าดับลง
					if (t_faction == 1) {
						clif_hat_effect_single(tsd, 228, false); // ปิดออร่าแดง
					} else if (t_faction == 2) {
						clif_hat_effect_single(tsd, 229, false); // ปิดออร่าฟ้า
					}
				}
			}
			// ==========================================
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	bool pk_pass = (sd && sd->attacker == true);
	bool cell_pvp = (sd && sd->state.pvp == 1); 

	// จะขึ้นเมาส์ดาบก็ต่อเมื่อ: สวิตช์เปิด + มีสังกัด + ไม่ใช่ Town + ไม่ใช่แมพ nofactionwar
	int my_faction = sd ? pc_readaccountreg(sd, add_str("#city_faction")) : 0;
	bool faction_war = (city_faction_war == 1 && my_faction > 0 && !mapdata->getMapFlag(MF_TOWN) && !mapdata->getMapFlag(MF_NOFACTIONWAR));

	// ยัด faction_war เข้าไปในบิตที่ 0, 4 และ 5 
	WBUFL(buf,4) = ((mapdata->getMapFlag(MF_PVP) || (sd && sd->duel_group > 0) || pk_pass || cell_pvp || faction_war )<<0)|
		...
		((mapdata->getMapFlag(MF_NOLOCKON) || mapdata_flag_vs(mapdata) || (sd && sd->duel_group > 0) || pk_pass || cell_pvp || faction_war )<<4)|
        ((mapdata->getMapFlag(MF_PVP) || cell_pvp || faction_war)<<5)|
		...
		(mapdata->getMapFlag(MF_GREENHEADPK));
#endif
...
std::string size_string(int size)
{
	switch (size){
		case SZ_SMALL:
			return msg_txt(NULL,2041);
		case SZ_MEDIUM:
			return msg_txt(NULL,2042);
		case SZ_BIG:
			return msg_txt(NULL,2043);
		default:
			return msg_txt(NULL,2044);
	}
}

std::string race_string(int race)
{
	switch (race){
		case RC_FORMLESS:
			return msg_txt(NULL,2021);
		case RC_UNDEAD:
			return msg_txt(NULL,2022);
		case RC_BRUTE:
			return msg_txt(NULL,2023);
		case RC_PLANT:
			return msg_txt(NULL,2024);
		case RC_INSECT:
			return msg_txt(NULL,2025);
		case RC_FISH:
			return msg_txt(NULL,2026);
		case RC_DEMON:
			return msg_txt(NULL,2027);
		case RC_DEMIHUMAN:
			return msg_txt(NULL,2028);
		case RC_ANGEL:
			return msg_txt(NULL,2029);
		case RC_DRAGON:
			return msg_txt(NULL,2030);
		case RC_PLAYER_HUMAN:
			return msg_txt(NULL,2031);
		case RC_PLAYER_DORAM:
			return msg_txt(NULL,2032);
		default:
			return msg_txt(NULL,2033);
	}
}

std::string element_string(int element)
{
	switch (element){
		case ELE_NEUTRAL:
		...
		case ELE_EARTH:
			return msg_txt(NULL,2003);
		case ELE_FIRE:
			return msg_txt(NULL,2004);
		case ELE_WIND:
			return msg_txt(NULL,2005);
		case ELE_POISON:
			return msg_txt(NULL,2006);
		case ELE_HOLY:
			return msg_txt(NULL,2007);
		case ELE_DARK:
			return msg_txt(NULL,2008);
		case ELE_GHOST:
			return msg_txt(NULL,2009);
		case ELE_UNDEAD:
			return msg_txt(NULL,2010);
		case ELE_ALL:
			return msg_txt(NULL,2011);
		default:
			return msg_txt(NULL,2012);
	}
}
...
void clif_name( struct block_list* src, struct block_list *bl, send_target target ){
	...
	switch( bl->type ){
		...
		case BL_MOB: {
			...
			// 🔥 [Fake Player] บังคับลบรายละเอียดทั้งหมด (Lv/HP) ให้เหลือแค่ชื่อเหมือนผู้เล่นจริง
			if (md->vd->class_ < 1000 || (md->vd->class_ >= 4000 && md->vd->class_ < 5000)) {
				PACKET_ZC_ACK_REQNAMEALL_NPC packet = { 0 };
				packet.packet_id = HEADER_ZC_ACK_REQNAMEALL_NPC;
				packet.gid = bl->id;
				...
				str_p += sprintf(str_p, "Lv. %d | %d", md->level, md->mob_id);
				str_p2 += sprintf(str_p2, "[%s][%s]",size_string(md->status.size).c_str(),race_string(md->status.race).c_str());
				str_p3 += sprintf(str_p3, "%s Lv. %d", element_string(md->status.def_ele).c_str(),md->status.ele_lv);
...
// ==========================================
	// [Addon Cell PVP] เช็กพิกัดทุกครั้งที่โหลดหน้าจอเสร็จ
	// (แก้บั๊กวาร์ปเข้าลาน, ล็อกอินในลาน, หรือตายแล้วเกิดใหม่ในลาน แล้วตีไม่ได้)
	// ==========================================
	if (map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP)) {
		// ถ้าโหลดหน้าจอเสร็จแล้วพบว่ายืนอยู่บนพื้น PVP ให้ยัดสถานะเมาส์ดาบและออร่าทันที!
		map_pvp_area(sd, true);
	}
	// ==========================================
	
	//[KIRA] PK PASS	
	if ((sd->attacker || sd->protector) && mapindex_id2name(sd->mapindex) && util::vector_exists(mapname_list, std::string(mapindex_id2name(sd->mapindex)))) {
		clif_end_pk_pass(sd);		
	}

	//[KIRA] PK PASS	
	if ((sd->attacker || sd->protector) && mapdata_flag_gvg(mapdata) && mapdata->getMapFlag(MF_PVP) && mapdata->getMapFlag(MF_TOWN) && mapdata->getMapFlag(MF_BATTLEGROUND)) {
		clif_end_pk_pass(sd);		
	}		
	...
	else if(mapdata->getMapFlag(MF_GREENHEADPK)) {
		clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
	} else
...
void clif_parse_ActionRequest_sub(map_session_data *sd, int action_type, int target_id, t_tick tick)
{
	...
	// [Custom] บังคับหยุดเดินทันทีสำหรับทุก Action (รวมถึงการโจมตี 0x00, 0x07)
	// สลัดทิ้งดีเลย์ Step Action ทำให้ตัวละครหันหน้าและพุ่งใส่มอนสเตอร์ทันทีแบบ Real-time
...
void clif_parse_TakeItem(int fd, map_session_data *sd)
{
	...
	do {
		...
		// [Custom] บังคับหยุดเดินทันทีเมื่อคลิกเก็บไอเทม
		// ทำให้ Pathfinding คำนวณเส้นทางพุ่งไปหาไอเทมใหม่ทันที ไม่ต้องรอให้เดินจบช่องเดิม
		pc_stop_walking(sd, 1);
		pc_stop_attack(sd);
...
// =========================================================
// Anti-Macro System Helper Function (Heuristic Detection)
// =========================================================
static bool clif_antimacro_check(struct map_session_data *sd) {
	if (!sd) return false;

	t_tick current_tick = gettick();
	int diff = (int)(current_tick - sd->macro_last_tick);
	
	// ถ้าระยะห่างน้อยกว่า 300ms และความคลาดเคลื่อนไม่เกิน 2ms
	if (diff > 0 && diff < 300 && abs(diff - sd->macro_last_diff) <= 2) {
		sd->macro_spam_count++;
		
		// ถ้ากดเป๊ะๆ ติดต่อกัน 15 ครั้ง
		if (sd->macro_spam_count > 15) {
			ShowWarning("Anti-Macro: ตรวจพบผู้เล่น %s ใช้โปรแกรมมาโคร! (Diff: %d ms)\n", sd->status.name, diff);
			
			char msg[256];
			sprintf(msg, "ผู้เล่น [%s] ถูกระบบลงโทษใบ้แดง 3 วัน และส่งเข้าคุก เนื่องจากตรวจพบการใช้มาโคร!", sd->status.name);
			intif_broadcast(msg, strlen(msg), 0);
			
			sc_start(&sd->bl, &sd->bl, SC_SILENCE, 10000, 0, 259200000);
			pc_setpos(sd, map_mapname2mapid("sec_pri"), 0, 0, CLR_OUTSIGHT);
			
			sd->macro_spam_count = 0;
			return true; // บล็อกคำสั่ง
		}
	} else {
		sd->macro_spam_count = 0;
	}
	
	sd->macro_last_diff = diff;
	sd->macro_last_tick = current_tick;
	return false; // ปล่อยผ่าน
}

// =========================================================
// Anti-Macro System (Map, Timer & IP Logger) - 4 Files
// =========================================================
static std::unordered_set<int> antimacro_item_list;
static std::unordered_set<int> antimacro_skill_list;
static std::unordered_set<std::string> antimacro_map_list; 
static bool check_all_items = false;
static bool check_all_skills = false;
static bool check_all_maps = false;
static int antimacro_ban_time = 10;

void clif_load_antimacro_config() {
	antimacro_item_list.clear();
	antimacro_skill_list.clear();
	antimacro_map_list.clear();
	check_all_items = false;
	check_all_skills = false;
	check_all_maps = false;
	antimacro_ban_time = 10; 
	
	char line[256];
	char val_str[64];

	// 1. โหลดเวลาแบน
	FILE* fp_time = fopen("db/custom/antimacro_time.txt", "r");
	if (fp_time) {
		while (fgets(line, sizeof(line), fp_time)) {
			if (line[0] == '/' && line[1] == '/') continue;
			if (sscanf(line, "%63s", val_str) == 1) {
				antimacro_ban_time = atoi(val_str);
				break;
			}
		}
		fclose(fp_time);
	...
	// 2. โหลดแผนที่
	FILE* fp_map = fopen("db/custom/antimacro_map.txt", "r");
	if (fp_map) {
		while (fgets(line, sizeof(line), fp_map)) {
			if (line[0] == '/' && line[1] == '/') continue;
			if (sscanf(line, "%63s", val_str) == 1) {
				if (strcasecmp(val_str, "all") == 0) check_all_maps = true;
				else antimacro_map_list.insert(val_str);
			}
		}
		fclose(fp_map);
		if (antimacro_map_list.empty()) check_all_maps = true;
	} else {
		check_all_maps = true;
	}

	// 3. โหลดไอเทม
	FILE* fp_item = fopen("db/custom/antimacro_item.txt", "r");
	if (fp_item) {
		while (fgets(line, sizeof(line), fp_item)) {
			if (line[0] == '/' && line[1] == '/') continue;
			if (sscanf(line, "%63s", val_str) == 1) {
				if (strcasecmp(val_str, "all") == 0) check_all_items = true;
				else if (atoi(val_str) > 0) antimacro_item_list.insert(atoi(val_str));
			}
		}
		fclose(fp_item);
	}
	...
	// 4. โหลดสกิล
	FILE* fp_skill = fopen("db/custom/antimacro_skill.txt", "r");
	if (fp_skill) {
		while (fgets(line, sizeof(line), fp_skill)) {
			if (line[0] == '/' && line[1] == '/') continue;
			if (sscanf(line, "%63s", val_str) == 1) {
				if (strcasecmp(val_str, "all") == 0) check_all_skills = true;
				else if (atoi(val_str) > 0) antimacro_skill_list.insert(atoi(val_str));
			}
		}
		fclose(fp_skill);
	}

	ShowStatus("Done reading '" CL_WHITE "db/custom/antimacro" CL_RESET "' files.\n");
}

static bool clif_antimacro_check(struct map_session_data *sd, int type, int id) {
	if (!sd) return false;

	// 1. เช็คแผนที่ (Map Check)
	if (!check_all_maps) {
		std::string mapname = map_mapid2mapname(sd->bl.m);
		if (antimacro_map_list.find(mapname) == antimacro_map_list.end()) {
			return false; 
		}
	}

	// 2. เช็ค Item/Skill
	if (type == 1 && !check_all_items && antimacro_item_list.find(id) == antimacro_item_list.end()) {
		sd->macro_spam_count = 0; return false;
	}
	if (type == 2 && !check_all_skills && antimacro_skill_list.find(id) == antimacro_skill_list.end()) {
		sd->macro_spam_count = 0; return false;
	}
	...
	if (diff > 0 && diff < 300 && abs(diff - sd->macro_last_diff) <= 2) {
		...
		if (sd->macro_spam_count > 15) {
			...
			if (antimacro_ban_time > 0) {
				ShowWarning("Anti-Macro: ผู้เล่น %s ถูกแบน %d นาที! (Type: %d, ID: %d, Diff: %d ms)\n", sd->status.name, antimacro_ban_time, type, id, diff);
				sprintf(msg, "ผู้เล่น [%s] ถูกแบนและส่งเข้าคุกเป็นเวลา %d นาที เนื่องจากตรวจพบมาโคร!", sd->status.name, antimacro_ban_time);
				intif_broadcast(msg, strlen(msg), 0);
				
				sc_start(&sd->bl, &sd->bl, SC_SILENCE, 10000, 0, antimacro_ban_time * 60000);
				pc_jail(*sd, antimacro_ban_time * 60);
			} else {
				ShowWarning("Anti-Macro: ผู้เล่น %s ถูกแบนถาวรและเก็บ IP! (Type: %d, ID: %d)\n", sd->status.name, type, id);
				sprintf(msg, "ผู้เล่น [%s] ถูกแบนถาวรและ Log IP เนื่องจากตรวจพบการใช้มาโคร!", sd->status.name);
				intif_broadcast(msg, strlen(msg), 0);
				
				// ป้องกันการแครชด้วยการเช็คว่า Session แจกจ่าย IP ปกติหรือไม่
				if (session_isValid(sd->fd)) {
					uint32 ip = session[sd->fd]->client_addr;
					FILE* fp_log = fopen("log/macro_ip_ban.log", "a");
					if (fp_log) {
						fprintf(fp_log, "MACRO BAN: Account: %d | Char: %s | IP: %d.%d.%d.%d\n", 
								sd->status.account_id, sd->status.name, CONVIP(ip));
						fclose(fp_log);
					}
				}
				
				sc_start(&sd->bl, &sd->bl, SC_SILENCE, 10000, 0, 315360000); 
				pc_jail(*sd, INT_MAX);
			}
...
void clif_parse_UseItem(int fd, map_session_data *sd)
{
	...
	// --- Anti-Macro Check ---
	if (clif_antimacro_check(sd)) return;
	// ------------------------
	...
	// --- ดักมาโครกดไอเทม ---
	if (clif_antimacro_check(sd, 1, sd->inventory.u.items_inventory[n].nameid)) return;
	// ----------------------
...
void clif_parse_skill_toid( map_session_data* sd, uint16 skill_id, uint16 skill_lv, int target_id ){
	...
	// --- ดักมาโครร่ายสกิลเป้าหมาย/รอบตัว ---
	if (clif_antimacro_check(sd, 2, skill_id)) return;
	// ---------------------------------
...
void clif_hat_effects( map_session_data* sd, struct block_list* bl, enum send_target target ){
	...
	auto send_hat_effects = [&]( enum send_target sendTarget, bool includeVoiceMic ){
		...
		p->packetLength = sizeof( struct PACKET_ZC_EQUIPMENT_EFFECT );
		p->aid = tsd->bl.id;
		p->status = 1;

		size_t count = 0;
		for( int16 effect : tsd->hatEffects ){
			if( !includeVoiceMic && effect == HAT_EF_MIC ){
				continue;
			}

			p->effects[count++] = effect;
			p->packetLength += sizeof( int16 );
		}
	...
	}else if( target == AREA && util::vector_exists( tsd->hatEffects, static_cast<int16>( HAT_EF_MIC ) ) ){
		send_hat_effects( AREA, false );

		struct PACKET_ZC_EQUIPMENT_EFFECT* p = (struct PACKET_ZC_EQUIPMENT_EFFECT*)packet_buffer;
		p->packetType = HEADER_ZC_EQUIPMENT_EFFECT;
		p->packetLength = sizeof( struct PACKET_ZC_EQUIPMENT_EFFECT ) + sizeof( int16 );
		p->aid = tsd->bl.id;
		p->status = 1;
...
//[KIRA] PK PASS ON
void pk_pass_attack_effect(struct block_list* bl){
#if PACKETVER_MAIN_NUM >= 20150507 || PACKETVER_RE_NUM >= 20150429 || defined(PACKETVER_ZERO)

	nullpo_retv( bl );

	if(!battle_config.pk_pass_attack_effect)
		return;
	...
	p->aid = bl->id;
	p->status = 1;
	p->effects[BL_CAST(BL_PC,bl)->hatEffects.size()] = battle_config.pk_pass_attack_effect;

	clif_send( p, p->packetLength, bl, AREA);
#endif
}

void pk_pass_protect_effect(struct block_list* bl){
#if PACKETVER_MAIN_NUM >= 20150507 || PACKETVER_RE_NUM >= 20150429 || defined(PACKETVER_ZERO)

	nullpo_retv( bl );

	if(!battle_config.pk_pass_protect_effect)
		return;
	...
	p->aid = bl->id;
	p->status = 1;
	p->effects[BL_CAST(BL_PC,bl)->hatEffects.size()] = battle_config.pk_pass_protect_effect;

	clif_send( p, p->packetLength, bl, AREA);
#endif
}
//[KIRA] PK PASS OFF
void pk_pass_attack_effect_off(struct block_list* bl){
#if PACKETVER_MAIN_NUM >= 20150507 || PACKETVER_RE_NUM >= 20150429 || defined(PACKETVER_ZERO)
...
void pk_pass_protect_effect_off(struct block_list* bl){
#if PACKETVER_MAIN_NUM >= 20150507 || PACKETVER_RE_NUM >= 20150429 || defined(PACKETVER_ZERO)
...
void clif_end_pk_pass(map_session_data* sd){
	if (!sd) 
		return;
...
void clif_hat_effect_single_target( map_session_data& sd, uint16 effectId, bool enable, enum send_target target ){
#if PACKETVER_MAIN_NUM >= 20150507 || PACKETVER_RE_NUM >= 20150429 || defined(PACKETVER_ZERO)
	struct PACKET_ZC_EQUIPMENT_EFFECT* p = (struct PACKET_ZC_EQUIPMENT_EFFECT*)packet_buffer;
...
void clif_hat_effect_single( map_session_data* sd, uint16 effectId, bool enable ){
	nullpo_retv( sd );
	clif_hat_effect_single_target( *sd, effectId, enable, AREA );
}
...
#else
	safestrncpy( item_name, p->item_name, min(p->packetLength, ITEM_NAME_LENGTH) );	std::shared_ptr<item_data> id = item_db.searchname( item_name );
...
void clif_CashShopLimited(map_session_data* sd ){
	...
	std::shared_ptr<s_cash_item_tab> tab = cash_shop_db.find(CASHSHOP_TAB_SALE);
	...
	for(int i = 0; i < tab->items.size(); i++ ){
		sale_item = sale_find_item(tab->items[i]->nameid,true);
		if(sale_item && sale_item->start <= now && sale_item->end > now ){
			...
			p->list[count].price = tab->items[i]->price;
...
//
//	if(!p->blacksmithBlessing)
//	{
//		clif_displaymessage(sd->fd, msg_txt(sd, 1551));
//		clif_refineui_info( sd, index );
//		return;
//	}
//	
...
//		char boardcast_msg[CHAT_SIZE_MAX];
//		memset(boardcast_msg, '\0', sizeof(boardcast_msg));
//		sprintf(boardcast_msg,msg_txt(sd,1703),char_name,id->ename.c_str(),item->refine+1);
...
//	uint16 blacksmith_amount = 0;
//
//	// Require blessing toggle (used as mandatory ingredient only)
//	if(!p->blacksmithBlessing)
//	{
//		clif_displaymessage(sd->fd, msg_txt(sd, 1551));
//		clif_refineui_info( sd, index );
...
//	}
//
//	// Check & require Blacksmith Blessing as ingredient (no protection effect)
//	if( p->blacksmithBlessing != 0 ){
//		blacksmith_amount = info->blessing_amount;
//		if( blacksmith_amount == 0 ){
//			return;
...
//	// Pay zeny
//	if( pc_payzeny( sd, cost->zeny, LOG_TYPE_CONSUME ) ){
//		clif_npc_buy_result( sd, e_purchase_result::PURCHASE_FAIL_MONEY );
//		return;
//	}
//
//	// Consume selected material
//	if( pc_delitem( sd, j, 1, 0, 0, LOG_TYPE_CONSUME ) ){
//		return;
//	}
//
//	// Consume blessing (ingredient only)
//	if( blacksmith_amount > 0 && pc_delitem( sd, blacksmith_index, blacksmith_amount, 0, 0, LOG_TYPE_CONSUME ) ){
//		return;
//	}
//
...
//	}else{
//		// Failure policy:
//		// - If attempting to reach +1 .. +4  => downgrade -1 (never break)
//		// - If attempting to reach +5 or higher => break
//		uint16 target_refine = (uint16)cap_value(item->refine + 1, 0, MAX_REFINE);
//
//		char char_name[NAME_LENGTH];
//		if( battle_config.broadcast_hide_name ){
//			std::string dispname = clif_hide_name(sd->status.name);
...
//
//		if( target_refine <= 4 ){
//			// Downgrade -1 (no break)
//			item->refine = cap_value( item->refine - 1, 0, MAX_REFINE );
//
//			// re-apply refine random options for downgraded level (if configured)
//			std::shared_ptr<s_refineopt> RefineRandomOpt = refine_randomopt_db.find(item->refine);
//			if(RefineRandomOpt != nullptr){
//				for(const auto& group : RefineRandomOpt->groups){
//					if(util::vector_exists(group->items, item->nameid)) {
...
//			}
//
//			clif_refine( fd, 2, index, item->refine ); // downgrade
//			clif_refineui_info(sd, index);
//		}else{
//			// Break (for +5 and above attempts)
//			clif_refine( fd, 1, index, item->refine );
//			pc_delitem( sd, index, 1, 0, 2, LOG_TYPE_CONSUME );
//		}
//
...
void clif_parse_refineui_refine( int fd, map_session_data* sd ){
	...
	// Pick refine cost row by chosen material
	std::shared_ptr<s_refine_cost> cost = nullptr;
	for( const auto& pair : info->costs ){
		if( pair.second->nameid == material ){
			cost = pair.second;
	...
	// --- Robust Enriched detector (เช็คทั้ง type/flag/ชื่อไอเท็ม) ---
	auto icontains_ci = [](const std::string& s, const char* needle)->bool{
	...
	auto is_enriched_material = [&](t_itemid nameid)->bool {
		// 1) ถ้ามี enum/flag ในสตรัคเจอร์ (ใส่เองตามโค้ดเบสของคุณ)
		#ifdef REFINE_MATERIAL_TYPE_ENRICHED
		if (cost && cost->type == REFINE_MATERIAL_TYPE_ENRICHED) return true;
		#endif
		#ifdef REFINE_COST_FLAG_ENRICHED
		if (cost && (cost->flags & REFINE_COST_FLAG_ENRICHED)) return true;
		#endif
		// 2) ตรวจจากชื่อไอเท็มใน item_db
		if (struct item_data* mdb = itemdb_search(nameid)){
			if( icontains_ci(mdb->ename, "Enriched") ) return true;
			#ifdef HAVE_JNAME
			if( icontains_ci(mdb->jname, "Enriched") ) return true;
			#endif
			#ifdef HAVE_NAME
			if( icontains_ci(mdb->name,  "Enriched") ) return true;
			#endif
		}
		// 3) ฟอลล์แบ็กไอดีคงที่ (ถ้ามีประกาศ)
		#ifdef ITEMID_ENRICHED_ELUNIUM
		if (nameid == ITEMID_ENRICHED_ELUNIUM) return true;
		#endif
		#ifdef ITEMID_ENRICHED_ORIDECON
		if (nameid == ITEMID_ENRICHED_ORIDECON) return true;
		#endif
		#ifdef ITEMID_ITEMID_GOLD
		if (nameid == ITEMID_GOLD) return true;
		#endif
		return false;
	};
	...
	if( cost->chance >= ( rnd() % 10000 ) ){
		...
		// ===== Failure =====
		uint16 target_refine = (uint16)cap_value(item->refine + 1, 0, MAX_REFINE);
		const bool enriched = is_enriched_material(material);
		...
		// ---- OVERRIDE: ใช้ Enriched => เฟล/แตก = ไม่แตก แต่ "ลดขั้น -1" เสมอ
		if( enriched ){
			item->refine = cap_value( item->refine - 1, 0, MAX_REFINE );

			// re-apply refine random options for downgraded level (ถ้ามี)
			if(auto RefineRandomOpt = refine_randomopt_db.find(item->refine)){
				for(const auto& group : RefineRandomOpt->groups){
					if(util::vector_exists(group->items, item->nameid)) {
						group->option_group->apply_refine(sd, *item, false);
						clif_delitem(sd, index, 1, 3);
			...
			clif_misceffect( &sd->bl, 2 );
			achievement_update_objective( sd, AG_ENCHANT_FAIL, 1, 1 );
			return; // สำคัญ: หยุดที่นี่ ไม่ให้ไปเข้ากิ่ง "แตก"
		}

		// ---- ปกติ (Type: Normal)
		// +1..+4 เฟล = ลด 1, +5 ขึ้นไป เฟล = แตก
		if( target_refine <= 4 ){
			item->refine = cap_value( item->refine - 1, 0, MAX_REFINE );
			...
			clif_refine( fd, 1, index, item->refine );                 // break
			pc_delitem( sd, index, 1, 0, 2, LOG_TYPE_CONSUME );        // ลบไอเท็มหลัก
		}
...
void clif_partybooking_ask( map_session_data* sd, map_session_data* joining_sd ){
    ...
    // Check if the party leader is in SC_AUTOATTACK state
    if (sd->sc.getSCE(SC_AUTOATTACK)) {
	if (sd->aa.party_auto == 1){ 		
        // Automatically accept the join request without showing UI
        party_join(joining_sd, sd->status.party_id);
        clif_partybooking_reply(joining_sd, sd, true);
        return;
    }
	}
...
void clif_parse_partybooking_join(int fd, map_session_data* sd) {
    ...
    // Remove existing requests from the target player
    auto& requests = tsd->party_booking_requests;
    auto it = std::find(requests.begin(), requests.end(), sd->status.char_id);
    if (it != requests.end()) {
        requests.erase(it);  // Remove the existing request
    }

    // Store information that the player tried to join the party
    requests.push_back(sd->status.char_id);

    // Check if the party leader is in SC_AUTOATTACK state
    if (tsd->sc.getSCE(SC_AUTOATTACK)) {
	if (sd->aa.party_auto == 1){ 	
        // Automatically accept the join request without showing UI
        party_join(sd, tsd->status.party_id);
        clif_partybooking_reply(sd, tsd, true);
        return;
	}	
    }
...
void clif_stall_vending_open(map_session_data *sd) {
	...
	for( int i = 0; i < MAX_INVENTORY; i++ ) {
		if( sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory_data[i] ) {
			if( sd->inventory_data[i]->type != IT_WEAPON && sd->inventory_data[i]->type != IT_ARMOR && sd->inventory_data[i]->type != IT_SHADOWGEAR && sd->inventory_data[i]->type != IT_CARD )
				continue;
...
void clif_stall_vending_list(map_session_data *sd, s_stall_data *st) {
	...
	if( st->vended_id == sd->status.char_id )
		...
		p->myStall = 0;
	p->expireTime = static_cast<int>((st->expire_time - time(NULL)) * 0); //if 0 == unlimited on client..
...
void do_init_clif(void) {
	...
	// --- โหลดรายชื่อไอเทม/สกิลที่ต้องตรวจจับมาโคร ---
	clif_load_antimacro_config();
	// ----------------------------------------
