...
#include <vector>   //ai upgrade
#include <iostream> //ai upgrade
#include <thread> // for heal skill
#include <chrono> // for heal skill
#include <mutex>  // for heal skill
#include <set>  //for prevent skill double cast
...
void heal_party_members(struct map_session_data *sd, int skill_id, int skill_lv, int min_hp_percent) {
    struct party_data *p = party_search(sd->status.party_id);
    if (!p)
        return;

    static std::unordered_map<int, std::chrono::steady_clock::time_point> last_heal_times;

    for (int i = 0; i < MAX_PARTY; i++) {
        if (p->data[i].sd && p->data[i].sd != sd) {
            struct map_session_data *tsd = p->data[i].sd;

            // Check if the party member is dead using pc_isdead
            if (pc_isdead(tsd))
                continue;

            // Calculate the current HP percentage of the party member
			int min_hp_party = ((tsd->status.max_hp * min_hp_percent) / 100);;
			int current_hp = status_get_hp(&tsd->bl);
            // Check if the party member's HP is below the min_hp_percent
            if (current_hp < min_hp_party) {
                auto now = std::chrono::steady_clock::now();

                // Prevent spamming heal by checking the last heal time
                auto it = last_heal_times.find(tsd->bl.id);
                if (it == last_heal_times.end() || std::chrono::duration_cast<std::chrono::milliseconds>(now - it->second).count() >= HEAL_COOLDOWN_MS) {
                    // Heal the party member
                    unit_skilluse_id(&sd->bl, tsd->bl.id, skill_id, skill_lv);
                    last_heal_times[tsd->bl.id] = now;
                }
            }
        }
    }
...
//Follow Player
void check_and_follow_ai_target(struct map_session_data* sd) {
	if (!sd || sd->status.party_id <= 0)
		return;

	const char* target_name = pc_readaccountregstr(sd, add_str("#AI_charselect$"));
	...
	for (int i = 0; i < MAX_PARTY; i++) {
		...
		if (strcmp(tsd->status.name, target_name) == 0) {
...
int status_calc_mob_(struct mob_data* md, uint8 opt)
{
	...
	if (opt&SCO_FIRST) { // Set basic level on respawn.
		...
		// --- [เพิ่มตรงนี้: ดึงค่าจาก Config มากดเลเวลมอนสเตอร์ตอนเกิด] ---
...
// ===================================================
	// 🔥 [Custom] เนิร์ฟสเตตัสร่างโคลน! หั่น STR/INT ต้นเหตุของดาเมจหลักพัน!
	// ===================================================
	if (md->npc_event[0] != '\0' && (strstr(md->npc_event, "OnFieldDead") || strstr(md->npc_event, "FPCtrl"))) {
		// หั่นสเตตัสพื้นฐานที่เกมจะเอาไปคูณเป็นดาเมจ
		status->str = 10 + (rnd() % 20);
		status->int_ = 10 + (rnd() % 20);
		status->dex = 10 + (rnd() % 20);
		status->agi = 10 + (rnd() % 20);
		status->vit = 10 + (rnd() % 20);
		status->luk = 10 + (rnd() % 20);
		
		// หั่นพลังโจมตีของอาวุธ/เวทมนตร์
		status->rhw.atk = 10 + (rnd() % 50);
		status->rhw.atk2 = 20 + (rnd() % 50);
		status->rhw.matk = 20 + (rnd() % 50);
	}
	// ===================================================
...
int status_calc_pc_sub(map_session_data* sd, uint8 opt)
{
	...
	// ล้าง log เดิมก่อนคำนวณใหม่
	sd->active_char_bonuses.clear();
	pc_char_pass(sd);
	...
	for (i = 0; i < MAX_INVENTORY; i++){
		...
		if(charm_info->type == IT_CHARM_UPGRADE && charm_info->max_charm_upgrae_stack && charm_upgrade_check_limit(charm, charm_info->nameid))
			charm.push_back(charm_info->nameid);
		else if (charm_info->type == IT_CHARM_UPGRADE && charm_info->max_charm_upgrae_stack && !charm_upgrade_check_limit(charm, charm_info->nameid))
			continue;
...
// ----- MISC CALCULATIONS -----
	...
	if(sc->count) {
		...
		if (sc->getSCE(SC_HUNGRY1)) {
			pc_bonus(sd, SP_ATK_RATE, -50);
			pc_bonus(sd, SP_MATK_RATE, -50);
		}
		if (sc->getSCE(SC_HUNGRY2)) {
			pc_bonus(sd, SP_ATK_RATE, -30);
			pc_bonus(sd, SP_MATK_RATE, -30);
		}
		if (sc->getSCE(SC_HUNGRY3)) {
...
#ifdef RENEWAL
		/* ---------------------------------------------------------
		 * ONLY Swordman + Two-Hand Quicken => +50% ASPD (percentage)
		 * - bonus หน่วยเป็นเปอร์เซ็นต์ (ไม่ใช่ step)
		 * - ใช้เฉพาะสาขา fixed == false (percentage path)
		 * --------------------------------------------------------- */
		{
			TBL_PC* sd = BL_CAST(BL_PC, bl);
			if (sd && sc->getSCE(SC_TWOHANDQUICKEN)) {
				/* จำกัดเฉพาะอาชีพ Swordman เท่านั้น
				 * หมายเหตุ: ถ้าบิลด์ของคุณใช้ enum อื่นสำหรับอาชีพแทน JOB_SWORDMAN
				 * ให้ปรับตรงนี้ให้ตรงของโปรเจกต์คุณ
				 */
				if (sd->status.class_ == JOB_SWORDMAN) {
					bonus += 50; // +50%
				}
			}
		}
...
#ifdef RENEWAL
		/* ---------------------------------------------------------
		 * ONLY Swordman + One-Hand Quicken => +20% ASPD (percentage)
		 * - bonus หน่วยเป็นเปอร์เซ็นต์ (ไม่ใช่ step)
		 * - ใช้เฉพาะสาขา fixed == false (percentage path)
		 * --------------------------------------------------------- */
		{
			TBL_PC* sd = BL_CAST(BL_PC, bl);
			if (sd && sc->getSCE(SC_ONEHAND)) {
				/* จำกัดเฉพาะอาชีพ Swordman เท่านั้น
				 * หมายเหตุ: ถ้าบิลด์ของคุณใช้ enum อื่นสำหรับอาชีพแทน JOB_SWORDMAN
				 * ให้ปรับตรงนี้ให้ตรงของโปรเจกต์คุณ
				 */
...
#ifdef RENEWAL
		/* ---------------------------------------------------------
		 * ONLY Merchant + ADRENALINE => +50% ASPD (percentage)
		 * - bonus หน่วยเป็นเปอร์เซ็นต์ (ไม่ใช่ step)
		 * - ใช้เฉพาะสาขา fixed == false (percentage path)
		 * --------------------------------------------------------- */
		{
			TBL_PC* sd = BL_CAST(BL_PC, bl);
			if (sd && sc->getSCE(SC_ADRENALINE)) {
				/* จำกัดเฉพาะอาชีพ Swordman เท่านั้น
				 * หมายเหตุ: ถ้าบิลด์ของคุณใช้ enum อื่นสำหรับอาชีพแทน JOB_MERCHANT
				 * ให้ปรับตรงนี้ให้ตรงของโปรเจกต์คุณ
				 */
				if (sd->status.class_ == JOB_MERCHANT) {
					bonus += 50; // +50%
				}
			}
		}
...
#ifdef RENEWAL
		/* ---------------------------------------------------------
		 * ONLY Archer + TRUESIGHT => +50% ASPD (percentage)
		 * - bonus หน่วยเป็นเปอร์เซ็นต์ (ไม่ใช่ step)
		 * - ใช้เฉพาะสาขา fixed == false (percentage path)
		 * --------------------------------------------------------- */
		{
			TBL_PC* sd = BL_CAST(BL_PC, bl);
			if (sd && sc->getSCE(SC_TRUESIGHT)) {
				/* จำกัดเฉพาะอาชีพ Archer เท่านั้น
				 * หมายเหตุ: ถ้าบิลด์ของคุณใช้ enum อื่นสำหรับอาชีพแทน JOB_ARCHER
				 * ให้ปรับตรงนี้ให้ตรงของโปรเจกต์คุณ
				 */
				if (sd->status.class_ == JOB_ARCHER) {
					bonus += 50; // +50%
				}
			}
		}
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_AUTOATTACK:
			...
			if(sd){
				t_tick now = gettick();
				// Initialize AA activity timestamps to avoid false "stuck" triggers right after enabling.
				sd->aa.last_move = now;
				sd->aa.last_attack = now;
				sd->aa.last_hit = now;
				sd->aa.last_pickup = now;
				// Initialize teleport timestamps so delay-based teleport conditions don't fire immediately.
				sd->aa.last_teleport = now;
				sd->aa.last_warp_time = now;
				sd->aa.teleport_lock_tick = 0;
				sd->aa.last_stuck_tp_try = 0;
				...
				sd->aa.start_tick = gettick();
				sd->aa.forced_tp_done = false;
				sd->aa.last_forced_tp_try = 0;
				sd->aa.last_emergency_wing = 0;
			}
			break;
...
//			
//					if (item_index >= 0) { 
//						if (pc_delitem(sd, item_index, 1, 0, 0, LOG_TYPE_CONSUME) == 0) {
...
if (pc_isdead(sd)) {
	...
	if (sd->aa.revive_auto == 1) {
		...
		// มีของในช่องของจริงไหม
		if (item_index >= 0 && sd->inventory.u.items_inventory[item_index].amount > 0) {
			...
			if (status_revive(&sd->bl, 100, 100)) {
				...
				// ถ้า revive fail จริง ๆ (กรณีไม่ตายสนิท)
				status_change_end(&sd->bl, SC_AUTOATTACK);
			}
		} else {
			// ไม่มี item
	...
	break;
			...
			bool skip = false;
			bool flywing = false;
			unsigned int cooldown_time = 5000; // Delay Use buff after teleport
			...
			// [1] เปิดเรดาร์สแกนสภาพแวดล้อม (สแกนรอบเดียวได้ค่าครบ 3 อย่าง)
			if (!flywing) {
			...
			if (move_tick >= aa_stuck_ms && attack_tick >= aa_stuck_ms && hit_tick >= aa_stuck_ms) {
				if (sd->status.zeny >= 60) {
					if (sd->aa.last_stuck_tp_try == 0 || DIFF_TICK(last_tick, sd->aa.last_stuck_tp_try) >= 1000) {
						...
						if (aa_teleport(sd)) {
							pc_payzeny(sd, 60, LOG_TYPE_OTHER);
					...
					if(sd->aa.itempick_id){
						...
						if(fitem_bl){
							...
							if(!check_distance_bl(&sd->bl, fitem_bl, 2)) {
								// [แก้ไข 2] กันบอทยืนเอ๋อจ้องไอเทม
							...
							sd->aa.itempick_id = 0; // เคลียร์ค่าถ้าไอเทมดรอปหายไปจากแมพแล้ว
						}
					}
				}
			}
...
//Auto-heal skill
			if(battle_config.autoattack_skillheal){
				// [แก้ไข 1] เอา && hit_tick > 2000 ออก เพื่อให้บอทฮีลตัวเองได้แม้กำลังโดนมอนรุมตี
				if(!sd->aa.autoheal.empty()){
					if(last_tick >= sd->aa.skill_cd){
						for(auto &itAutoheal : sd->aa.autoheal){
							// [แก้ไข 2] แก้สมการ % เลือด ป้องกันบั๊กเซิร์ฟดับจากการหารด้วยศูนย์ (Divide by Zero)
							if(sd->status.max_hp > 0 && ((status->hp * 100 / sd->status.max_hp) <= itAutoheal.min_hp) && pc_checkskill(sd, itAutoheal.skill_id) >= itAutoheal.skill_lv)
							{
								if(last_tick >= itAutoheal.last_use){
									if(unit_skilluse_id(bl, bl->id, itAutoheal.skill_id, itAutoheal.skill_lv)){
										// [แก้ไข] ลบเครื่องหมาย + ที่เบิ้ลกันอยู่ (+ +) ออก
			...
			if(battle_config.autoattack_item_potion){
				if(sd->aa.autopotion.size()){
					for(auto &itAutopotion : sd->aa.autopotion){
						//HP
						// [แก้ไข 2] แก้สมการ % เลือด ป้องกันเซิร์ฟดับ
						if(itAutopotion.min_hp > 0 && sd->status.max_hp > 0 && ((status->hp * 100 / sd->status.max_hp) <= itAutopotion.min_hp)){
						...
						//SP
						// [แก้ไข 2] แก้สมการ % SP ป้องกันเซิร์ฟดับ
						if(itAutopotion.min_sp > 0 && sd->status.max_sp > 0 && ((status->sp * 100 / sd->status.max_sp) <= itAutopotion.min_sp)){
							at_index = pc_search_inventory(sd, itAutopotion.item_id);
			...
			// [แก้ไข 3] ใส่ static const เข้าไป เพื่อไม่ให้เซิร์ฟเวอร์ต้องสร้างตัวแปรนี้ใหม่ทุกๆ วินาที (ลด CPU ของเซิร์ฟเวอร์ได้มหาศาล)
// [แก้ไข 3] ใส่ static const เข้าไป เพื่อไม่ให้เซิร์ฟเวอร์ต้องสร้างตัวแปรนี้ใหม่ทุกๆ วินาที (ลด CPU ของเซิร์ฟเวอร์ได้มหาศาล)
			static const std::set<int> skill_Buff_list = {33, 75, 111, 112, 113, 114, 155};
							
			//Buff SKill
			if (gettick() - sd->aa.last_warp_time >= cooldown_time) {
				if (gettick() - sd->aa.last_buff_time >= cooldown_buff) {
					// Buff skills
						...
						if (!skip && !pc_issit(sd) && sd->aa.autobuffskills.size()) {
							for (auto& itAutobuffskills : sd->aa.autobuffskills) {
								...
								if (itAutobuffskills.is_active
									...
									// MO_CALLSPIRITS  CH_SOULCOLLECT
									if ((itAutobuffskills.skill_id == 261 || itAutobuffskills.skill_id == 401) && sd->spiritball == 5)
										continue;
									// AL_ANGELUS  //PR_KYRIE
									if ((itAutobuffskills.skill_id == 33 || itAutobuffskills.skill_id == 73 || itAutobuffskills.skill_id == 74)
										&& unit_counttargeted(&sd->bl) > 0) {
										continue;
									}
									//AL_ANGELUS   PR_GLORIA		
									if(skill_Buff_list.find(itAutobuffskills.skill_id) != skill_Buff_list.end()) {
										if (gettick() < itAutobuffskills.last_use + 3000) {
											continue;
										}

										itAutobuffskills.last_use = gettick();
									}

									if (!sd->sc.getSCE(skill_get_sc(itAutobuffskills.skill_id))
										...
										sd->aa.last_buff_time = gettick();
										skip = true;
									}

									// --- แก้ไขที่ 1: ลบเงื่อนไข if (sd->aa.stay_mode == 1) ออก บอทจะได้บัพให้ปาร์ตี้ตอนเดินตามได้ ---
										...
										if (p != NULL) {
											for (int i = 0; i < MAX_PARTY; i++) {
												if (p->party.member[i].account_id > 0) {
													struct map_session_data* member_sd = map_id2sd(p->party.member[i].char_id);
													if (member_sd && pc_isdead(member_sd)) {
									...
									if (!party_member_dead && sd->aa.autobuffskills.size()) {
										for (auto& itAutobuffskills_pty : sd->aa.autobuffskills) {
											if (buff_party_members(sd, itAutobuffskills_pty.skill_id, itAutobuffskills_pty.skill_lv)) {
												sd->aa.last_buff_time = gettick();
											}
										}
									}
			...
			// --- แก้ไขที่ 2: เปลี่ยน sd->aa.heal_party เป็น heal_check_skill เพื่อให้ฮีลแรงตามเลเวลสกิลจริงๆ ---
			if(sd->aa.heal_party > 0 && heal_check_skill > 0){	
				heal_party_members(sd, AL_HEAL, heal_check_skill, sd->aa.heal_hp);
			}
			// ---------------------------------------------------------------------------------
...
// Function to check and buff party members
const int BUFF_COOLDOWN_MS = 1000; // Cooldown in milliseconds 

bool buff_party_members(struct map_session_data *sd, int skill_id, int skill_lv) {
    struct party_data *p = party_search(sd->status.party_id);
    if (!p)
        return false;

    int status_change = skill_get_sc(skill_id); // Get the status change related to the skill ID
    static std::unordered_map<int, std::chrono::steady_clock::time_point> last_buff_times;
    bool buffed_someone = false;
    ...
    for (int i = 0; i < MAX_PARTY; i++) {
        if (p->data[i].sd && p->data[i].sd != sd) {
            ...
            if (check_distance_bl(&sd->bl, &tsd->bl, AREA_SIZE)) {
                auto now = std::chrono::steady_clock::now();

                // Check if the buff is not already active on the party member
                if (!tsd->sc.getSCE(status_change)) {
                    auto it = last_buff_times.find(tsd->bl.id);
                    if (it == last_buff_times.end() || std::chrono::duration_cast<std::chrono::milliseconds>(now - it->second).count() >= BUFF_COOLDOWN_MS) {
                        // Buff the party member
                        unit_skilluse_id(&sd->bl, tsd->bl.id, skill_id, skill_lv);
                        last_buff_times[tsd->bl.id] = now;
                        buffed_someone = true; // Flag that we successfully buffed someone
						break; 
                    }
                }
            }
...
int buildin_autopick_sub(struct block_list *bl, va_list ap)
{
	...
	// ===================================================
	// 🔥 [Custom] แก้บั๊กลูปสแกนทับค่าไอเทม! 
	// ถ้าเรดาร์เจอไอเทมที่ตรงเงื่อนไขแล้ว ให้หยุดสแกนชิ้นอื่นมาทับ!
	// ===================================================
	if (*itempick_id != 0) 
		return 1; // ล็อคเป้าแล้ว ข้ามการสแกนขยะชิ้นอื่นบนพื้นไปเลย
...
bool aa_check_item_pickup(map_session_data *sd, struct block_list *bl)
{
	...
	if(bl && bl->type == BL_ITEM && bl->m == sd->bl.m && !pc_cant_act(sd)){
		...
		// ===================================================
		// 🔥 [Custom] อุดรอยรั่ว! เดินอัดกำแพงเวลาเก็บของเฉพาะชิ้น
		// ===================================================
		if(sd->aa.pickup_item_config == 1 && !sd->aa.pickup_item_id.empty()){
			...
			// ถ้าไม่ใช่ของที่ตั้งไว้ให้เก็บ ให้เมินไปเลย!
			if (!is_match) return false; 
		}

		// 🔥 ด่านสุดท้าย: ไม่ว่าจะเป็นเก็บทุกอย่าง หรือเก็บเฉพาะอย่าง ต้องมาเช็คทางเดินตรงนี้เสมอ!
...
bool aa_check_target(map_session_data *sd, unsigned int id)
{
	...
	if (md->status.hp <= 0 || md->special_state.summon)
		return false;

	// ===================================================
	// 🔥 [Custom] บังคับให้เรดาร์ Auto Attack ตาบอด มองไม่เห็นโคลน!
	// ดักหน้าสุดเลย ถ้าเป็นโคลนให้เลิกสนใจทันที!
	// ===================================================
	if (md->npc_event[0] != '\0' && (strstr(md->npc_event, "OnFieldDead") || strstr(md->npc_event, "FPCtrl"))) {
		return false; 
	}
	...
	// ---- ATTACK TARGET CHECK (range + reachable) ----
	if (path_search(NULL, sd->bl.m, sd->bl.x, sd->bl.y, bl->x, bl->y, 1, CELL_CHKNOREACH) &&
		distance_xy(sd->bl.x, sd->bl.y, bl->x, bl->y) < 11) {
		if (!sd->aa.target_id && sd->aa.mobs.id.size()) {
			return util::vector_exists(sd->aa.mobs.id, md->mob_id);
		}
		return true;
	}
	return false;
...
int buildin_autoattack_sub(struct block_list *bl, va_list ap)
{
	...
	// Do not clear an already-selected target when other mobs fail checks
...
// --------------------------------------------------
// AutoAttack helpers: Danger Radar & Mob Aggro Count
// --------------------------------------------------
struct s_aa_radar_data {
	int aggro_count;
	bool found_danger;
	bool hunting_boss;
};

static int aa_danger_radar_sub(struct block_list* bl, va_list ap)
{
	map_session_data* sd = va_arg(ap, map_session_data*);
	struct s_aa_radar_data* radar = va_arg(ap, struct s_aa_radar_data*);

	if (!sd || !radar || !bl || bl->type != BL_MOB)
		return 0;
...
static void aa_scan_environment(map_session_data* sd, int range, int &aggro_count, bool &found_danger, bool &hunting_boss)
{
	...
	struct s_aa_radar_data radar = {0, false, false};
	map_foreachinrange(aa_danger_radar_sub, &sd->bl, range, BL_MOB, sd, &radar);
	
	aggro_count = radar.aggro_count;
	found_danger = radar.found_danger;
	hunting_boss = radar.hunting_boss;
}

// Flywing-only teleport (does NOT use Teleport skill), respects autoattack_teleport and flywing toggle.
static bool aa_flywing_only(map_session_data* sd)
{
	if (!sd)
		return false;
	...
	// Teleport lock: at most one teleport per server tick
	{
		t_tick now = gettick();
		if (sd->aa.teleport_lock_tick == now)
			return true; // already teleported this tick
	}

	if (sd->state.autotrade)
		return false;

	// NOTE: In this codebase, use_flywing == 0 means "allow using flywing".
	...
	for (auto& it : ai_item_flywings) {
		int idx = pc_search_inventory(sd, it.itemid);
		if (idx < 0)
			continue;
		...
		// Use item (or emulate flywing by delete+randomwarp as the original code does)
		if (it.is_delete) {
			std::shared_ptr<item_data> id_data = item_db.find(it.itemid);
			if (id_data != nullptr && !id_data->flag.delay_consume) {
				pc_delitem(sd, idx, 1, 1, 0, LOG_TYPE_CONSUME);
			}
...
bool aa_teleport(map_session_data *sd)
{
	...
	{
		t_tick now = gettick();
		if (sd && sd->aa.teleport_lock_tick == now)
			return true; // already teleported this tick
	}
			...
			for(auto &it : ai_item_flywings ){
				...
				if (i >= 0){
					if(it.leader_only && aa_party_teleport(sd, it.itemid)){
						...
						// --- [แก้ไขตามคำขอ: หักวิงทุกชนิด ยกเว้น 12887 รหัสเดียว] ---
						if (it.itemid == 12887) {
							// ไอเทม 12887: ไม่ต้องทำอะไร (ปกป้องไอเทม)
						} 
						else {
							// ไอเทมอื่นๆ ทุกชนิด: บังคับลบ 1 ชิ้นทันทีที่ใช้งาน
...
uint64 CharBonusDatabase::parseBodyNode(const ryml::NodeRef &node){
	...
	// 👈 สร้างข้อมูลใหม่เสมอ ไม่ดึงของเก่ามาเขียนทับ
	std::shared_ptr<s_char_bonus> char_bonus = std::make_shared<s_char_bonus>();
	char_bonus->jobid = constant;
	char_bonus->job_name = jobname;
	...
	if (this->nodeExists(node, "Icon")) {
		...
		if (!script_get_constant(icon_name.c_str(), &icon_constant)) {
			this->invalidWarning(node["Icon"], "Icon %s is invalid, defaulting to EFST_BLANK.\n", icon_name.c_str());
			icon_constant = EFST_BLANK;
		}

		if (icon_constant < EFST_BLANK || icon_constant >= EFST_MAX) {
			this->invalidWarning(node["Icon"], "Icon %s is out of bounds, defaulting to EFST_BLANK.\n", icon_name.c_str());
			icon_constant = EFST_BLANK;
		}

		char_bonus->icon = static_cast<efst_type>(icon_constant);
	} else {
	...
	// 👈 บันทึกลง Database ด้วย ID รันลำดับ (+1 ไปเรื่อยๆ) จะได้ไม่ทับกัน
	this->entry_id++;
	this->put(this->entry_id, char_bonus);
...
uint64 CharBonusComboDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	for (const auto &combosit : combos_list) {
		...
		if (this->nodeExists(node, "Script")) {
			...
			col_combo->script_text = script; // 👈 เพิ่มบรรทัดนี้ลงไป
...
const std::string NoSkillDatabase::getDefaultLocation(){
	return std::string(db_path) + "/custom/no_skill.yml";
}

uint64 NoSkillDatabase::parseBodyNode( const ryml::NodeRef& node ){
	uint16 group_id;

	if (!this->asUInt16(node, "GroupId",group_id))
		return 0;

	std::shared_ptr<s_no_skill> noskill = this->find(group_id);
	bool exists = noskill != nullptr;

	if (!exists) {
		if (!this->nodesExist(node,{ "GroupId","MapLists","SkillLists"}))
			return 0;

		noskill = std::make_shared<s_no_skill>();
		noskill->group_id = group_id;
	}
	...
	if(this->nodeExists(node, "MapLists" )){
		for(const ryml::NodeRef& MapNode : node["MapLists"]){
			...
			if(util::vector_exists(noskill->maps, map_index))
				continue;

			noskill->maps.push_back(map_index);
		}
	}

	if (this->nodeExists(node, "SkillLists")) {
		for(const ryml::NodeRef& skillNode : node["SkillLists"]){
			...
			util::tolower(skill_name);

			int skill_id = skill_name2id(skill_name.c_str());

			if (skill_id == 0) {
				this->invalidWarning(skillNode["Skill"], "skill %s does not exist, skipping.\n", skill_name.c_str());
				continue;
			}

			if(util::vector_exists(noskill->skills,skill_id))
				continue;

			noskill->skills.push_back(skill_id);
		}
	}

	if (!exists)
		this->put(noskill->group_id, noskill);

	return 1;
}

NoSkillDatabase no_skill_db;
