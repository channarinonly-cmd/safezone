...
BUILDIN_FUNC(spawn_fakeplayer)
{
	...
	sd->bl.type = BL_PC; 
	sd->fd = 0; 
	sd->bl.id = npc_get_new_npc_id(); 
	sd->status.account_id = sd->bl.id; 
	sd->status.char_id = sd->bl.id; 
	sd->mapindex = map_id2index(m);
	...
	// 🌟 [FIX CRASH] ป้องกันเซิร์ฟเวอร์แครชจาก Timer ผิดพลาด!
	...
	sd->battle_status.hp = 10000;
	sd->battle_status.max_hp = 10000;
	sd->battle_status.sp = 1000;
	sd->battle_status.max_sp = 1000;
	sd->battle_status.speed = 150; 

	status_change_init(&sd->bl);
	unit_dataset(&sd->bl); 
	
	// 🌟 บังคับหันหน้าทิศใต้ (4 = ใต้)
	sd->ud.dir = 4; 
	...
	char* data;
	Sql_GetData(mmysql_handle, 0, &data, NULL); safestrncpy(sd->status.name, fake_name ? fake_name : data, NAME_LENGTH);
	Sql_GetData(mmysql_handle, 1, &data, NULL); sd->status.class_ = atoi(data);
	Sql_GetData(mmysql_handle, 2, &data, NULL); sd->status.base_level = atoi(data);
	Sql_GetData(mmysql_handle, 3, &data, NULL); sd->status.hair = atoi(data);
	Sql_GetData(mmysql_handle, 4, &data, NULL); sd->status.hair_color = atoi(data);
	Sql_GetData(mmysql_handle, 5, &data, NULL); sd->status.clothes_color = atoi(data);
	Sql_GetData(mmysql_handle, 6, &data, NULL); sd->status.weapon = atoi(data);
	Sql_GetData(mmysql_handle, 7, &data, NULL); sd->status.shield = atoi(data);
	Sql_GetData(mmysql_handle, 8, &data, NULL); sd->status.head_top = atoi(data);
	Sql_GetData(mmysql_handle, 9, &data, NULL); sd->status.head_mid = atoi(data);
	Sql_GetData(mmysql_handle, 10, &data, NULL); sd->status.head_bottom = atoi(data);
	Sql_GetData(mmysql_handle, 11, &data, NULL); sd->status.sex = (data[0] == 'F' ? 0 : 1);
	...
	sd->vd.weapon = sd->status.weapon;
	sd->vd.shield = sd->status.shield;
	...
	// ระบบดึงข้อมูลไอเทมสวมใส่ (เอาไว้ให้ผู้เล่นส่องของได้)
	if (SQL_SUCCESS == Sql_Query(mmysql_handle, "SELECT nameid, equip, card0, card1, card2, card3, refine FROM `inventory` WHERE char_id = %d AND equip > 0", char_id)) {
		int idx = 0;
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle) && idx < MAX_INVENTORY) {
			Sql_GetData(mmysql_handle, 0, &data, NULL); sd->inventory.u.items_inventory[idx].nameid = atoi(data);
			Sql_GetData(mmysql_handle, 1, &data, NULL); sd->inventory.u.items_inventory[idx].equip = atoi(data);
			Sql_GetData(mmysql_handle, 2, &data, NULL); sd->inventory.u.items_inventory[idx].card[0] = atoi(data);
			Sql_GetData(mmysql_handle, 3, &data, NULL); sd->inventory.u.items_inventory[idx].card[1] = atoi(data);
			Sql_GetData(mmysql_handle, 4, &data, NULL); sd->inventory.u.items_inventory[idx].card[2] = atoi(data);
			Sql_GetData(mmysql_handle, 5, &data, NULL); sd->inventory.u.items_inventory[idx].card[3] = atoi(data);
			Sql_GetData(mmysql_handle, 6, &data, NULL); sd->inventory.u.items_inventory[idx].refine = atoi(data);
			
			sd->inventory_data[idx] = itemdb_search(sd->inventory.u.items_inventory[idx].nameid);
			if (sd->inventory_data[idx] != NULL) {
				sd->inventory.u.items_inventory[idx].identify = 1; 
				sd->inventory.u.items_inventory[idx].amount = 1;
				idx++;
			} else {
				sd->inventory.u.items_inventory[idx].nameid = 0;
				sd->inventory.u.items_inventory[idx].equip = 0;
			}
		}
		Sql_FreeResult(mmysql_handle);
		pc_setequipindex(sd);
	...
	sd->bl.x = x;
	sd->bl.y = y;
	sd->ud.dir = rand() % 8; // 🌟 ย้ายมาวางตรงนี้! (สุ่มตั้งแต่ตอนลงพิกัด)
	...
	// 🌟 แก้เป็น AREA เพื่อให้ทุกคนในแมพเห็นว่าบอทตัวนี้หันหน้าทางทิศใต้นะ!
	clif_changed_dir(&sd->bl, AREA);

	map_getmapdata(m)->users++; 
	script_pushint(st, sd->bl.id); 
	return SCRIPT_CMD_SUCCESS; 
}
// ==========================================================
// 2. ระบบล้างบอท (อัปเดตใหม่ เคลียร์ห้องแชทก่อนลบกันบัคห้องผี)
// ==========================================================
BUILDIN_FUNC(despawn_fakeplayer)
{
	int gid = script_getnum(st, 2);
	struct map_session_data* sd = map_id2sd(gid);
	
	if (sd && sd->fd == 0) { 
		// 🌟 ใช้ chatID และ chat_leavechat ตามที่เซิร์ฟเวอร์ลูกพี่ต้องการ!
		if (sd->chatID > 0) {
...
// ==========================================================
// 3. ระบบสั่งบอทนั่ง / ยืน (แก้ไข Error Compile ผ่าน 100%)
// ==========================================================
BUILDIN_FUNC(fakeplayer_action)
{
	int gid = script_getnum(st, 2);
	int action = script_getnum(st, 3); 
	struct map_session_data* sd = map_id2sd(gid);
	
	if (sd && sd->fd == 0) {
		if (action == 1) {
			// ส่งแพ็กเก็ตภาพบอกผู้เล่นในจอให้เห็นว่า "บอทนั่งแล้วนะ" (ข้ามตัวแปร state ไปเลย)
			clif_sitting(&sd->bl);
		} else {
			// ส่งแพ็กเก็ตภาพบอกผู้เล่นในจอให้เห็นว่า "บอทยืนแล้วนะ"
			clif_standing(&sd->bl);
		}
	}
	return SCRIPT_CMD_SUCCESS;
...
// ==========================================================
// 4. ระบบบังคับบอทเดิน (สั่งตรงจาก C++ ทะลวงข้อจำกัด PC)
// ==========================================================
BUILDIN_FUNC(fakeplayer_walk)
{
	int gid = script_getnum(st, 2);
	...
	if (sd && sd->fd == 0) { // เช็คให้ชัวร์ว่าเป็นบอทแน่นอน
		// 🌟 เปลี่ยนชื่อฟังก์ชันเป็น unit_walktoxy ตามที่เซิร์ฟเวอร์ต้องการเป๊ะๆ!
...
// ==========================================================
// 5. ระบบบังคับบอทตั้งห้องแชท (Waitroom)
// ==========================================================
BUILDIN_FUNC(fakeplayer_chat)
{
	int gid = script_getnum(st, 2);
	...
	if (sd && sd->fd == 0) { 
		// 🌟 ใช้ chatID และ chat_createpcchat ตามสไตล์คลาสสิก
		if (!sd->chatID) { 
			chat_createpcchat(sd, title, "", 2, 1);
		}
		script_pushint(st, 1);
	} else {
		script_pushint(st, 0);
...
BUILDIN_FUNC(criticalmagic)
{
    struct map_session_data *sd = NULL;
    if (!script_rid2sd_(st, &sd, __func__))
        return SCRIPT_CMD_FAILURE;

    int value = script_getnum(st, 2);
    sd->magic_critical = value; 
...
/*==========================================
 * delitemgrade <item_id/name>,<amount>,<grade>{,<char_id>};
 * ลบไอเทมตาม ID และ Grade โดยไม่สนการตีบวกหรือการ์ด
 *------------------------------------------*/
BUILDIN_FUNC(delitemgrade)
{
	TBL_PC *sd;
	t_itemid nameid;

	// 1. ดึงตัวละคร (รองรับการระบุ char_id เป็นพารามิเตอร์ที่ 5)
	if (!script_charid2sd(5, sd)) {
		script_pushint(st, 0);
		return SCRIPT_CMD_FAILURE;
	}

	// 2. เช็คว่าใส่ ID หรือ ชื่อไอเทมมา
	if (script_isstring(st, 2)) {
		const char* item_name = script_getstr(st, 2);
		std::shared_ptr<item_data> id = item_db.searchname(item_name);
		if (id == nullptr) {
			ShowError("buildin_delitemgrade: unknown item \"%s\".\n", item_name);
			script_pushint(st, 0);
			return SCRIPT_CMD_FAILURE;
		}
		nameid = id->nameid;
		...
		nameid = script_getnum(st, 2);
		if (!item_db.exists(nameid)) {
			ShowError("buildin_delitemgrade: unknown item \"%u\".\n", nameid);
			script_pushint(st, 0);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	// 3. วนลูปหาในกระเป๋า (Inventory)
	for (int i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].nameid == nameid && sd->inventory.u.items_inventory[i].amount > 0) {
			// เช็คว่าเกรดตรงกับที่ต้องการไหม
			if (sd->inventory.u.items_inventory[i].enchantgrade == grade) {
				int delete_amount = std::min((int)(amount - deleted), (int)sd->inventory.u.items_inventory[i].amount);
				
				pc_delitem(sd, i, delete_amount, 0, 0, LOG_TYPE_SCRIPT);
				deleted += delete_amount;
				
				// ถ้าลบครบจำนวนที่ต้องการแล้วให้ออกลูป
				if (deleted >= amount)
					break;
			}
		}
	...
	// คืนค่ากลับไปให้สคริปต์ (1 = ลบครบ, 0 = ลบไม่ครบ/ไม่มีของ)
	script_pushint(st, deleted >= amount ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
/**
 * บังคับคำนวณสเตตัส (รวมถึง EXP, Drop Rate บัฟต่างๆ) ใหม่
 * recalcstat({<char_id>});
 **/
BUILDIN_FUNC(recalcstat)
{
	TBL_PC *sd;
	
	// ตรวจสอบว่าดึงตัวละครที่สคริปต์กำลังรันอยู่ได้หรือไม่ (รองรับการใส่ char_id ด้วย)
	if (!script_charid2sd(2,sd))
		return SCRIPT_CMD_FAILURE;
		
	// สั่งฟังก์ชัน C++ ให้คำนวณ Status ของตัวละครนี้ใหม่ทั้งหมด!
...
BUILDIN_FUNC(setattackpk)
{
    ...
    if (mapindex_id2name(sd->mapindex) && 
        util::vector_exists(mapname_list, std::string(mapindex_id2name(sd->mapindex)))) {
	...
	int time = battle_config.pk_pass_attack_time * 1000;

    clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
	pk_pass_protect_effect_off(&sd->bl);
	pk_pass_attack_effect(&sd->bl);
	add_timer(gettick() + time, pc_attack_pkpass, sd->bl.id, 0);

	push_val(st->stack, C_INT, 1);
    return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(setprotectpk)
{
    TBL_PC *sd;
    if (!script_charid2sd(2,sd)){
        return SCRIPT_CMD_FAILURE;
	...
	int time = battle_config.pk_pass_protect_time * 1000;

    clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
	pk_pass_attack_effect_off(&sd->bl);
	pk_pass_protect_effect(&sd->bl);
	add_timer(gettick() + time, pc_protect_pkpass, sd->bl.id, 0);
...
BUILDIN_FUNC(get_unique_id)
{
	...
	script_pushint(st, sd->shield_unique_id);
#else
	script_pushint(st, 0);
#endif
	return SCRIPT_CMD_SUCCESS;
...
BUILDIN_FUNC(getshieldhwid)
{
	TBL_PC* sd = nullptr;
	if (!script_rid2sd(sd)) {
		script_pushstr(st, (char*)"");
		return SCRIPT_CMD_SUCCESS;
	}
#if LIONSHIELD_ENABLED
	script_pushstr(st, sd->shield_hwid);
#else
	script_pushstr(st, (char*)"");
#endif
	return SCRIPT_CMD_SUCCESS;
...
BUILDIN_FUNC(setcharnamecolor)
{
	...
	if (color == nullptr || color[0] == '\0') {
		pc_setregistry_str(sd, add_str("CharNameColor$"), "");
		sd->colored_name[0] = '\0';
	} else {
		char color_clean[7];
		safestrncpy(color_clean, color, sizeof(color_clean));
		pc_setregistry_str(sd, add_str("CharNameColor$"), color_clean);
		safesnprintf(sd->colored_name, sizeof(sd->colored_name), "^%s%s", color_clean, sd->status.name);
	}
	// Do not push an immediate custom name-refresh packet here. This client
	// does not define packet 0x2b0 and can freeze while the NPC dialog is open.
	// The color is saved and will be applied by normal name packets after relog
	// or when the character leaves/re-enters nearby clients' view.
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(openurl)
{
	TBL_PC* sd = nullptr;
	if (!script_rid2sd(sd)) {
		return SCRIPT_CMD_SUCCESS;
...
/*=============================================================
 * mobdynamicdrop <mob_id or name>,<item_id>,<rate>,<duration_sec>,{<steal protected>,{<random option group id>}};
 *
 * - Runtime dynamic drops (in-memory) that behave like normal mob drops
 *   (Bubble Gum / drop bonuses apply via mob_getdroprate).
 * - duration_sec: 0 = no expiry (until removed or map-server restart)
 * - rate: 1..10000 (10000 = 100%)
 *
 * Example:
 *   mobdynamicdrop 1002, 512, 10000, 600; // Poring drop Apple 100% for 10 minutes
 *=============================================================*/
BUILDIN_FUNC(mobdynamicdrop)
{
	uint16 mob_id;
	t_itemid item_id;
	int rate;
	int duration_sec;
	bool steal_protected = false;
	uint16 group = 0;
	std::shared_ptr<s_random_opt_group> randomopt_group = nullptr;

	// mob id/name
	if( script_isstring(st, 2) ) {
		mob_id = mobdb_searchname(script_getstr(st, 2));
		if( mob_id == 0 ) {
			ShowError("mobdynamicdrop: bad mob name given %s\n", script_getstr(st, 2));
			return SCRIPT_CMD_FAILURE;
		}
	} else {
		mob_id = script_getnum(st, 2);
		if( mobdb_checkid(mob_id) == 0 ) {
			ShowError("mobdynamicdrop: bad mob id given %d\n", mob_id);
			return SCRIPT_CMD_FAILURE;
		}
	}

	item_id = script_getnum(st, 3);
	if( item_db.exists(item_id) == false ) {
		ShowError("mobdynamicdrop: Nonexistant item %u requested.\n", item_id);
		return SCRIPT_CMD_FAILURE;
	}

	rate = script_getnum(st, 4);
	if( rate < 1 || rate > 10000 ) {
		ShowError("mobdynamicdrop: Invalid rate %d (min 1, max 10000).\n", rate);
		return SCRIPT_CMD_FAILURE;
	}

	duration_sec = script_getnum(st, 5);
	if( duration_sec < 0 ) duration_sec = 0;

	if( script_hasdata(st, 6) )
		steal_protected = script_getnum(st, 6) != 0;

	if( script_hasdata(st, 7) ) {
		group = script_getnum(st, 7);
		if( group > 0 ) {
			randomopt_group = random_option_group.find(group);
			if( randomopt_group == nullptr ) {
				ShowError("mobdynamicdrop: Unknown random option group %d.\n", group);
				return SCRIPT_CMD_FAILURE;
			}
			if( !itemdb_isequip(item_id) ) {
				ShowError("mobdynamicdrop: Random option group can't be used with this item type (item Id: %d).\n", item_id);
				return SCRIPT_CMD_FAILURE;
			}
		}
	}

	t_tick expire_tick = 0;
	if( duration_sec > 0 )
		expire_tick = gettick() + (t_tick)duration_sec * 1000;

	if( mob_dynamic_drop_add(mob_id, item_id, (uint16)rate, expire_tick, steal_protected, group) == false )
		return SCRIPT_CMD_FAILURE;
...
/*=============================================================
 * delmobdynamicdrop <mob_id or name>,<item_id>;
 *=============================================================*/
BUILDIN_FUNC(delmobdynamicdrop)
{
	uint16 mob_id;
	t_itemid item_id;
	...
	if( script_isstring(st, 2) ) {
		mob_id = mobdb_searchname(script_getstr(st, 2));
		if( mob_id == 0 ) {
			ShowError("delmobdynamicdrop: bad mob name given %s\n", script_getstr(st, 2));
			return SCRIPT_CMD_FAILURE;
		}
	} else {
		mob_id = script_getnum(st, 2);
		if( mobdb_checkid(mob_id) == 0 ) {
			ShowError("delmobdynamicdrop: bad mob id given %d\n", mob_id);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	item_id = script_getnum(st, 3);
	if( item_db.exists(item_id) == false ) {
		ShowError("delmobdynamicdrop: Nonexistant item %u requested.\n", item_id);
		return SCRIPT_CMD_FAILURE;
	}

	script_pushint(st, mob_dynamic_drop_remove(mob_id, item_id) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
// Unit was already forced to walk.
	if (ud != nullptr && ud->state.force_walk) {
		...
		// ===================================================
		// 🔥 [Custom] ซ่อน Warning unitwalk เฉพาะของบอทประชากรแฝง (FPCtrl)
		// ===================================================
		bool is_fake_bot = false;
		if (bl != nullptr && bl->type == BL_MOB) {
			struct mob_data *md = (struct mob_data *)bl;
			// เช็คว่าใช่บอทที่มาจาก NPC แผงควบคุมของเราไหม
			if (md && md->npc_event[0] != '\0' && strstr(md->npc_event, "FPCtrl")) {
				is_fake_bot = true;
			}
		}
		
		// ถ้าไม่ใช่บอทของเรา ให้แจ้งเตือน Error และส่งค่าความล้มเหลวตามปกติ
		if (!is_fake_bot) {
			ShowWarning("buildin_%s: Unit has already been forced to walk and not reached it's destination yet.\n", cmd);
			return SCRIPT_CMD_FAILURE; 
		}
		
		// 🔥 ถ้าเป็นบอทสมหวัง ให้หลอกเซิร์ฟเวอร์ว่า "ทำงานสำเร็จแล้ว" (SUCCESS) 
		// เซิร์ฟเวอร์จะได้ไม่พ่นคำว่า "returned failure" ออกมาในคอนโซลอีก!
		return SCRIPT_CMD_SUCCESS;
		// ===================================================
	}
 ...
 *
 * c_sc_start  <effect_id>,<duration>;
 */
BUILDIN_FUNC(c_sc_start)
{
	map_session_data *sd;
	enum sc_type type;
	int tick;
	...
	status_change_start(&sd->bl, &sd->bl, type, 10000, 1, 0, 0, 0, tick, SCSTART_NOAVOID);
	status_calc_pc(sd,SCO_FORCE);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(opencollection) {
	...
	script_pushint(st, storage_collectionStorage_load(sd, battle_config.collection_storage_id, STOR_MODE_PUT));
	return SCRIPT_CMD_SUCCESS;
}
...
// --- Custom Charm Refine Command (Fixed UI Update) ---
BUILDIN_FUNC(charm_setrefine)
{
	TBL_PC *sd;
	if (script_rid2sd(sd)) {
		t_itemid nameid = script_getnum(st, 2);
		int target_refine = script_getnum(st, 3);

		for (int i = 0; i < MAX_INVENTORY; i++) {
			if (sd->inventory.u.items_inventory[i].nameid == nameid) {
				
				// 1. ส่งคำสั่งบอกหน้าจอผู้เล่น (Client) ว่า "ซ่อน/ลบ" ของชิ้นนี้ออกไปชั่วคราว
				clif_delitem(sd, i, sd->inventory.u.items_inventory[i].amount, 0);

				// 2. ปรับค่าตีบวกบนฝั่งเซิร์ฟเวอร์
				sd->inventory.u.items_inventory[i].refine = target_refine;
				
				// 3. ส่งคำสั่งให้หน้าจอผู้เล่น "แสดง" ของชิ้นเดิม (ที่ค่า + อัปเดตแล้ว) กลับมา
				clif_additem(sd, i, sd->inventory.u.items_inventory[i].amount, 0);

				// 4. รีเฟรชสเตตัสตัวละครให้โบนัสทำงาน
				status_calc_pc(sd, SCO_NONE);
...
BUILDIN_FUNC(autoattackstrinfo)
{
	...
	switch(id){
		...
		//Stop walk
		case GET_INFO_STOP_WALK:
			if(!sd->aa.stay_mode)
				buf += msg_txt(NULL,1599);
			else
				buf += msg_txt(NULL,1600);

			script_pushstrcopy(st, buf.c_str());
			break;

		//auto revive
		case GET_INFO_REVIVE:
			if(!sd->aa.revive_auto)
				buf += msg_txt(NULL,1598);
			else
				buf += msg_txt(NULL,1597);

			script_pushstrcopy(st, buf.c_str());
			break;	

		//auto party
		case GET_INFO_PARTY_AUTO:
			if(!sd->aa.party_auto)
				buf += msg_txt(NULL,1596);
			else
				buf += msg_txt(NULL,1595);

			script_pushstrcopy(st, buf.c_str());
			break;				
			
		//heal party
		case GET_INFO_HEAL_PARTY:
			if (sd->aa.heal_party > 0) {
				char buffer[128];
				safesnprintf(buffer, sizeof(buffer), msg_txt(NULL, 1594), sd->aa.heal_party, sd->aa.heal_hp);
		...
		case GET_INFO_TELEPORT: // Teleport
			...
			// --- Swarm teleport (NEW) ---
			if(!sd->aa.teleport.swarm_enable)
				buf += msg_txt(NULL, 1642);
			else
				buf += msg_txt(NULL, 1643);

			if(sd->aa.teleport.swarm_count > 0){
				safesnprintf(buffer, sizeof(buffer), msg_txt(NULL, 1644), sd->aa.teleport.swarm_count);
				buf += buffer;
			}
...
BUILDIN_FUNC(autoattackintinfo)
{
	...
	switch(id){
		...
		case GET_INFO_STOP_WALK: // Stop walk
			script_pushint(st, sd->aa.stay_mode);
			break;
			
		case GET_INFO_REVIVE: // auto revive
			script_pushint(st, sd->aa.revive_auto);
			break;
			
		case GET_INFO_PARTY_AUTO: // auto party
			script_pushint(st, sd->aa.party_auto);
			break;
		...
		case GET_INFO_TELEPORT: // Teleport
			switch(index){
				...
				case 4:
					...
					break;
				case 5:
					num = sd->aa.teleport.swarm_enable;
					break;
				case 6:
					num = sd->aa.teleport.swarm_count;
					break;
		...
		case GET_INFO_SKILL_RATE:
			...
			case 1001: // skill swarm gate (min mobs targeting to allow attack skills, 0=off)
				script_pushint(st, sd->aa.skill_swarm_min);
				break;
	...
	if(result.size() > 0){
		...
		switch(id){
				...
				if(result.size() == 3){ // id = 7 - teleport (index,value)
					...
					switch(index){
						...
						case 4:
							...
							break;
						case 5:
							sd->aa.teleport.swarm_enable = std::stoi(result.at(2)) != 0;
							break;
						case 6:
							sd->aa.teleport.swarm_count = cap_value(std::stoi(result.at(2)), 0, 50);
							break;
					}
				}
				break;
			...
			case GET_INFO_SKILL_RATE:
				...
				case 1001: // skill swarm gate (min mobs targeting to allow attack skills, 0=off)
					if(result.size() == 2){ // id = 1001 - skill swarm gate (min_mobs)
						int v = std::stoi(result.at(1));
						if(v < 0) v = 0;
						if(v > 50) v = 50;
						sd->aa.skill_swarm_min = v;
					}
					break;
...
case GET_INFO_FLEE_MOB:
			...
			case GET_INFO_FOCUS_MOB:
				if(result.size() == 2){  // id 12
					sd->aa.focus_mob = std::stoi(result.at(1));
				}
				break;

			case GET_INFO_STOP_WALK:
				if(result.size() >= 3 && std::stoi(result.at(1)) == 1) { // id = 13 - follow party member by name
					pc_setaccountregstr(sd, add_str("#AI_charselect$"), result.at(2).c_str());
					sd->aa.stay_mode = 1;
					if (sd->status.party_id > 0) {
						...
						if (p) {
							for (int i = 0; i < MAX_PARTY; i++) {
								...
								if (!tsd || tsd == sd)
									continue;
								if (strcmp(tsd->status.name, result.at(2).c_str()) == 0 && tsd->bl.m == sd->bl.m) {
									pc_follow(sd, tsd->bl.id);
									break;
								}
							}
						}
					}
				} else if(result.size() == 2) // id = 13 - stop walk (status)
					sd->aa.stay_mode = std::stoi(result.at(1));
				if(result.size() == 2 && std::stoi(result.at(1)) == 0)
					pc_setaccountregstr(sd, add_str("#AI_charselect$"), "");
				break;	
				
			case GET_INFO_REVIVE:
				if(result.size() == 2) // id = 14 - Auto Revive (status)
					sd->aa.revive_auto = std::stoi(result.at(1));
				break;	
				
			case GET_INFO_PARTY_AUTO:
				if(result.size() == 2) // id = 15 - Auto Party (status)
					sd->aa.party_auto = std::stoi(result.at(1));
				break;	
				
			case GET_INFO_HEAL_PARTY:
				if(result.size() == 4){ // id = 16 - Heal Party (index, heal_party, heal_hp)
					int index = std::stoi(result.at(1)); 
					
					switch(index){
						case 0:
							sd->aa.heal_party = std::stoi(result.at(2)); 
							sd->aa.heal_hp = std::stoi(result.at(3)); 
							break;
					}
				}
			break;				
...
// ดึงฟังก์ชันมาจาก mob.cpp
extern std::string get_bounty_map_string();

/*
 * คำสั่งสคริปต์ใหม่: getbountymaps()
 * ใช้ดึงข้อความรายชื่อแมพโบนัส Zeny ปัจจุบันไปแสดงใน NPC
 */
BUILDIN_FUNC(getbountymaps)
{
	std::string result = get_bounty_map_string();
	script_pushstrcopy(st, result.c_str());
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(check_stamina)
{
	TBL_PC* sd;
...
BUILDIN_FUNC(setcityfaction) {
	map_session_data *sd = NULL;
	if (script_rid2sd(sd)) {
		sd->city_faction = script_getnum(st, 2);
	}
	return 0;
}

BUILDIN_FUNC(setmayor) {
	map_session_data *sd = NULL;
	if (script_rid2sd(sd)) {
		sd->is_mayor = script_getnum(st, 2);
	}
	return 0; // เปลี่ยนจาก true เป็น 0
}
// ==========================================
// [Custom Mod] คำสั่งสคริปต์ setitemstealth
// ==========================================
int buildin_setitemstealth(struct script_state *st)
{
	uint32 item_id = script_getnum(st, 2);
	int rate_percent = script_getnum(st, 3);

	if (rate_percent < 0) rate_percent = 0; // ป้องกันแอดมินใส่ค่าติดลบ

	if (rate_percent == 100) {
		stealth_drop_rates.erase(item_id); // 100% คือปกติ ให้ลบออกจากระบบ
	} else {
		stealth_drop_rates[item_id] = rate_percent; // บันทึกเรทใหม่
	}
	
	return 0; // เปลี่ยนจาก return true เป็น return 0 เพื่อให้ตรงกับชนิด int
}
/*==========================================
 * [Fake Player] คำสั่งวิงหายตัวแบบมีเอฟเฟกต์ (ทีละตัว)
 *==========================================*/
BUILDIN_FUNC(unitvanish) {
	int gid = script_getnum(st, 2);
	struct block_list *bl = map_id2bl(gid);
	if (bl && bl->type == BL_MOB) {
		clif_specialeffect(bl, 4, AREA); // แสดงเอฟเฟกต์แสงวาร์ป
		clif_clearunit_area(bl, CLR_TELEPORT); 
		unit_free(bl, CLR_OUTSIGHT); // ลบออบเจกต์ทิ้งแบบไม่มีศพ
	}
	return SCRIPT_CMD_SUCCESS;
}

/*==========================================
 * [Fake Player] คำสั่งวิงหายตัวแบบมีเอฟเฟกต์ (ยกแมพ)
 *==========================================*/
static int buildin_mobvanish_sub(struct block_list *bl, va_list ap) {
	struct mob_data *md = (struct mob_data *)bl;
	const char *event = va_arg(ap, const char *);
	if (md->npc_event[0] && strcmp(md->npc_event, event) == 0) {
		clif_specialeffect(bl, 4, AREA);
		clif_clearunit_area(bl, CLR_TELEPORT); 
		unit_free(bl, CLR_OUTSIGHT); 
	}
	return 0;
}
BUILDIN_FUNC(mobvanish) {
	int16 m = map_mapname2mapid(script_getstr(st, 2));
	if (m >= 0) map_foreachinmap(buildin_mobvanish_sub, m, BL_MOB, script_getstr(st, 3));
	return SCRIPT_CMD_SUCCESS;
}

/*==========================================
 * [Fake Player] คำสั่งเสกป้ายตั้งร้านค้า (Packet Injection)
 *==========================================*/
static int buildin_mobvending_sub(struct block_list *bl, va_list ap) {
	struct mob_data *md = (struct mob_data *)bl;
	const char *event = va_arg(ap, const char *);
	const char *title = va_arg(ap, const char *);
	
	if (md->npc_event[0] && strcmp(md->npc_event, event) == 0) {
		// เช็คคลาส 5 (พ่อค้าคลาส 1)
		if (md->vd->class_ == 5 || md->vd->class_ == 4006) {
			unsigned char buf[256];
			unsigned short packet_len = (int)strlen(title) + 9;
			buf[0] = 0x13; buf[1] = 0x01; // รหัสแพ็กเก็ต ZC_STORE_ENTRY
			SETVALUE(buf, 2, packet_len); 
			SETVALUE(buf, 4, bl->id);     
			safestrncpy((char*)buf + 8, title, 240);
			clif_send(buf, packet_len, bl, AREA);
		}
	}
	return 0;
}
BUILDIN_FUNC(mobvending) {
	int16 m = map_mapname2mapid(script_getstr(st, 2));
	if (m >= 0) map_foreachinmap(buildin_mobvending_sub, m, BL_MOB, script_getstr(st, 3), script_getstr(st, 4));
	return SCRIPT_CMD_SUCCESS;
}
...
// (^~_~^) Gepard Shield End
	...
	BUILDIN_DEF(unitvanish, "i"),   // [System 1] วิงหายทีละตัว
	BUILDIN_DEF(mobvanish, "ss"),   // [System 2, 3] วิงหายยกแมพ
	BUILDIN_DEF(mobvending, "sss"), // <--- ตัวนี้สำคัญมาก
	...
	BUILDIN_DEF(recalcstat,"?"), // <- เพิ่มบรรทัดนี้ลงไป
	BUILDIN_DEF(setattackpk,"?"),
	BUILDIN_DEF(setprotectpk,"?"),
	...
	BUILDIN_DEF(mobdynamicdrop,"viii??"), // runtime mob drops (Gum/bonuses apply)
	BUILDIN_DEF(delmobdynamicdrop,"vi"),
	BUILDIN_DEF(axtoi,"s"),
	BUILDIN_DEF(query_sql,"s*"),
	BUILDIN_DEF(query_logsql,"s*"),
	BUILDIN_DEF(escape_sql,"v"),
	BUILDIN_DEF(get_unique_id,"?"),
	BUILDIN_DEF(getshieldhwid,"?"),
	BUILDIN_DEF(setcharnamecolor,"s"),
	BUILDIN_DEF(openurl,"s"),
	BUILDIN_DEF(atoi,"s"),
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(charm_setrefine, "ii"),
	...
	BUILDIN_DEF(check_stamina,""),
	BUILDIN_DEF(setcityfaction, "i"),
	BUILDIN_DEF(setmayor, "i"),
	BUILDIN_DEF(setitemstealth, "ii"),
	BUILDIN_DEF(delitemgrade, "vii?"),
