...
{
	if (zeny >= battle_config.vending_tax_min) {
		int total_tax_rate = (battle_config.vending_tax / 100);
		int is_town = 0;

		if (Sql_Query(mmysql_handle, "SELECT 1 FROM `mayor_towns` WHERE `town_map` = '%s' AND `is_active` = 1", map_mapid2mapname(sd->bl.m)) == SQL_SUCCESS) {
			if (Sql_NumRows(mmysql_handle) > 0) is_town = 1;
			Sql_FreeResult(mmysql_handle);
		}

		if (sd->vending_tax_locked > total_tax_rate) {
			total_tax_rate = sd->vending_tax_locked;
		}

		if (total_tax_rate > 0) {
            double tax = zeny * ((double)total_tax_rate / 100.0);
            zeny -= tax;

            // ตรวจสอบวงเล็บตรง if นี้ให้ดี
...
void vending_purchasereq(map_session_data* sd, int aid, int uid, const uint8* data, int count)
{
	...
	// --- [เพิ่มใหม่] คำนวณและแจ้งเตือนระบบภาษีขณะขายได้ ---
	double original_z = z;
	int locked_rate = vsd->vending_tax_locked; // ดึงภาษีที่ล็อคไว้
	double tax_val = 0;
	if (original_z >= battle_config.vending_tax_min && locked_rate > 0) {
		tax_val = original_z * (locked_rate / 100.0);
	}
	
	z = vending_calc_tax(vsd, original_z);
	pc_getzeny(vsd, (int)z, LOG_TYPE_VENDING, sd->status.char_id);

	char output[256];
	// แจ้งคนขาย (ได้รับเงินสุทธิ)
	if(tax_val > 0) {
		sprintf(output, msg_txt(vsd, 1592), (int)z, locked_rate, (int)tax_val);
		clif_displaymessage(vsd->fd, output);
		
		// แจ้งคนซื้อ (จ่ายยอดรวม)
		sprintf(output, msg_txt(sd, 1591), (int)original_z, locked_rate, (int)tax_val);
	...
	for( i = 0; i < count; i++ ) {
		...
		short idx    = *(uint16*)(data + 4*i + 2);
		idx -= 2;
		double item_z = ((double)vsd->vending[vend_list[i]].value * (double)amount);
		...
		// คำนวณราคา Report หลังหักภาษีสำหรับไอเทมชิ้นนี้
		double item_z_after_tax = item_z;
		if (item_z >= battle_config.vending_tax_min && locked_rate > 0) {
			item_z_after_tax = item_z - (item_z * (locked_rate / 100.0));
		}

		clif_vendingreport(vsd, idx, amount, sd->status.char_id, (int)item_z_after_tax);
...
int8 vending_openvending(map_session_data* sd, const char* message, const uint8* data, int count, struct s_autotrader *at)
{
	...
	// ========================================================
	// ??? [C++ Engine แก้ไข] ระบบสแกนเว้นระยะห่างเปิดป้ายตั้งขายขั้นต่ำ 2 ช่อง
	// ?? บังคับตรวจสอบเฉพาะผู้เล่นจริงที่กดเปิดร้านปกติ ส่วนบอทเปิดเซิร์ฟโหลด Autotrade (at != NULL) ให้ปล่อยผ่านทันที
	// ========================================================
	if (!at) {
		int rx, ry;
		bool is_vending_space_blocked = false;

		for (rx = sd->bl.x - 2; rx <= sd->bl.x + 2 && !is_vending_space_blocked; rx++) {
			for (ry = sd->bl.y - 2; ry <= sd->bl.y + 2; ry++) {
				if (map_getcell(sd->bl.m, rx, ry, CELL_CHKNOPASS) || map_getcell(sd->bl.m, rx, ry, CELL_CHKNOVENDING)) {
					is_vending_space_blocked = true;
					break;
				}
				if (map_count_oncell(sd->bl.m, rx, ry, BL_PC, 1) > 0) {
					is_vending_space_blocked = true;
					break;
				}
				if (map_count_oncell(sd->bl.m, rx, ry, BL_NPC | BL_STALL, 0) > 0) {
					is_vending_space_blocked = true;
					break;
				}
			}
		...
		if (is_vending_space_blocked) {
			clif_displaymessage(sd->fd, "ระบบไม่อนุญาตให้กางป้ายตั้งร้านขายของในรัศมี 2 ช่องรอบ NPC หรือป้ายร้านค้าอื่นค่ะ!");
			clif_skill_fail(sd, MC_VENDING, USESKILL_FAIL_LEVEL, 0);
			return 5;
		}
	}
	// ======================================================================================================
	// ==========================================
	// [Addon] City Faction War - Vending Restriction
	// ==========================================
	...
	if (city_faction_war == 1 && battle_config.faction_vending_block == 1) {
		...
		if (mapdata != nullptr) {
			// เขต Prontera (ต้องเป็นฝ่าย 2 เท่านั้น)
			if (strcmp(mapdata->name, "prontera") == 0 && my_faction != 2) {
				clif_displaymessage(sd->fd, msg_txt(sd, 3000)); 
				clif_skill_fail(sd, MC_VENDING, USESKILL_FAIL_LEVEL, 0);
				return 1; 
			}
			// เขต Morocc (ต้องเป็นฝ่าย 1 เท่านั้น)
			else if (strcmp(mapdata->name, "morocc") == 0 && my_faction != 1) {
				clif_displaymessage(sd->fd, msg_txt(sd, 3001)); 
				clif_skill_fail(sd, MC_VENDING, USESKILL_FAIL_LEVEL, 0);
				return 1; 
			}
		}
	...
	// --- [เพิ่มใหม่] แจ้งเตือนภาษีก่อนตั้งร้าน และล็อคค่าภาษี ---
	int current_tax_rate = battle_config.vending_tax / 100; // ค่าเริ่มต้น
	if (Sql_Query(mmysql_handle, "SELECT `tax_rate` FROM `mayor_towns` WHERE `town_map` = '%s' AND `is_active` = 1", map_mapid2mapname(sd->bl.m)) == SQL_SUCCESS) {
		if (Sql_NumRows(mmysql_handle) > 0 && Sql_NextRow(mmysql_handle) == SQL_SUCCESS) {
			char* db_data;
			Sql_GetData(mmysql_handle, 0, &db_data, NULL);
			if (db_data) current_tax_rate = atoi(db_data);
		}
		Sql_FreeResult(mmysql_handle);
	}

	// ส่งข้อความ 1593 ไปหาพ่อค้าตอนตั้งร้าน (จำลองหน้าต่าง NPC mes)
	if(current_tax_rate > 0) {
		char tax_info[256];
		sprintf(tax_info, msg_txt(sd, 1593), current_tax_rate, (100 - current_tax_rate));
		
		clif_scriptmes(*sd, sd->bl.id, tax_info);
		clif_scriptclose(sd, sd->bl.id);
	}

	// ล็อคค่าภาษีไว้ที่ตัวพ่อค้าทันที!
	sd->vending_tax_locked = current_tax_rate; 
	// -----------------------------------------------------
