...
int8 buyingstore_setup(map_session_data* sd, unsigned char slots){
	...
	// ==========================================
	// [Addon] City Faction War - Buying Store UI 
	// ==========================================
	...
	if (city_faction_war == 1 && battle_config.faction_vending_block == 1) {
		...
		if (mapdata != nullptr) {
			if (strcmp(mapdata->name, "prontera") == 0 && my_faction != 2) {
				clif_displaymessage(sd->fd, msg_txt(sd, 3002)); // ดึงข้อความ 3002
				return 3;
			}
			else if (strcmp(mapdata->name, "morocc") == 0 && my_faction != 1) {
				clif_displaymessage(sd->fd, msg_txt(sd, 3003)); // ดึงข้อความ 3003
				return 3;
			}
		}
	}
...
if( map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKNOBUYINGSTORE) )
	...
	// ==========================================
	// [Addon] City Faction War - Buying Store Create (Anti-WPE)
	// ==========================================
	extern int city_faction_war;
	if (city_faction_war == 1 && battle_config.faction_vending_block == 1) {
		int my_faction = pc_readaccountreg(sd, add_str("#city_faction"));
	...
	// ========================================================
	// 🛡️ [C++ Engine แก้ไข] ระบบสแกนเว้นระยะห่างเปิดป้ายรับซื้อขั้นต่ำ 2 ช่อง
	// 🌟 ดักจับเฉพาะผู้เล่นปกติ ส่วนบอทโหลดเซิร์ฟเวอร์ Autotrade (at != NULL) ปล่อยผ่านชิว ๆ
	// ========================================================
	if (!at) {
		int rx, ry;
		bool is_buying_space_blocked = false;

		for (rx = sd->bl.x - 2; rx <= sd->bl.x + 2 && !is_buying_space_blocked; rx++) {
			for (ry = sd->bl.y - 2; ry <= sd->bl.y + 2; ry++) {
				if (map_getcell(sd->bl.m, rx, ry, CELL_CHKNOPASS) || map_getcell(sd->bl.m, rx, ry, CELL_CHKNOBUYINGSTORE)) {
					is_buying_space_blocked = true;
					break;
				}
		...
		if (is_buying_space_blocked) {
			clif_displaymessage(sd->fd, "ระบบไม่อนุญาตให้ตั้งร้านค้ารับซื้อในรัศมี 2 ช่องรอบ NPC หรือป้ายร้านค้าอื่นค่ะ!");
			sd->buyingstore.slots = 0;
			clif_buyingstore_open_failed(sd, BUYINGSTORE_CREATE, 0);
			return 1;
		}
	...
	// check item list (โค้ดเดิมของเซิร์ฟเวอร์รันต่อตรงนี้ปกติ)
	for( i = 0; i < count; i++ ){
