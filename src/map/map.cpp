...
{
	int pos;
	int16 x, y; // เพิ่มตัวแปรสำหรับพิกัด
	nullpo_ret(bl);
	...
	// --- [แก้ไข] เพิ่มการตรวจสอบขอบเขต (Boundary Check) ป้องกัน Segfault ---
	if( x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys )
	{
		ShowError("map_delblock: out-of-bounds coordinates (\"%s\",%d,%d), map is %dx%d\n", mapdata->name, x, y, mapdata->xs, mapdata->ys);
		
		// บีบพิกัดให้อยู่ในขอบเขตแผนที่ที่ปลอดภัย เพื่อป้องกันการคำนวณ Array Index ติดลบหรือเกินขนาด
...
int map_getcellp(struct map_data* m,int16 x,int16 y,cell_chk cellchk)
{
	...
	switch(cellchk)
	{
		...
		case CELL_CHKICEWALL:
			return (cell.icewall);
		case CELL_CHKPVP:	// Addon Cell PVP
			return (cell.pvp);
// ----- [Addon] เขตสันติภาพ ห้ามสงคราม 2 เมือง -----
		case CELL_CHKNOFACTIONWAR:
			return (cell.nofactionwar);
		// ------------------------------------------------
...
enum e_mapflag map_getmapflag_by_name(char* name)
{
	...
	// ----- [Addon] ดักจับชื่อธง nofactionwar แบบตรงๆ -----
	if (!strcasecmp(name, "nofactionwar")) return MF_NOFACTIONWAR;
	// --------------------------------------------------
...
// Addon Cell PVP: ระบบจัดการเมื่อเข้า/ออกพื้นที่
int map_pvp_area(struct map_session_data* sd, bool flag)
{
	if (!sd) return 0;

	if (flag) { // เมื่อก้าวเข้าโซน
		if (sd->pvp_timer == INVALID_TIMER) {
			sd->state.pvp = 1; 
			clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF); 
			
			// ถ้าเขา "ไม่ได้" กดยา PK PASS มาก่อน ค่อยเปิดออร่าแดงให้ 
			// (ถ้าเขากดมาแล้ว ออร่ามันจะติดอยู่แล้ว ไม่ต้องส่งซ้ำ)
			if (!sd->attacker) {
			...
			clif_displaymessage(sd->fd, "--- Welcome to PVP Arena: Free PK! ---");
		}
	} else { // เมื่อก้าวออกโซน
		if (sd->state.pvp) {
			sd->state.pvp = 0; 

			// --- [ป้องกันบั๊กทับซ้อน PK PASS] ---
			// 1. ถ้าเขาไม่ได้เป็น Attacker ของระบบ PK ให้ลบออร่าแดงของลานประลองทิ้ง
			if (!sd->attacker) {
				pk_pass_attack_effect_off(&sd->bl); 
			}
			
			// 2. ถ้าเขายังมีบัฟยา PK PASS (ไม่ว่าจะโจมตีหรือป้องกัน) ค้างอยู่ ให้คงสถานะเมาส์ดาบไว้
			if (sd->attacker || sd->protector) {
				clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
			} else {
				// 3. แต่ถ้าเป็นคนธรรมดาที่ไม่ได้กดยา ให้คืนค่าเมาส์เป็นรูปมือปกติ
