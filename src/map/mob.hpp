...
extern std::unordered_map<uint32, int> stealth_drop_rates;
...
// ---------------------------------------------------------
// Purpose:
//   Event-style drops that must behave like "real mob drops"
//   so Bubble Gum / drop bonuses / penalties can apply.
//
// Notes:
//   - Stored in memory only; cleared on map-server restart.
//   - Rate uses rAthena drop scale: 1..10000 (10000 = 100%).
//   - Duration is optional; expired entries are pruned on use.
// =========================================================
struct mob_dynamic_drop {
	t_itemid nameid;
	uint16 rate;      // 1..10000
	bool steal_protected;
	uint16 randomopt_group;
	t_tick expire_tick; // 0 = no expiry
};
...
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, t_itemid nameid, bool is_real, mob_data* md = nullptr);
