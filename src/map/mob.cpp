...
struct ZenyBountyConfig {
	...
	int aoe_penalty;
	int mvp_mult; 
	int map_change_interval; 
	int require_stamina; 
	int drop_chance; 
	int aa_chance_penalty; 
	std::string gm_override_map; 
	std::vector<uint16> aoe_skills;
	std::vector<std::string> maps_low;  // แมพเวลน้อย
	std::vector<std::string> maps_mid;  // แมพเวลกลาง
	std::vector<std::string> maps_high; // แมพเวลสูง
} zeny_conf;

// =========================================================
// [Item Daily] Dynamic Option Database Config
// =========================================================
struct DailyOptionRule {
	int type;
	int subtype_equip; 
	int min_level;
	std::string group_name;
};
std::vector<DailyOptionRule> daily_option_rules;

void item_daily_option_init() {
	daily_option_rules.clear();
	FILE* fp = fopen("db/custom/item_daily_option.conf", "r");
	if (!fp) { 
		ShowWarning("Item Daily Option: 'db/custom/item_daily_option.conf' not found.\n"); 
		return; 
	}
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		char t_str[128], s_str[128], g_str[128];
		int min_lv = 0;
		
		if (sscanf(line, " %127[^,], %127[^,], %d, %127[^\r\n] ", t_str, s_str, &min_lv, g_str) == 4) {
			auto trim = [](char* s) {
				char* start = s;
			...
			trim(t_str); trim(s_str); trim(g_str);
			
			int64 t_val = 0, s_val = 0;
			
			if (!script_get_constant(t_str, &t_val)) t_val = atoi(t_str);
			if (strcmpi(s_str, "ANY") != 0 && strcmpi(s_str, "0") != 0) {
				if (!script_get_constant(s_str, &s_val)) s_val = atoi(s_str);
			}
			
			daily_option_rules.push_back({(int)t_val, (int)s_val, min_lv, std::string(g_str)});
		}
	}
	fclose(fp);
	ShowStatus("Item Daily Option: Loaded %d custom rules successfully.\n", (int)daily_option_rules.size());
}

#include <unordered_set>

// ตัวแปรเก็บ ID ไอเทมที่อยู่ใน Blacklist
std::unordered_set<t_itemid> blocked_tame_items;

void tame_blacklist_init() {
	blocked_tame_items.clear();
	const char* filepath = "db/custom/disable_tame_items.txt";
	FILE* fp = fopen(filepath, "r");
	
	if (!fp) {
		ShowWarning("Tame Blacklist: '%s' not found.\n", filepath);
		return;
	}

	char line[1024];
	while (fgets(line, sizeof(line), fp)) {
		if (line[0] == '/' && line[1] == '/') continue; // ข้ามบรรทัดคอมเมนต์
		
		// ตัดตัวขึ้นบรรทัดใหม่ออก
		char* p = strchr(line, '\n'); if (p) *p = 0;
		...
		// ค้นหา ID จากชื่อ AegisName
		std::shared_ptr<item_data> it = item_db.search_aegisname(line);
		if (it != nullptr) {
			blocked_tame_items.insert(it->nameid);
		} else {
			ShowWarning("Tame Blacklist: Unknown item '%s'\n", line);
		}
	}
	fclose(fp);
	ShowStatus("Tame Blacklist: Loaded %zu items successfully.\n", blocked_tame_items.size());
}
...
void zeny_bounty_init() {
	...
	zeny_conf.require_stamina = 1; zeny_conf.drop_chance = 30; zeny_conf.aa_chance_penalty = 50; 
	zeny_conf.gm_override_map = "none"; zeny_conf.aoe_skills = {89, 84, 83, 85, 142, 153, 384};
	zeny_conf.maps_low = {"prt_fild08"}; zeny_conf.maps_mid = {"moc_fild02"}; zeny_conf.maps_high = {"lhz_fild01"};

	FILE* fp = fopen("db/custom/zeny_global.conf", "r");
	if (!fp) { ShowInfo("Zeny Bounty: 'db/custom/zeny_global.conf' not found. Using defaults.\n"); return; }

	char line[1024], key[1024], val[1024];
	while (fgets(line, sizeof(line), fp)) {
		if (line[0] == '/' && line[1] == '/') continue;
		if (sscanf(line, "%1023[^:]: %1023[^\r\n]", key, val) == 2) {
			char* ptr = val + strlen(val) - 1;
			...
			else if (strcmpi(key, "AoEPenalty") == 0) zeny_conf.aoe_penalty = atoi(ptr);
			else if (strcmpi(key, "MvpMultiplier") == 0) zeny_conf.mvp_mult = atoi(ptr);
			else if (strcmpi(key, "MapChangeInterval") == 0) zeny_conf.map_change_interval = atoi(ptr); 
			else if (strcmpi(key, "RequireStamina") == 0) zeny_conf.require_stamina = atoi(ptr); 
			else if (strcmpi(key, "DropChance") == 0) zeny_conf.drop_chance = atoi(ptr); 
			else if (strcmpi(key, "AutoAttackChancePenalty") == 0) zeny_conf.aa_chance_penalty = atoi(ptr);
			else if (strcmpi(key, "GmOverrideMap") == 0) zeny_conf.gm_override_map = std::string(ptr); 
			else if (strcmpi(key, "AoESkills") == 0) {
				zeny_conf.aoe_skills.clear();
				char* p = strtok(ptr, ", ");
				while (p) { zeny_conf.aoe_skills.push_back(atoi(p)); p = strtok(NULL, ", "); }
			}
			// อ่านค่าแมพ 3 ระดับ
			else if (strcmpi(key, "MapsLow") == 0) {
				zeny_conf.maps_low.clear(); char* p = strtok(ptr, ", "); while (p) { zeny_conf.maps_low.push_back(std::string(p)); p = strtok(NULL, ", "); }
			}
			else if (strcmpi(key, "MapsMid") == 0) {
				zeny_conf.maps_mid.clear(); char* p = strtok(ptr, ", "); while (p) { zeny_conf.maps_mid.push_back(std::string(p)); p = strtok(NULL, ", "); }
			}
...
// ---------------------------------------------------------
// See mob.hpp for details.
// =========================================================
static std::unordered_map<uint16, std::vector<mob_dynamic_drop>> mob_dynamic_drop_db;

// [Custom] Stealth Drop
std::unordered_map<uint32, int> stealth_drop_rates;

bool mob_dynamic_drop_add(uint16 mob_id, t_itemid nameid, uint16 rate, t_tick expire_tick, bool steal_protected, uint16 randomopt_group){
	if( mob_id == 0 || nameid == 0 )
		return false;
	if( rate < 1 || rate > 10000 )
		return false;

	auto& vec = mob_dynamic_drop_db[mob_id];

	// update existing entry
	for( auto& d : vec ){
		if( d.nameid == nameid ){
			d.rate = rate;
			d.expire_tick = expire_tick;
			d.steal_protected = steal_protected;
			d.randomopt_group = randomopt_group;
			return true;
		}
	}
	...
	nd.nameid = nameid;
	nd.rate = rate;
	nd.expire_tick = expire_tick;
	nd.steal_protected = steal_protected;
	nd.randomopt_group = randomopt_group;
	vec.push_back( nd );
	return true;
}

bool mob_dynamic_drop_remove(uint16 mob_id, t_itemid nameid){
	auto it = mob_dynamic_drop_db.find( mob_id );
	if( it == mob_dynamic_drop_db.end() )
		return false;
	auto& vec = it->second;
	size_t before = vec.size();
	vec.erase( std::remove_if( vec.begin(), vec.end(), [&]( const mob_dynamic_drop& d ){
		return d.nameid == nameid;
	}), vec.end() );
	if( vec.empty() )
		mob_dynamic_drop_db.erase( it );
	return before != vec.size();
}

void mob_dynamic_drop_clear_all(void){
	mob_dynamic_drop_db.clear();
}
...
struct mob_data* mob_spawn_dataset(struct spawn_data *data)
{
	...
	// --- [ดึงค่าจาก Config: กดหัวเลเวลมอนสเตอร์ตอนเกิดในระบบ Mob] ---
	if (battle_config.mob_max_level_cap > 0 && md->level > battle_config.mob_max_level_cap) {
		md->level = battle_config.mob_max_level_cap;
	}
	// --------------------------------------------------------
...
static int mob_ai_sub_hard_activesearch(struct block_list *bl,va_list ap)
{
	...
	// --- Advanced Minion Class Hate ---
	if (battle_config.minion_hate_range > 0 && md->master_id > 0 && bl->type == BL_PC) {
		struct mob_data *master_md = (struct mob_data *)map_id2bl(md->master_id);
		if (master_md) {
			int current_master_type = 1; 
			if (master_md->get_bosstype() == BOSSTYPE_MVP) current_master_type = 3; 
			else if (master_md->get_bosstype() == BOSSTYPE_MINIBOSS) current_master_type = 2;

			if (battle_config.penalty_mob_type == 4 || battle_config.penalty_mob_type == current_master_type) {
				map_session_data *tsd = (map_session_data *)bl;
				int target_job = tsd->status.class_;
				if (target_job >= 0 && target_job < 6000 && battle_config.penalty_job_list[target_job] == true) {
					if (distance_bl(&md->bl, bl) <= battle_config.minion_hate_range) {
						(*target) = bl;
						md->target_id = bl->id;
						md->state.aggressive = 1; 
						return 1; // บังคับล็อคเป้าหมายนี้ทันที
					}
				}
			}
		}
...
int mob_randomwalk(struct mob_data *md,t_tick tick)
{
	...
	// ===================================================
	// 🔥 [Custom AI] ระบบ Fake Player (เดิน 30 ช่อง + อโคบัพตัวเอง)
	// ===================================================
	if (md->npc_event[0] != '\0') {
		
		if (strstr(md->npc_event, "OnParade")) return 0; 
		
		if (strstr(md->npc_event, "OnFieldDead")) {
			// อโค (Class 4) หรือ พรีส (Class 8): ยืนว่างๆ ให้บัพ/ฮีล ตัวเองก่อนเดิน
			if ((md->vd->class_ == 4 || md->vd->class_ == 8) && DIFF_TICK(tick, md->ud.canact_tick) > 0) {
				if (md->status.hp < md->status.max_hp) {
					unit_skilluse_id(&md->bl, md->bl.id, 28, 10); // Heal
					md->ud.canact_tick = tick + 1500; return 0; 
				} else if (rnd() % 100 < 20) { 
					int skill_id = (rnd() % 2 == 0) ? 34 : 29; // Bless หรือ Agi
					unit_skilluse_id(&md->bl, md->bl.id, skill_id, 10); 
					md->ud.canact_tick = tick + 2000; return 0; 
				}
			}

			// เดินไกล 30 ช่อง
			int nx = md->bl.x + (rnd() % 61 - 30); 
			int ny = md->bl.y + (rnd() % 61 - 30);
			if (map_getcell(md->bl.m, nx, ny, CELL_CHKPASS)) { 
				if (unit_walktoxy(&md->bl, nx, ny, 0)) {
					md->next_walktime = tick + rnd_value(3000, 5000); 
					return 1;
				}
			}
			return 0; 
		...
		if (strstr(md->npc_event, "OnTownBot") || strstr(md->npc_event, "OnBotDead")) {
			int target_x = md->bl.x + (rnd() % 31 - 15);
			int target_y = md->bl.y + (rnd() % 31 - 15);
			if (map_getcell(md->bl.m, target_x, target_y, CELL_CHKPASS)) {
				if (unit_walktoxy(&md->bl, target_x, target_y, 0)) {
					md->next_walktime = tick + rnd_value(3000, 8000);
					return 1;
				}
			}
			return 0;
	...
	r=rnd(); // โค้ดสุ่มเดินปกติของเซิร์ฟเวอร์จะอยู่ต่อจากตรงนี้
...
static bool mob_ai_sub_hard(struct mob_data *md, t_tick tick)
{
	...
	if( md->attacked_id && mode&MD_CANATTACK )
	{
		...
		{	//Rude attacked check.
			...
			// บล็อกโคลนไม่ให้วิงหนีเมื่อโดนตีฟรี เพื่อลดภาระเซิร์ฟเวอร์
			&&  !(md->npc_event[0] != '\0' && strstr(md->npc_event, "OnFieldDead")) 
		...
		if( (abl = map_id2bl(md->attacked_id)) && (!tbl || mob_can_changetarget(md, abl, mode)) )
		{
			...
			if( md->bl.m != abl->m || abl->prev == NULL
				...
				// ===================================================
				// 🔥 [Custom AI] ยกเว้นให้โคลน (OnFieldDead) เป็นศัตรูที่มอนสเตอร์ต้องสู้กลับ ไม่วิงหนี!
				// ===================================================
				|| (battle_check_target(&md->bl, abl, BCT_ENEMY) <= 0 && !(abl->type == BL_MOB && ((struct mob_data*)abl)->npc_event[0] != '\0' && strstr(((struct mob_data*)abl)->npc_event, "OnFieldDead")))
	...
	// ===================================================
	// 🔥 [Custom AI] โคลนป่า (OnFieldDead) ทำตัวเหมือนผู้เล่น
	// ===================================================
	if ((mode&MD_AGGRESSIVE && (!tbl || slave_lost_target)) || md->state.skillstate == MSS_FOLLOW)
	{
		map_foreachinallrange (mob_ai_sub_hard_activesearch, &md->bl, view_range, 
			(md->npc_event[0] != '\0' && strstr(md->npc_event, "OnFieldDead")) ? BL_MOB : DEFAULT_ENEMY_TYPE(md), 
			md, &tbl, mode);
	}
	else
	if (mode&MD_CHANGECHASE && (md->state.skillstate == MSS_RUSH || md->state.skillstate == MSS_FOLLOW))
	{
		...
		map_foreachinallrange (mob_ai_sub_hard_changechase, &md->bl, search_size, 
			(md->npc_event[0] != '\0' && strstr(md->npc_event, "OnFieldDead")) ? BL_MOB : DEFAULT_ENEMY_TYPE(md), 
			md, &tbl);
	}
	// ===================================================
...
//Target exists, attack or loot as applicable.
	...
	// ===================================================
	// 🔥 [Custom AI] สมองต่อสู้แยกระดับอาชีพ (อัพเกรดสกิลจัดเต็ม!)
	// ===================================================
	if (md->npc_event[0] != '\0' && strstr(md->npc_event, "OnFieldDead")) {
		
		int c_class = md->vd->class_;
		
		// 1. ฮีลและบัพตัวเอง (Acolyte=4 / Priest=8)
		if ((c_class == 4 || c_class == 8) && DIFF_TICK(tick, md->ud.canact_tick) > 0) {
			if (md->status.hp < md->status.max_hp / 2) {
				unit_skilluse_id(&md->bl, md->bl.id, 28, 10); // Heal
				md->ud.canact_tick = tick + 1500;
				return true;
			}
			// เช็คว่าถ้ายังไม่มี Bless ให้ร่าย
			if (!md->sc.getSCE(SC_BLESSING)) {
				unit_skilluse_id(&md->bl, md->bl.id, 34, 10); // Bless
				md->ud.canact_tick = tick + 1500;
				return true;
			}
			// เช็คว่าถ้ายังไม่มี Agi ให้ร่าย
			if (!md->sc.getSCE(SC_INCREASEAGI)) {
				unit_skilluse_id(&md->bl, md->bl.id, 29, 10); // Agi
				md->ud.canact_tick = tick + 1500;
				return true;
			}
		}

		if (tbl && battle_check_range(&md->bl, tbl, 10) && DIFF_TICK(tick, md->ud.canact_tick) > 0) {
			
			// 2. Mage=2 / Wizard=9 : สาดเวทย์รัวๆ 
			if (c_class == 2 || c_class == 9) {
				int r = rnd() % 3;
				int skill_id = (r == 0) ? 19 : ((r == 1) ? 20 : 81); // Fire, Cold, Lightning
				unit_skilluse_id(&md->bl, tbl->id, skill_id, 5); 
				md->ud.canact_tick = tick + 2000;
				return true; // บังคับหยุด ไม่ให้เดินไปตีประชิด
			}
			
			// 3. Archer=3 / Hunter=11 : Double Strafe (46)
			if (c_class == 3 || c_class == 11) {
				if (rnd() % 100 < 50) { // โอกาส 50% ร่ายสกิล (สลับกับยิงธรรมดา)
					unit_skilluse_id(&md->bl, tbl->id, 46, 10); // Double Strafe
					md->ud.canact_tick = tick + 1500;
					return true; 
				}
			}
			
			// 4. อาชีพโจมตีประชิด (ต้องเดินเข้าประชิดก่อนถึงจะใช้สกิล)
			if (battle_check_range(&md->bl, tbl, md->status.rhw.range)) {
				
				// Swordman=1 / Knight=7 : Bash (5)
				if (c_class == 1 || c_class == 7) {
					if (rnd() % 100 < 40) {
						unit_skilluse_id(&md->bl, tbl->id, 5, 10); // Bash
						md->ud.canact_tick = tick + 1500;
						return true;
					}
				}
				// Merchant=5 / Blacksmith=10 : Mammonite (39)
				else if (c_class == 5 || c_class == 10) {
					if (rnd() % 100 < 40) {
						unit_skilluse_id(&md->bl, tbl->id, 42, 10); // Mammonite
						md->ud.canact_tick = tick + 1500;
						return true;
					}
				}
				// Thief=6 / Assassin=12 : Double Attack 
				// (เราใช้สกิล NPC_COMBOATTACK (175) เพื่อจำลองการฟันรัวๆ แบบ Double Attack ของมอนสเตอร์)
				else if (c_class == 6 || c_class == 12) {
					if (rnd() % 100 < 40) {
						unit_skilluse_id(&md->bl, tbl->id, 48, 10); // Combo Attack
						md->ud.canact_tick = tick + 1500;
						return true;
					}
				}
...
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, t_itemid nameid, bool is_real, mob_data* md)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			if(is_real){
				if( sd->sc.getSCE(SC_GUM_ETC) && id->type == IT_ETC )
					drop_rate_bonus += sd->sc.getSCE(SC_GUM_ETC)->val2;

				if( sd->sc.getSCE(SC_GUM_USABLE) && (id->type == IT_USABLE || id->type == IT_HEALING) )
					drop_rate_bonus += sd->sc.getSCE(SC_GUM_USABLE)->val2;
				
				if( sd->sc.getSCE(SC_GUM_EQUIP) && (id->type == IT_ARMOR || id->type == IT_WEAPON) )
					drop_rate_bonus += sd->sc.getSCE(SC_GUM_EQUIP)->val2;

				if( sd->sc.getSCE(SC_GUM_CARD) && id->type == IT_CARD )
					drop_rate_bonus += sd->sc.getSCE(SC_GUM_CARD)->val2;
				...
				if( sd->sc.getSCE(SC_GUM_USABLE) && (id->type == IT_USABLE || id->type == IT_HEALING) )
					drop_rate_bonus += sd->sc.getSCE(SC_GUM_USABLE)->val1;
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if(!(type&2) && //No exp
		...
		// ===============================================================
		// Zeny Bounty Map Logic
		// ===============================================================
		time_t rawtime; time(&rawtime);
		const char* dead_mob_map = map_mapid2mapname(md->bl.m);
		...
		if (!zeny_conf.system_enable || zeny_conf.drop_chance <= 0) {
			...
			if (strstr(zeny_conf.gm_override_map.c_str(), dead_mob_map) != nullptr) is_bounty_map = true;
		} 
		else {
			int safe_interval = (zeny_conf.map_change_interval > 0) ? zeny_conf.map_change_interval : 60;
			int current_block = (rawtime / 60) / safe_interval; 

			auto check_match = [&](std::vector<std::string>& v, int offset) -> bool {
		...
		for(i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if(battle_config.zeny_from_mobs && md->level && zeny_conf.system_enable && zeny_conf.drop_chance > 0) {
				...
				int final_drop_chance = zeny_conf.drop_chance; // โอกาสดรอปตั้งต้น (เช่น 30%)
				
				// ถ้าเปิด Auto Attack ให้หักเปอร์เซ็นต์โอกาสดรอปลง
				if (is_autoattack && zeny_conf.aa_chance_penalty > 0) {
					int penalty = zeny_conf.aa_chance_penalty;
					if (penalty > 100) penalty = 100; // กันบัคใส่ค่าเกิน 100
					
					// สูตร: 30 * (100 - 50) / 100 = 15%
					final_drop_chance = (final_drop_chance * (100 - penalty)) / 100;
				}
				
				// ==========================================
				// --- ระบบ Zeny Gum (เพิ่มโอกาสดรอปเงิน) ---
				// ==========================================
				if (tmpsd[i]->sc.getSCE(SC_ZENYGUM)) {
					int bonus_rate = tmpsd[i]->sc.getSCE(SC_ZENYGUM)->val1; 
					
					// [สูตรที่ 1] คูณ % โอกาสเพิ่มจากของเดิม
					// (เช่น โอกาสดั้งเดิม 30% กดกัม +100% โอกาสจะกลายเป็น 60%)
					final_drop_chance = final_drop_chance + (final_drop_chance * bonus_rate / 100);
					
					// [หรือถ้าอยากใช้สูตร 2 แบบสายเปย์ บวกทับไปเลย ให้ลบ // บรรทัดล่างออก แล้วใส่ // ทับบรรทัดบนครับ]
					// final_drop_chance += bonus_rate; 
				}

				// ล็อกเป้าไม่ให้โอกาสเกิน 100%
				if (final_drop_chance > 100) 
					final_drop_chance = 100;
				// ==========================================
				
				// สุ่มโอกาสได้รับเงินตามตัวแปร final_drop_chance
				bool is_lucky = (rnd() % 100) < final_drop_chance;
				
				if (zeny_conf.require_stamina == 1 && !has_stamina) {
					zeny = 0; 
				} else if (!is_bounty_map) {
					zeny = 0; // ตีผิดแมพ
				} else if (!is_lucky && md->get_bosstype() != BOSSTYPE_MVP) {
					zeny = 0; // โชคไม่ดี สุ่มไม่ผ่าน (ยกเว้นบอส)
				} else {
					int player_lv = tmpsd[i]->status.base_level;
					int mob_lv = md->level;

					// --- Custom: Spoof Monster Level for Level Penalty Only ---
					// สร้างตัวแปรหลอกขึ้นมา เพื่อไม่ให้กระทบก้อนเงินตั้งต้น
					int spoofed_mob_lv = mob_lv;
					if (battle_config.mob_max_level_cap > 0 && spoofed_mob_lv > battle_config.mob_max_level_cap) {
						spoofed_mob_lv = battle_config.mob_max_level_cap;
					}
					// ----------------------------------------------------------
					
					// --- Level Penalty ---
					// *** จุดสำคัญ 1: ใช้ตัวแปรหลอก (spoofed_mob_lv) เพื่อไม่ให้โดนหักเงิน ***
					int level_diff = abs(player_lv - spoofed_mob_lv); 
					int lv_penalty = 100; 
					if (level_diff > zeny_conf.level_diff) { 
						lv_penalty = 100 - ((level_diff - zeny_conf.level_diff) * zeny_conf.penalty_per_level); 
						if (lv_penalty < zeny_conf.min_penalty) lv_penalty = zeny_conf.min_penalty; 
					}

					// --- AoE Penalty ---
					uint16 last_skill = tmpsd[i]->ud.skill_id; 
					int aoe_penalty = 100; 
					if (std::find(zeny_conf.aoe_skills.begin(), zeny_conf.aoe_skills.end(), last_skill) != zeny_conf.aoe_skills.end()) {
						aoe_penalty = zeny_conf.aoe_penalty; 
					}

					// --- คำนวณ Zeny ตั้งต้นจากสูตร Character Level ---
					// *** จุดสำคัญ 2: ใช้เลเวลจริง (mob_lv) เพื่อให้ก้อนเงินยังใหญ่สมฐานะบอส 150 ***
					int64 min_zeny = (player_lv + (mob_lv * 8)) / 4;
					int64 max_zeny = (player_lv + (mob_lv * 16)) / 2;
					
					if (min_zeny > max_zeny) std::swap(min_zeny, max_zeny); 
					int64 base_zeny = (max_zeny == min_zeny) ? min_zeny : (rnd() % (max_zeny - min_zeny + 1)) + min_zeny;

					// --- คำนวณผลลัพธ์สุทธิ ---
					int64 final_zeny = (int64)(base_zeny * per * bonus / 100. * lv_penalty / 100. * aoe_penalty / 100.);

					if( md->get_bosstype() == BOSSTYPE_MVP ) {
						final_zeny *= zeny_conf.mvp_mult;
					}

					// ล็อกเพดานเงินไม่ให้เกิน MAX_ZENY
					if (final_zeny > MAX_ZENY) final_zeny = MAX_ZENY;
					if (final_zeny < 0) final_zeny = 0;

					zeny = (int)final_zeny; 

					// ==========================================
					// 🔥 แจกเงินเข้าตัว + ระบบแชร์ปาร์ตี้ (อิงการตั้งค่า "แชร์ EXP")
					// ==========================================
					if (zeny > 0) {
						struct party_data *p = (tmpsd[i]->status.party_id > 0) ? party_search(tmpsd[i]->status.party_id) : NULL;

						// 1. เช็คว่ามีปาร์ตี้ และตั้งค่าปาร์ตี้เป็น "แชร์ประสบการณ์" (Even Share EXP = p->party.exp > 0)
						if (p && p->party.exp > 0) {
							int p_count = 0;
							map_session_data *p_sd[MAX_PARTY]; // เก็บรายชื่อคนที่ออนไลน์และอยู่แมพเดียวกัน

							// นับจำนวนลูกตี้ที่ออนไลน์และอยู่แมพเดียวกัน
							for (int m = 0; m < MAX_PARTY; m++) {
								map_session_data *pmd = p->data[m].sd;
								if (pmd && pmd->bl.m == md->bl.m) {
									p_sd[p_count++] = pmd;
								}
							}

							if (p_count > 0) {
								int share_zeny = zeny / p_count; // เอาเงินส่วนดาเมจนั้นมาหารตามจำนวนคนในแมพ
								if (share_zeny > 0) {
									for (int m = 0; m < p_count; m++) {
										pc_getzeny(p_sd[m], share_zeny, LOG_TYPE_PICKDROP_MONSTER);
										char zeny_msg[128];
										sprintf(zeny_msg, msg_txt(p_sd[m], 670), share_zeny); // ข้อความสีเหลือง
										clif_disp_overhead(&p_sd[m]->bl, zeny_msg);
									}
								}
							}
						} 
						// 2. กรณีไม่มีปาร์ตี้, ไม่ได้ตั้งแชร์ EXP, หรือเลเวลห่างกันจนแชร์ EXP ไม่ได้
						else {
							// ใครทำดาเมจ คนนั้นได้เงินเข้าตัวไปเลย (ของใครของมัน)
							pc_getzeny(tmpsd[i], zeny, LOG_TYPE_PICKDROP_MONSTER);
							
							char zeny_msg[128];
							sprintf(zeny_msg, msg_txt(tmpsd[i], 670), zeny);
							clif_disp_overhead(&tmpsd[i]->bl, zeny_msg);
						}
					} // <--- ปิด if (zeny > 0)

					// *** จุดสำคัญ ***: เซ็ตค่า zeny เป็น 0 ทิ้งซะ!
					// เพื่อป้องกันไม่ให้ระบบ rAthena เอาเงินไปแจกซ้ำเบิ้ลสองในระบบ party เดิม
					zeny = 0; 
				} // <--- ปิดเงื่อนไข if (battle_config.zeny_from_mobs && md->level) (ที่ครอบระบบ Zeny)
			} // <--- ปิดลูป for (i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++)
			// ===============================================================
...
#endif
						// บรรทัดนี้สำคัญ ห้ามลบ! เป็นตัวแจก EXP เข้าตัวละคร
						pc_gainexp(tmpsd[i], &md->bl, base_exp, job_exp, 0);
					}
				}
			}
			
			// เคลียร์ประวัติการทำดาเมจของบอส MVP
			if( md->get_bosstype() == BOSSTYPE_MVP )
				pc_damage_log_clear(tmpsd[i],md->bl.id);
		}

		// ระบบแชร์ EXP ปาร์ตี้ของ rAthena (Zeny จะกลายเป็น 0 เพราะเราย้ายไปหารเองด้านบนแล้ว)
...
for (i = 0; i < MAX_MOB_DROP; i++) {
    ...
    // 2. เช็คเงื่อนไข Stamina และการมีอยู่ของตัวละครผู้เล่น (sd)
    if (!sd || (md->get_bosstype() != BOSSTYPE_MVP && 
        md->mob_id != 1388 && md->mob_id != 1641 && md->mob_id != 1307 && md->mob_id != 1283 && md->mob_id != 1302 && 
        md->mob_id != 1582 && md->mob_id != 1091 && md->mob_id != 1093 && md->mob_id != 1957 && md->mob_id != 1205 && 
        md->mob_id != 1120 && md->mob_id != 1259 && md->mob_id != 1643 && md->mob_id != 1645 && md->mob_id != 1720 && 
        md->mob_id != 2198 && md->mob_id != 1640 && md->mob_id != 1090 && md->mob_id != 1289 && md->mob_id != 1262 && 
        md->mob_id != 1203 && md->mob_id != 1956 && md->mob_id != 1644 && md->mob_id != 1991 && md->mob_id != 1204 && 
        md->mob_id != 1089 && md->mob_id != 1092 && md->mob_id != 1765 && md->mob_id != 1642 && 
        !map_getmapflag(m, MF_NOSTAMINA) && !sd->sc.getSCE(SC_STAMINA) && battle_config.stamina_enable))
        continue;

    // 3. ตรวจสอบระบบ Disable Drop
    bool no_skip = true;
    bool is_mvp_reward = false;

    // ตรวจสอบว่าไอเทมชิ้นนี้อยู่ในลิสต์ MvpDrops ของมอนสเตอร์หรือไม่
    for (int j = 0; j < MAX_MVP_DROP; j++) {
        if (md->db->mvpitem[j].nameid == md->db->dropitem[i].nameid && md->db->mvpitem[j].nameid > 0) {
            is_mvp_reward = true;
            break;
        }
    }

    // ถ้าเป็นไอเทม MVP หรืออยู่ใน allow_drops ให้ถือว่า no_skip = true (ไม่โดนปิดดรอป)
    if (is_mvp_reward || util::vector_exists(allow_drops, md->db->dropitem[i].nameid)) {
        no_skip = true;
    } else {
        // ตรวจสอบการปิดดรอปตาม Config (มอนสเตอร์ปกติ/มินิ/บอส)
        if (battle_config.disable_mob_type) {
            if (md->get_bosstype() == BOSSTYPE_NONE && (battle_config.disable_mob_type & DIS_MOB_NORMAL))
                no_skip = false;
            else if (md->get_bosstype() == BOSSTYPE_MINIBOSS && (battle_config.disable_mob_type & DIS_MOB_MINI))
                no_skip = false;
            else if (md->get_bosstype() == BOSSTYPE_MVP && (battle_config.disable_mob_type & DIS_MOB_BOSS))
                no_skip = false;
        }

        // หากติดเงื่อนไขปิดดรอป ให้เช็คประเภทไอเทมเพื่อทำการข้าม (Skip)
        if (battle_config.disable_drop_type && !no_skip) {
            if (it->type == IT_HEALING && (battle_config.disable_drop_type & DIS_HEALING)) continue;
            if (it->type == IT_USABLE && (battle_config.disable_drop_type & DIS_USABLE)) continue;
            if ((it->type == IT_ETC || it->type == IT_AMMO) && (battle_config.disable_drop_type & DIS_ETC)) continue;
            if (it->type == IT_ARMOR && (battle_config.disable_drop_type & DIS_ARMOR)) continue;
            if (it->type == IT_WEAPON && (battle_config.disable_drop_type & DIS_WEAPON)) continue;
            if (it->type == IT_CARD && (battle_config.disable_drop_type & DIS_CARD)) continue;
        }
    } // <--- หลังปีกกานี้

    // ==========================================
    // [Custom] สวิตช์ปิดการดรอปไอเทมสัตว์เลี้ยง และ ของจับสัตว์
    // ==========================================
    if (battle_config.disable_pet_drops) {
        if (it->type == IT_PETEGG || it->type == IT_PETARMOR) {
            continue; 
        }
        if (blocked_tame_items.find(it->nameid) != blocked_tame_items.end()) {
            continue; 
        }
    }
    // ==========================================

    // 4. ลดอัตราดรอปกรณีใช้ Auto Attack (Config พิเศษ)
    if (battle_config.autoattack_reduce_droprate && mvp_sd && mvp_sd->sc.getSCE(SC_AUTOATTACK)) {
        bool reduce = false;
        if (it->type == IT_HEALING && (battle_config.autoattack_reduce_mode & AA_HEALING)) reduce = true;
        else if (it->type == IT_USABLE && (battle_config.autoattack_reduce_mode & AA_USABLE)) reduce = true;
        else if ((it->type == IT_ETC || it->type == IT_AMMO) && (battle_config.autoattack_reduce_mode & AA_ETC)) reduce = true;
        else if (it->type == IT_ARMOR && (battle_config.autoattack_reduce_mode & AA_ARMOR)) reduce = true;
        else if (it->type == IT_WEAPON && (battle_config.autoattack_reduce_mode & AA_WEAPON)) reduce = true;
        else if (it->type == IT_CARD && (battle_config.autoattack_reduce_mode & AA_CARD)) reduce = true;
    ...
    // ==========================================
    // [Custom Mod] Stealth Drop Economy System
    // ==========================================
    auto it_stealth = stealth_drop_rates.find(md->db->dropitem[i].nameid);
    if (it_stealth != stealth_drop_rates.end()) {
        drop_rate = (drop_rate * it_stealth->second) / 100;
    }

// --- Advanced Class Drop Penalty ---
		if (battle_config.penalty_drop_rate < 100 && sd) {
			int current_mob_type = 1;
			if (md->get_bosstype() == BOSSTYPE_MVP) current_mob_type = 3;
			else if (md->get_bosstype() == BOSSTYPE_MINIBOSS) current_mob_type = 2;

        if (battle_config.penalty_mob_type == 4 || battle_config.penalty_mob_type == current_mob_type) {
            int job_id = sd->status.class_;
            if (job_id >= 0 && job_id < 6000 && battle_config.penalty_job_list[job_id] == true) {
                drop_rate = (drop_rate * battle_config.penalty_drop_rate) / 100;
    ...
    // กรณีเป็นไข่สัตว์เลี้ยง
    if (mvp_sd && it->type == IT_PETEGG) {
        pet_create_egg(mvp_sd, md->db->dropitem[i].nameid);
        continue;
    }

    // 6. สร้างไอเทมและประมวลผลการได้รับ
    ditem = mob_setdropitem(&md->db->dropitem[i], 1, md->mob_id);
    apply_grade_item(ditem, md->db->dropitem[i].nameid);

    // ประกาศการดรอปไอเทมหายาก
    if (mvp_sd && md->db->dropitem[i].rate <= battle_config.rare_drop_announce) {
        char message[128];
        sprintf(message, msg_txt(NULL, 541), mvp_sd->status.name, md->name, it->ename.c_str(), (float)fake_drop_rate / 100);
        intif_broadcast(message, (int)strlen(message) + 1, BC_DEFAULT);
    }

    // ส่งไอเทมเข้าตัว (Autoloot) หรือลงพื้น
    mob_item_drop(md, dlist, ditem, 0, battle_config.autoloot_adjust ? drop_rate : md->db->dropitem[i].rate, homkillonly || merckillonly);
}

// --------------------------------------------------
		// Dynamic mob drops (runtime) - script controlled
		// - Behaves like "real mob drop" (Bubble Gum / bonuses apply)
		// - Uses the same drop_modifier as normal drops
		// --------------------------------------------------
		{
			auto itdyn = mob_dynamic_drop_db.find( md->mob_id );
			if( itdyn != mob_dynamic_drop_db.end() ){
				auto& vec = itdyn->second;
				// prune expired
				const t_tick now = gettick();
				vec.erase( std::remove_if( vec.begin(), vec.end(), [&]( const mob_dynamic_drop& d ){
					return ( d.expire_tick > 0 && now >= d.expire_tick );
				}), vec.end() );
				if( vec.empty() ){
					mob_dynamic_drop_db.erase( itdyn );
				}else{
					for( const auto& d : vec ){
						// --------------------------------------------------
						// STAMINA CHECK: require SC_STAMINA to allow dynamic drops
						// --------------------------------------------------
						if( battle_config.stamina_enable ){
							if( sd == nullptr || sd->sc.getSCE(SC_STAMINA) == nullptr )
								continue;
						}

						std::shared_ptr<item_data> it = item_db.find( d.nameid );
						if( it == nullptr )
							continue;
						
						// ==========================================
						// [Custom] สวิตช์ปิดการดรอปไอเทมสัตว์เลี้ยง (Dynamic)
						// ==========================================
						if (battle_config.disable_pet_drops) {
							if (it->type == IT_PETEGG || it->type == IT_PETARMOR) {
								continue;
						...
						// reuse normal drop calculator so Gum/bonuses apply
						drop_rate = mob_getdroprate( src, md->db, d.rate, drop_modifier, d.nameid, true, md );
						
						// ==========================================
						// [Item Daily] คำนวณ Bubble Gum เพิ่มเติมและห้ามเกิน 10000 (100%)
						// ==========================================
						if (sd && sd->sc.getSCE(SC_ITEMBOOST)) {
							drop_rate = drop_rate * (100 + sd->sc.getSCE(SC_ITEMBOOST)->val1) / 100;
						}
						...
						mobdrop.nameid = d.nameid;
						mobdrop.rate = drop_rate; 
						mobdrop.steal_protected = d.steal_protected;
						mobdrop.randomopt_group = d.randomopt_group;

						// ===================================================
						// 🔥 [Custom Hack] ระบบอ่าน Option จากไฟล์ .conf แยก (ฉบับรวบยอด คลีนสุดๆ)
						// ===================================================
						if (mobdrop.randomopt_group == 0 && it != nullptr) {
							for (const auto& rule : daily_option_rules) {
								if (it->type != rule.type) continue;
								if (rule.subtype_equip != 0) { 
									if (it->type == IT_WEAPON && it->subtype != rule.subtype_equip) continue;
									if (it->type == IT_ARMOR && !(it->equip & rule.subtype_equip)) continue;
								}
								if (it->elv < rule.min_level) continue;
								
								uint16 opt_id = 0;
								// ดึงชื่อกลุ่มจากไฟล์ .conf ตรงๆ (เช่น OP_Dagger) ไปค้นหาไอดีในระบบได้เลย
								random_option_group.option_get_id(rule.group_name.c_str(), opt_id);
								if (opt_id > 0) {
									mobdrop.randomopt_group = opt_id;
									break; // เจออันที่ตรงเงื่อนไขปุ๊บ ยัดออฟชั่นแล้วหยุดวนลูปทันที
								}
							}
						}
						// ===================================================
						...
						// ส่ง drop_rate เข้า mob_item_drop ด้วย
		...
		if (sd) {
			...
			for (const auto &globaldrop : global_drop_db) {
				// ตรวจสอบ MapLists ก่อน
				if (globaldrop.second->maps.size() > 0 && !util::vector_exists(globaldrop.second->maps, sd->bl.m)) {
					continue;  // ข้ามแผนที่ที่ไม่อยู่ใน MapLists
				}
		
				// ตรวจสอบ MobLists (ถ้ามี)
				if (globaldrop.second->mobs.size() > 0) {
					if (!util::vector_exists(globaldrop.second->mobs, md->mob_id)) {
						continue;  // ข้ามมอนสเตอร์ที่ไม่อยู่ใน MobLists
					}
				} else {
					// ถ้าไม่มี MobLists ให้ตรวจสอบ NoMobLists
					if (globaldrop.second->no_mobs.size() > 0 && util::vector_exists(globaldrop.second->no_mobs, md->mob_id)) {
						continue;  // ข้ามมอนสเตอร์ที่อยู่ใน NoMobLists
					}
				}
		
				// วนลูปตรวจสอบไอเท็มใน Global Drop
				for (const auto &globaldropitem : globaldrop.second->items) {
					int reduce_rate = 0;
		
					if (globaldrop.second->ReduceRateMaps.size()) {
						std::shared_ptr<s_global_drop_map> ReduceInfo = util::map_find(globaldrop.second->ReduceRateMaps, (uint16)sd->bl.m);
						if (ReduceInfo)
							reduce_rate = ReduceInfo->rate;
					}
		
					int final_rate = globaldropitem.second->rate;  // ใช้ globaldropitem ที่ประกาศในลูปนี้
					if (reduce_rate)
						final_rate = (int)(final_rate * (100 - reduce_rate) / 100.0);
		
					if (rnd() % 10000 <= final_rate) {
						if (globaldropitem.second->max_drop > 0) {
							int remain = is_reach_max_drop(globaldropitem.second->itemid, globaldropitem.second->max_drop);
							if (remain) {
								bool isvip = globaldropitem.second->is_vip == 1 ? true : false;
		
								if (isvip && !pc_isvip(sd))
									continue;
		
								bool isstamina = globaldropitem.second->is_stamina == 1 ? true : false;
		
								if (isstamina && !sd->sc.getSCE(SC_STAMINA))
									continue;
		
								if (globaldropitem.second->levelrange && abs((int)sd->status.base_level - (int)md->level) > globaldropitem.second->levelrange)
									continue;
		
								if (globaldropitem.second->is_autoloot) {
									struct item item_tmp = {};
									item_tmp.nameid = globaldropitem.second->itemid;
									item_tmp.refine = globaldropitem.second->refine;
									item_tmp.identify = globaldropitem.second->is_identify;
		
									if (globaldropitem.second->option_group)
										globaldropitem.second->option_group->apply(item_tmp);
		
									pc_additem(sd, &item_tmp, 1, LOG_TYPE_PICKDROP_MONSTER);
								} else {
									struct s_mob_drop mobdrop;
									memset(&mobdrop, 0, sizeof(struct s_mob_drop));
									mobdrop.nameid = globaldropitem.second->itemid;
									ditem = mob_setdropitem(&mobdrop, 1, md->mob_id);
									ditem->item_data.refine = globaldropitem.second->refine;
									ditem->item_data.identify = globaldropitem.second->is_identify;
		
									if (globaldropitem.second->option_group)
										globaldropitem.second->option_group->apply(ditem->item_data);
		
									mob_item_drop(md, dlist, ditem, 0, globaldropitem.second->rate, homkillonly || merckillonly);
								}
								if (globaldropitem.second->is_announce) {
									char message[CHAT_SIZE_MAX];
									std::shared_ptr<item_data> id = item_db.find(globaldropitem.second->itemid);
									sprintf(message, msg_txt(NULL, 1553), sd->status.name, item_db.create_item_link(id).c_str(), remain - 1);
									clif_broadcast(NULL, message, (int)strlen(message) + 1, BC_BLUE, ALL_CLIENT);
								}
								global_drop_insert_log(sd, md->mob_id, globaldropitem.second->itemid, globaldropitem.second->refine);
							}
						} else {
							bool isvip = globaldropitem.second->is_vip == 1 ? true : false;
						...
						if(globaldropitem.second->mob_level_min > 0 && md->level < globaldropitem.second->mob_level_min)
							continue;

						if(globaldropitem.second->mob_level_max > 0 && md->level > globaldropitem.second->mob_level_max)
							continue;
							...
							if (globaldropitem.second->is_announce) {
								char message[CHAT_SIZE_MAX];
								std::shared_ptr<item_data> id = item_db.find(globaldropitem.second->itemid);
								sprintf(message, msg_txt(NULL, 1552), sd->status.name, item_db.create_item_link(id).c_str());
								clif_broadcast(NULL, message, (int)strlen(message) + 1, BC_BLUE, ALL_CLIENT);
							}
						}
					}
				}  // ปิดลูป globaldropitem
			}  // ปิดลูป globaldrop
		}
...
// process script-granted zeny bonus (get_zeny_num) [Skotlex]
			if( sd->bonus.get_zeny_num && rnd()%100 < sd->bonus.get_zeny_rate ) {
				...
				// --- Custom: Spoof Monster Level for Item/Card Zeny Bonus ---
				int spoofed_mob_level = md->level;
				if (battle_config.mob_max_level_cap > 0 && spoofed_mob_level > battle_config.mob_max_level_cap) {
					spoofed_mob_level = battle_config.mob_max_level_cap;
				}
				// ------------------------------------------------------------
				
				i = sd->bonus.get_zeny_num > 0 ? sd->bonus.get_zeny_num : -spoofed_mob_level * sd->bonus.get_zeny_num;
				if (!i) i = 1;
	...
	if( mvp_sd && md->get_bosstype() == BOSSTYPE_MVP ){
		...
		if( !(map_getmapflag(m, MF_NOMVPLOOT) || type&1) ) {
			...
			for(i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				// --- เริ่มส่วน Disable Drop สำหรับ MVP ---
				bool no_skip = true;

				// ตรวจสอบว่าระบบตั้งค่าให้ปิดดรอปบอส MVP หรือไม่ (DIS_MOB_BOSS = 4)
				if(battle_config.disable_mob_type) {
					if(md->get_bosstype() == BOSSTYPE_MVP && (battle_config.disable_mob_type & DIS_MOB_BOSS))
						no_skip = false;
				}

				// ถ้าไอเทมอยู่ใน Allow Drop List (ข้อยกเว้น) ให้ดรอปได้เสมอ
				if(util::vector_exists(allow_drops, mdrop[i].nameid))
					no_skip = true;			

// กรองประเภทไอเทมเพื่อปิดดรอป
				if(battle_config.disable_drop_type && !no_skip) {
					if(i_data->type==IT_HEALING && (battle_config.disable_drop_type & DIS_HEALING)) continue;
					if(i_data->type==IT_USABLE && (battle_config.disable_drop_type & DIS_USABLE)) continue;
					if((i_data->type==IT_ETC || i_data->type==IT_AMMO) && (battle_config.disable_drop_type & DIS_ETC)) continue;
					if(i_data->type==IT_ARMOR && (battle_config.disable_drop_type & DIS_ARMOR)) continue;
					if(i_data->type==IT_WEAPON && (battle_config.disable_drop_type & DIS_WEAPON)) continue;
					if(i_data->type==IT_CARD && (battle_config.disable_drop_type & DIS_CARD)) continue;
				}
				// --- จบส่วน Disable Drop สำหรับ MVP ---
...
// --- Advanced Class Drop Penalty (MVP) ---
				if (battle_config.penalty_drop_rate < 100 && mvp_sd) {
					int current_mob_type = 3; // MVP
					if (battle_config.penalty_mob_type == 4 || battle_config.penalty_mob_type == current_mob_type) {
						int job_id = mvp_sd->status.class_;
						if (job_id >= 0 && job_id < 6000 && battle_config.penalty_job_list[job_id] == true) {
							temp = (temp * battle_config.penalty_drop_rate) / 100;
							if (temp < 1) temp = 0;
						}
					}
				}
				...
				// เอาไอเทมเข้าตัวผู้เล่น (ถ้าของเต็ม/น้ำหนักเกิน จะหล่นลงพื้น)
				...
				// เก็บประวัติการได้ไอเทม MVP
				log_pick_mob(md, LOG_TYPE_MVP, -1, &item);
				
				// ถ้าแจกของสำเร็จแล้ว 1 ชิ้น และไม่ได้ตั้งค่าโหมดแจกทุกชิ้นไว้ ให้หยุดลูปแจกของทันที
...
int mobskill_use(struct mob_data *md, t_tick tick, int event, int64 damage)
{
	...
	// ===================================================
	// 🔥 1. ปิดระบบดึงสกิลจาก Database ของร่างโคลน 100% (ป้องกันสกิลมั่ว)
	// ===================================================
	if (md->npc_event[0] != '\0' && strstr(md->npc_event, "OnFieldDead")) {
		return 0; 
	}
	...
	// ===================================================
	// 🔥 [Custom] ปลดล็อคให้โคลน (OnFieldDead) ใช้สกิลได้ แม้เซิร์ฟเวอร์จะปิด mob_skill_rate
	// ===================================================
	bool is_clone = (md->npc_event[0] != '\0' && strstr(md->npc_event, "OnFieldDead"));
	
	if ( (!battle_config.mob_skill_rate && !is_clone) || md->ud.skilltimer != INVALID_TIMER || (ms.empty() && !is_clone) || status_has_mode(&md->status,MD_NOCAST) )
		return 0;
	// ===================================================
...
void mob_db_load(bool is_reload){
	zeny_bounty_init(); // <-- เพิ่มบรรทัดนี้ลงไป
	item_daily_option_init();
	tame_blacklist_init(); // <--- เพิ่มบรรทัดนี้ลงไป
	if( !is_reload ) {
...
// ฟังก์ชันสำหรับส่งข้อมูล Dynamic Drop ให้ระบบอื่น (เช่น @mi) นำไปแสดงผล
const std::vector<mob_dynamic_drop>* mob_get_dynamic_drops(uint16 mob_id) {
	auto it = mob_dynamic_drop_db.find(mob_id);
	if (it != mob_dynamic_drop_db.end()) {
		return &it->second;
	}
...
// =========================================================================
// Custom Zeny Bounty - Get Config Values for @mi
// =========================================================================
int mob_get_zeny_drop_chance(void) {
	return zeny_conf.drop_chance; // ดึงจากตัวแปรที่ระบบโหลดไว้แล้วโดยตรง
}

int mob_get_zeny_autoattack_penalty(void) {
	return zeny_conf.aa_chance_penalty; // ดึงจากตัวแปรระบบโดยตรง
}
