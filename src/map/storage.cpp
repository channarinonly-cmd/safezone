...
static int storage_additem(map_session_data* sd, struct s_storage *stor, struct item *it, int amount)
{
	...
	if(sd->state.collection_open){
		if(!item_is_collection(it->nameid)){
			sprintf(output_msg,msg_txt(sd,1541),item_db.create_item_link(cdata).c_str());
			...
			sprintf(output_msg,msg_txt(sd,1544),item_db.create_item_link(cdata).c_str());
			clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output_msg, false, SELF);
			return 1;
			...
			sprintf(output_msg,msg_txt(sd,1545),item_db.create_item_link(cdata).c_str(), battle_config.collection_max_stored);
			clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output_msg, false, SELF);
			return 1;				
		}
	}
		...
		for( i = 0; i < stor->max_amount; i++ ) {
			if( compare_item(&stor->u.items_storage[i], it) ) { // existing items found, stack them
				if( amount > MAX_AMOUNT - stor->u.items_storage[i].amount || ( data->stack.storage && amount > data->stack.amount - stor->u.items_storage[i].amount ) )
					...
					if(sd->state.collection_open){
						sprintf(output_msg,msg_txt(sd,1546),item_db.create_item_link(cdata).c_str(), amount);
						clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output_msg, false, SELF);
					}
	...
	if(sd->state.collection_open){
		if(item_is_collection_stack(it->nameid)){
			...
			// success add item to collection
			sprintf(output_msg,msg_txt(sd,1542),item_db.create_item_link(cdata).c_str());
			clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output_msg, false, SELF);
		}
	}
...
void storage_storageget(map_session_data *sd, struct s_storage *stor, int index, int amount)
{
	...
	if(sd->state.collection_open){
		...
		if(amount > 1)
			sprintf(output_msg,msg_txt(sd,1547),item_db.create_item_link(item_data).c_str(), amount);
		else
			sprintf(output_msg,msg_txt(sd,1543),item_db.create_item_link(item_data).c_str());
