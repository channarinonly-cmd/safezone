...
/*=========================================
 * Check charbonus (@charbonus)
 *-----------------------------------------*/
ACMD_FUNC(charbonus)
{
    nullpo_retr(-1, sd);

    clif_displaymessage(fd, "===== Char Bonus (current char / latest only) =====");

    // ---------- helpers (เฉพาะในฟังก์ชันนี้ เพื่อกันชื่อซ้ำ) ----------
    // จับ "สายอาชีพ" ให้ Job: Swordman แมตช์ Knight/Crusader/... เป็นอาทิ
    auto same_family = [&](int16 classid, int16 base_job)->bool {
        int mapid = pc_jobid2mapid(classid);
        switch (base_job) {
            case JOB_SWORDMAN: return mapid == MAPID_SWORDMAN;
            case JOB_MAGE:     return mapid == MAPID_MAGE;
            case JOB_ARCHER:   return mapid == MAPID_ARCHER;
            case JOB_ACOLYTE:  return mapid == MAPID_ACOLYTE;
            case JOB_MERCHANT: return mapid == MAPID_MERCHANT;
            case JOB_THIEF:    return mapid == MAPID_THIEF;
            default:           return classid == base_job; // ถ้า YAML ใส่คลาสย่อยมา
        }
    };

    auto eff_level_for_row = [&](const s_char_bonus* row)->int {
        // ใช้เฉพาะ "ตัวที่เล่นอยู่" (current char)
        if (!row) return -1;
        if (row->jobid == JOB_ALL)
            return (int)sd->status.base_level;
        if (same_family(sd->status.class_, row->jobid))
            return (int)sd->status.base_level;
        return -1;
    };

    auto extract_key = [&](const std::string& script)->std::string {
        // ดึง key ประเภทโบนัสจาก "bonus <KEY>,..."
        size_t p = script.find("bonus");
        ...
        while (p < script.size()){
            char c = script[p];
            if ((c>='A'&&c<='Z')||(c>='a'&&c<='z')||(c>='0'&&c<='9')||c=='_'){ ++p; continue; }
            break;
        }
        if (p == s) return {};
        return script.substr(s, p-s); // เช่น bWeightLimit
    };

    auto split_lines = [&](const std::string& script)->std::vector<std::string> {
        // แตกสคริปต์เป็นบรรทัด ๆ (จบด้วย ';')
    ...
    auto job_label = [&](int16 jobid)->std::string {
        if (jobid == JOB_ALL) return std::string("All");
        // พยายามเดาชื่อจาก DB ถ้ามีเก็บไว้; ถ้าไม่มีก็โชว์เป็นหมายเลข
        for (const auto &kvx : char_bonus_db){
            const s_char_bonus* r = kvx.second.get();
            if (r && r->jobid == jobid && !r->job_name.empty())
                return r->job_name;
        }
        return std::string("JobID ") + std::to_string((int)jobid);
    };
    // -------------------------------------------------------------------

    // เลือก “รายการล่าสุด” ต่อประเภทโบนัส ต่ออาชีพใน Account ที่ทำให้ตัวปัจจุบันได้บัฟ
    struct Selected { const s_char_bonus* row; int eff_lv; };
    std::map<std::string, Selected> best_by_job_and_key; // key = "<jobid>|<bonus_key>"

    for (const auto &kv : char_bonus_db){
        const s_char_bonus* row = kv.second.get();
        if (!row || !row->script) continue;

        int eff = eff_level_for_row(row);
        if (eff < 0 || eff < row->level) continue; // ไม่ถึงเงื่อนไข

        std::string bonus_key = extract_key(row->script_text);
        if (bonus_key.empty())
            bonus_key = std::string("RAW_") + std::to_string((unsigned)kv.first); // กันเคสสคริปต์แปลก

        std::string gk = std::to_string((int)row->jobid) + "|" + bonus_key;
        auto it = best_by_job_and_key.find(gk);
        if (it == best_by_job_and_key.end() || row->level > it->second.row->level)
            best_by_job_and_key[gk] = {row, eff}; // เก็บ LevelNeed สูงสุดที่ ? eff
    }

    // พิมพ์บล็อกบน + เก็บรวมสำหรับ total
    int shown = 0;
    std::vector<std::string> total_lines;
    for (const auto &pair : best_by_job_and_key){
        const s_char_bonus* row = pair.second.row;
        int eff_lv = pair.second.eff_lv;

        char hdr[256];
        safesnprintf(hdr, sizeof(hdr),
            "• คุณมี[%s] Lv %d (You: %d) อยู่ในAccount ได้รับโบนัส",
            row->jobid == JOB_ALL ? "All" : (row->job_name.empty() ? "Job" : row->job_name.c_str()),
            (int)row->level, eff_lv);
        clif_displaymessage(fd, hdr);

        for (const auto& ln : split_lines(row->script_text)){
            std::string out = "  - " + ln;
            clif_displaymessage(fd, out.c_str());
            total_lines.push_back(ln);
        }
        shown++;
    }

    if (!shown)
        clif_displaymessage(fd, "  (ไม่มีรายการที่เข้าเงื่อนไข)");

    // ===== แสดงรายตัวละครในแอคเคานต์ (รวมตัวที่ออนไลน์) =====
    clif_displaymessage(fd, "");
    clif_displaymessage(fd, "===== Char Bonus by account characters =====");

    for (const auto &c : sd->char_bonus) {
        const bool is_current = (c.charid == sd->status.char_id);

        // เลือก “ล่าสุด” ต่อประเภทโบนัสสำหรับตัวละครนี้
        std::map<std::string, const s_char_bonus*> per_best; // key = bonus_key -> row*
        for (const auto &kv : char_bonus_db) {
            const s_char_bonus* row = kv.second.get();
            if (!row || !row->script) continue;
            ...
            int eff = -1;
            if (row->jobid == JOB_ALL) {
                if (!is_current) continue; // JOB_ALL อิงเลเวลตัวที่ล็อกอินอยู่เท่านั้น
                eff = (int)sd->status.base_level;
            } else if (same_family(c.jobid, row->jobid)) {
                eff = (int)c.level; // ใช้เลเวลของตัวละครนี้ (จับตามสายอาชีพ)
            } else {
                continue;
            }

            if (eff < row->level) continue; // ยังไม่ถึง LevelNeed

            std::string key = extract_key(row->script_text);
            if (key.empty())
                key = std::string("RAW_") + std::to_string((unsigned)kv.first);

            auto it = per_best.find(key);
            if (it == per_best.end() || row->level > it->second->level)
                per_best[key] = row; // เก็บรายการที่ LevelNeed สูงสุดที่ ? eff
        }

        // พิมพ์หัว + รายการ
        std::string jl = job_label(c.jobid);
        if (per_best.empty()) {
            char hdr[256];
            safesnprintf(hdr, sizeof(hdr),
                "• [%s]%s Char Lv %d  ยังไม่ถึงเงื่อนไขใด",
                jl.c_str(), is_current ? " (online)" : "", (int)c.level);
            clif_displaymessage(fd, hdr);
        } else {
            int max_need = 0;
            for (const auto &pp : per_best)
                if (pp.second && pp.second->level > max_need) max_need = pp.second->level;

            char hdr[256];
            safesnprintf(hdr, sizeof(hdr),
                "• [%s]%s Char Lv %d  ใช้เงื่อนไข Lv %d ได้รับโบนัส",
                jl.c_str(), is_current ? " (online)" : "", (int)c.level, max_need);
            clif_displaymessage(fd, hdr);

            for (const auto &pp : per_best) {
                const s_char_bonus* row = pp.second;
                if (!row) continue;
                for (const auto &ln : split_lines(row->script_text)) {
                    std::string out = "  - " + ln;
                    clif_displaymessage(fd, out.c_str());
                    total_lines.push_back(ln); // รวมเข้ากอง total
                }
            }
        }
    }

    // ===== เติม "คอมโบ" ให้เข้ารวมด้วย (ถ้าเข้าเงื่อนไขทั้งไอดี) =====
    for (const auto &kv2 : char_bonus_combo_db) {
        const auto &combo = *kv2.second;
        if (!combo.script) continue;

        bool ok = true;
        for (int jid : combo.jobs) {
            bool found = false;
            for (const auto &c : sd->char_bonus) {
                if (same_family(c.jobid, (int16)jid) && c.level >= combo.level) { found = true; break; }
            }
            if (!found) { ok = false; break; }
        }
        if (!ok) continue;

        // ทำชื่อคอมโบจากรายชื่ออาชีพ
        std::string jobs;
        for (size_t i = 0; i < combo.jobs.size(); ++i) {
            if (i) jobs += " + ";
            jobs += job_label((int16)combo.jobs[i]);
        }

        char hdr2[256];
        safesnprintf(hdr2, sizeof(hdr2), "• คุณมี[Combo: %s] Lv %d  อยู่ในAccount ได้รับโบนัส", jobs.c_str(), (int)combo.level);
        clif_displaymessage(fd, hdr2);

        for (const auto &ln : split_lines(combo.script_text)) {
            std::string out = "  - " + ln;
            clif_displaymessage(fd, out.c_str());
            total_lines.push_back(ln); // รวมเข้ากอง total
        }
    ...
    // บล็อกสรุปทั้งหมด
    clif_displaymessage(fd, "");
    clif_displaymessage(fd, "===== Char Bonus total =====");
    clif_displaymessage(fd, "รวมโบนัสที่ได้ทั้งหมด");

    std::set<std::string> seen;
    for (const auto& ln : total_lines){
...
*-----------------------------------------*/
ACMD_FUNC(resist) {
	char resis_state[CHAT_SIZE_MAX]; // ? เปลี่ยนจาก 100 เป็น CHAT_SIZE_MAX
	...
	struct {
		...
		{ NULL, 0 },
		{ "   [ %d ] ต้านทานขนาด Small", 0 }, // 1
		{ "   [ %d ] ต้านทานขนาด  Medium", 0 },
		{ "   [ %d ] ต้านทานขนาด  Large", 0 },

		{ "   [ %d ] ต้านทานประเภทท Normal", 0 },
		{ "   [ %d ] ต้านทานประเภท Boss", 0 },
		{ "   [ %d ] Guardian class resist", 0 },
		{ "   [ %d ] Battlefield class resist", 0 }, // 7

		{ "   [ %d ] ต้านทานเผ่า Formless", 0 },
		{ "   [ %d ] ต้านทานเผ่า Undead", 0 },
		{ "   [ %d ] ต้านทานเผ่า Brute", 0 },
		{ "   [ %d ] ต้านทานเผ่า Plant", 0 },
		{ "   [ %d ] ต้านทานเผ่า Insect", 0 },
		{ "   [ %d ] ต้านทานเผ่า Fish", 0 },
		{ "   [ %d ] ต้านทานเผ่า Demon", 0 },
		{ "   [ %d ] ต้านทานเผ่า Demi-Human", 0 },
		{ "   [ %d ] ต้านทานเผ่า Angel", 0 },
		{ "   [ %d ] ต้านทานเผ่า Dragon", 0 },
		{ "   [ %d ] ต้านทานเผ่า Player", 0 }, // 18

		{ "   [ %d ] ต้านทานจาก Goblin", 0 },
		{ "   [ %d ] ต้านทานจาก  Kobold", 0 },
		{ "   [ %d ] ต้านทานจาก  Orc", 0 },
		{ "   [ %d ] ต้านทานจาก  Golem", 0 },
		{ "   [ %d ] ต้านทานจาก  Guardian", 0 },
		{ "   [ %d ] ต้านทานจาก  Ninja", 0 },
		{ "   [ %d ] ต้านทานจาก GVG", 0 },
		{ "   [ %d ] ต้านทานจาก  Battlefield", 0 },
		{ "   [ %d ] ต้านทานจาก  Treasure", 0 },
		{ "   [ %d ] ต้านทานจาก  Biolab", 0 },
		{ "   [ %d ] ต้านทานจาก  Manuk", 0 },
		{ "   [ %d ] ต้านทานจาก  Splendide", 0 },
		{ "   [ %d ] ต้านทานจาก  Scaraba", 0 }, // 31

		{ "   [ %d ] ต้านทานธาตุ Neutral ", 0 },
		{ "   [ %d ] ต้านทานธาตุ Water", 0 },
		{ "   [ %d ] ต้านทานธาตุ Earth", 0 },
		{ "   [ %d ] ต้านทานธาตุ Fire", 0 },
		{ "   [ %d ] ต้านทานธาตุ Wind", 0 },
		{ "   [ %d ] ต้านทานธาตุ Poison", 0 },
		{ "   [ %d ] ต้านทานธาตุ Holy", 0 },
		{ "   [ %d ] ต้านทานธาตุ Dark", 0 },
		{ "   [ %d ] ต้านทานธาตุ Ghost", 0 },
		{ "   [ %d ] ต้านทานธาตุ Undead", 0 }, // 41

		{ "   [ %d ] ป้องกัน Neutral element", 0 },
		{ "   [ %d ] ป้องกัน Water element", 0 },
		{ "   [ %d ] ป้องกัน Earth element", 0 },
		{ "   [ %d ] ป้องกัน Fire element", 0 },
		{ "   [ %d ] ป้องกัน Wind element", 0 },
		{ "   [ %d ] ป้องกัน Poison element", 0 },
		{ "   [ %d ] ป้องกัน Holy element", 0 },
		{ "   [ %d ] ป้องกัน Dark element", 0 },
		{ "   [ %d ] ป้องกัน Ghost element", 0 },
		{ "   [ %d ] ป้องกัน Undead element", 0 }, // 51

		{ "   [ %d ] ต้านทานการโจมตีประเภท Long range", 0 },
		{ "   [ %d ] ต้านทานการโจมตีประเภท Melee", 0 }, // 53
		{ "   [ %d ] ต้านทานอาวุธ", 0 },
		{ "   [ %d ] ต้านทานเวทมนตร์", 0 }, 
		{ "   [ %d ] ต้านทานอื่นๆ", 0 }, // 56
		{ NULL, 0 }
...
ACMD_FUNC(statsdamage) {
	char statsdamage_state[CHAT_SIZE_MAX]; // ? เปลี่ยนจาก 100 เป็น CHAT_SIZE_MAX
	char info[CHAT_SIZE_MAX];
	int j, count = 0;
	struct {
		const char* format;
...
// Addon Cell PVP
	if (sd->state.pvp || (map_getmapflag(sd->bl.m, MF_NOTELEPORT) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE))) {
		clif_displaymessage(fd, msg_txt(sd,248));	// You are not authorized to warp from your current map.
		return -1;
	}
...
ACMD_FUNC(baselevelup)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd, msg_txt(sd,986)); // usage
		return -1;
	}

	if (level > 0) {
		if (sd->status.base_level >= pc_maxbaselv(sd)) {
			clif_displaymessage(fd, msg_txt(sd,47)); // can't go higher
			return -1;
		}
		...
		clif_displaymessage(fd, msg_txt(sd,21)); // raised
	} else {
		if (sd->status.base_level == 1) {
			clif_displaymessage(fd, msg_txt(sd,158)); // can't go lower
			return -1;
		}
		level *= -1;
		...
		if (sd->status.status_point < status_point) sd->status.status_point = 0;
		else                                         sd->status.status_point -= status_point;

		if (sd->status.trait_point < trait_point) sd->status.trait_point = 0;
		else                                      sd->status.trait_point -= trait_point;

		sd->status.base_level -= (unsigned int)level;
		clif_displaymessage(fd, msg_txt(sd,22)); // lowered
...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		// ==========================================
		// โหลดไฟล์ตั้งค่า Advanced Status Reflect ใหม่
		battle_read_custom_yaml();
		// ==========================================
...
/*==========================================
 * Show Monster DB Info   v 1.0 (Final Faction Buff)
 *------------------------------------------*/
ACMD_FUNC(mobinfo)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, msg_txt(sd,1239)); // Please enter a monster name/ID
		return -1;
	}
	...
	if (count >= MAX_SEARCH) {
		sprintf(atcmd_output, msg_txt(sd,269), MAX_SEARCH); 
		clif_displaymessage(fd, atcmd_output);
		count = MAX_SEARCH;
	}
	...
	for (uint16 k = 0; k < count; k++) {
		...
		// ==========================================
		// [Faction War] เพิ่มโบนัส EXP ไปโชว์ที่หน้าต่าง @mi 
		// ==========================================
		if (sd->sc.getSCE(SC_FACTION_BUFF)) {
			int f_rate = sd->sc.getSCE(SC_FACTION_BUFF)->val1; // ดึงค่า 10% จากสคริปต์
			if (f_rate <= 0) f_rate = 10; // กันบั๊ก
			base_exp += (base_exp * f_rate) / 100;
			job_exp += (job_exp * f_rate) / 100;
		}
		// ==========================================
		...
		if( mob->get_bosstype() == BOSSTYPE_MVP )
			sprintf(atcmd_output, msg_txt(sd,1240), mob->name.c_str(), mob->jname.c_str(), mob->sprite.c_str(), mob->id); 
		else
			sprintf(atcmd_output, msg_txt(sd,1241), mob->name.c_str(), mob->jname.c_str(), mob->sprite.c_str(), mob->id); 
		clif_displaymessage(fd, atcmd_output);
		
		// --- [เพิ่มโค้ดหลอกตา @mi ให้โชว์เลเวลตาม Config] ---
		int display_lv = mob->lv;
		if (battle_config.mob_max_level_cap > 0 && display_lv > battle_config.mob_max_level_cap) {
			display_lv = battle_config.mob_max_level_cap;
		}
		// --------------------------------------------------
		
		// เปลี่ยนจาก mob->lv เป็น display_lv ที่บรรทัดด้านล่างนี้
		sprintf(atcmd_output, msg_txt(sd,1242), display_lv, mob->status.max_hp, base_exp, job_exp, MOB_HIT(mob), MOB_FLEE(mob)); 
		clif_displaymessage(fd, atcmd_output);
		
		sprintf(atcmd_output, msg_txt(sd,1243), mob->status.def, mob->status.mdef,mob->status.str, mob->status.agi, mob->status.vit, mob->status.int_, mob->status.dex, mob->status.luk);
		clif_displaymessage(fd, atcmd_output);

		sprintf(atcmd_output, msg_txt(sd,1244), mob->status.batk + mob->status.rhw.atk, mob->status.batk + mob->status.rhw.atk2, mob->status.rhw.range, mob->range2 , mob->range3, msize[mob->status.size], mrace[mob->status.race], melement[mob->status.def_ele], mob->status.ele_lv);
		clif_displaymessage(fd, atcmd_output);
#ifdef RENEWAL
		sprintf(atcmd_output, msg_txt(sd, 827), mob->status.res, mob->status.mres);
		clif_displaymessage(fd, atcmd_output);
#endif
		// drops
		clif_displaymessage(fd, msg_txt(sd,1245)); 
		strcpy(atcmd_output, " ");
		unsigned int j = 0;
		...
		// ==========================================
		// [Normal Drops] ไอเทมทั่วไป (ดึงโบนัสอัตโนมัติจาก mob_getdroprate)
		// ==========================================
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if (mob->get_bosstype() != BOSSTYPE_MVP) {
				if (id->type == IT_WEAPON || id->type == IT_ARMOR) continue; 
			}

			// ==========================================
			// [Custom @mi Drop] คำนวณบัฟแยกตามประเภทไอเทม
			// ==========================================
			int current_drop_modifier = drop_modifier;

			if (sd) {
				// 1. บัฟจาก Faction War (บวกให้ทุกประเภทไอเทม)
				if (sd->sc.getSCE(SC_FACTION_BUFF)) {
					current_drop_modifier += sd->sc.getSCE(SC_FACTION_BUFF)->val1; 
				}

				// 2. บัฟจาก Bubble Gum (แยกตามประเภทไอเทม)
				if (id->type == IT_ETC && sd->sc.getSCE(SC_GUM_ETC)) {
					current_drop_modifier += sd->sc.getSCE(SC_GUM_ETC)->val1;
				} else if ((id->type == IT_USABLE || id->type == IT_HEALING) && sd->sc.getSCE(SC_GUM_USABLE)) {
					current_drop_modifier += sd->sc.getSCE(SC_GUM_USABLE)->val1;
				} else if ((id->type == IT_WEAPON || id->type == IT_ARMOR) && sd->sc.getSCE(SC_GUM_EQUIP)) {
					current_drop_modifier += sd->sc.getSCE(SC_GUM_EQUIP)->val1;
				}
			}
			// ==========================================

			int droprate = mob_getdroprate( &sd->bl, mob, mob->dropitem[i].rate, current_drop_modifier, mob->dropitem[i].nameid, false);

			sprintf(atcmd_output2, " - %s  %02.02f%%", item_db.create_item_link( id ).c_str(), (float)droprate / 100);
			strcat(atcmd_output, atcmd_output2);
			if (++j % 2 == 0) { // แสดงแค่บรรทัดละ 2 ชิ้น ข้อความจะไม่ล้น 256 ตัวอักษร
				clif_displaymessage(fd, atcmd_output);
				strcpy(atcmd_output, " ");
			}
		}
		// โค้ดเดิมคือ: else if (j % 3 != 0)
		if (j == 0)
			clif_displaymessage(fd, msg_txt(sd,1246));
		else if (j % 2 != 0) // ? แก้เลข 3 เป็นเลข 2 ตรงนี้ครับ
			clif_displaymessage(fd, atcmd_output);

		// ==========================================
		// [MVP Items] ไม่ถูกบวกโบนัส Faction แน่นอน
		// ==========================================
		if( mob->get_bosstype() == BOSSTYPE_MVP ){
			float mvppercent, mvpremain;
			sprintf(atcmd_output, msg_txt(sd,1247), mob->mexp); 
			clif_displaymessage(fd, atcmd_output);
			
			// ? แสดงหัวข้อ MVP Drops แล้วขึ้นบรรทัดใหม่ทันที
			strcpy(atcmd_output, msg_txt(sd,1248)); 
			clif_displaymessage(fd, atcmd_output);
			strcpy(atcmd_output, " "); // เคลียร์ข้อความเพื่อเตรียมรับไอเทม
			
			mvpremain = 100.0; 
			j = 0;
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				if (mob->mvpitem[i].nameid == 0) continue;
				...
				if (mvppercent > 0) {
					...
					// ? เพิ่มระบบตัดบรรทัด ป้องกันเซิร์ฟดับจากข้อความยาวเกิน
					if (j % 2 == 0) { 
						clif_displaymessage(fd, atcmd_output);
						strcpy(atcmd_output, " ");
					}
			...
			if (j == 0)
				clif_displaymessage(fd, msg_txt(sd,1249)); 
			else if (j % 2 != 0) // ? ดักจับชิ้นที่เหลือเศษ
				clif_displaymessage(fd, atcmd_output);
		}

		// ==========================================
		// [Item Daily] ไอเทมประจำวัน (ดึงโบนัสอัตโนมัติจาก mob_getdroprate)
		// ==========================================
		if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `item_id`, `rate` FROM `item_daily_drops` WHERE `mob_id` = %d", mob->id) ) {
			Sql_ShowDebug(mmysql_handle);
		} else if( Sql_NumRows(mmysql_handle) > 0 ) {
			clif_displaymessage(fd, msg_txt(sd, 439));
			
			while( SQL_SUCCESS == Sql_NextRow(mmysql_handle) ) {
        char* data = NULL;
        int daily_item = 0, daily_rate = 0;
        
        Sql_GetData(mmysql_handle, 0, &data, NULL);
        if (data != NULL) {
            daily_item = atoi(data);
        }
        
        Sql_GetData(mmysql_handle, 1, &data, NULL);
        if (data != NULL) {
            daily_rate = atoi(data);
        }
        
        std::shared_ptr<item_data> id = item_db.find(daily_item);
        if (id != nullptr) {
					...
					int final_display_rate = mob_getdroprate(&sd->bl, mob, daily_rate, current_drop_modifier, (t_itemid)daily_item, false);
					char msg[CHAT_SIZE_MAX];
					sprintf(msg, msg_txt(sd, 440), item_db.create_item_link(id).c_str(), daily_item, (float)final_display_rate / 100.0);
					clif_displaymessage(fd, msg);
				}
			}
		}
	...
	// ==========================================
	// [Zeny Drops] ไม่ถูกบวกโบนัส Faction แน่นอน
	// ==========================================
	if (sd != NULL) {
		int base_zeny_chance = mob_get_zeny_drop_chance() * 100; 
		int final_zeny_chance = base_zeny_chance;

		if (sd->sc.getSCE(SC_AUTOATTACK)) {
			int auto_penalty = mob_get_zeny_autoattack_penalty(); 
			if (auto_penalty > 0) {
				final_zeny_chance = final_zeny_chance - (final_zeny_chance * auto_penalty / 100);
			}
		}

		if (sd->sc.getSCE(SC_ZENYGUM)) {
			int bonus_rate = sd->sc.getSCE(SC_ZENYGUM)->val1;
			final_zeny_chance = final_zeny_chance + (final_zeny_chance * bonus_rate / 100);
		}

		if (final_zeny_chance > 10000) final_zeny_chance = 10000;
		if (final_zeny_chance < 0) final_zeny_chance = 0;

		char zeny_msg[256];
		if (base_zeny_chance == final_zeny_chance) {
			sprintf(zeny_msg, " ~ โอกาสดรอป Zeny: %0.2f%%", (float)final_zeny_chance / 100);
		} else if (sd->sc.getSCE(SC_AUTOATTACK) && !sd->sc.getSCE(SC_ZENYGUM) && mob_get_zeny_autoattack_penalty() > 0) {
			sprintf(zeny_msg, " ~ โอกาสดรอป Zeny (โดนเนิร์ฟ Auto): %0.2f%%", (float)final_zeny_chance / 100);
		} else {
			sprintf(zeny_msg, " ~ โอกาสดรอป Zeny (สถานะปัจจุบัน): %0.2f%%", (float)final_zeny_chance / 100);
		}
		clif_displaymessage(fd, zeny_msg);
	}
...
// ==========================================
// [Item Daily / World Map] ระบบส่งออกข้อมูลแผนที่เกิดมอนสเตอร์
// ==========================================
ACMD_FUNC(exportmap)
{
	// สร้างตารางใหม่ที่มีคอลัมน์ mob_name รองรับ
	if( SQL_ERROR == Sql_Query(mmysql_handle, "CREATE TABLE IF NOT EXISTS `mob_spawn_data` (`mob_id` INT NOT NULL, `mob_name` VARCHAR(50) NOT NULL, `spawn_string` TEXT NOT NULL, PRIMARY KEY (`mob_id`))") ) {
		Sql_ShowDebug(mmysql_handle);
		return -1;
	}
	Sql_Query(mmysql_handle, "TRUNCATE TABLE `mob_spawn_data`");

	int count = 0;
	// วนลูปเช็คมอนสเตอร์ ID 1000 ถึง 4000
	for (int i = 1000; i <= 4000; i++) {
		std::shared_ptr<s_mob_db> mob = mob_db.find(i);
		if (mob == nullptr) continue;

		const std::vector<spawn_info> spawns = mob_get_spawns(i);
		if (spawns.size() > 0) {
			std::string output_str = ""; // ใช้ std::string ป้องกัน Buffer Overflow อัตโนมัติ
			
			for(auto& spawn : spawns) {
				int16 mapid = map_mapindex2mapid(spawn.mapindex);
				if (mapid < 0) continue;
				char temp[128];
				// รูปแบบการเก็บใหม่: map_name:amount|map_name2:amount2|
				snprintf(temp, sizeof(temp), "%s:%d|", map_getmapdata(mapid)->name, spawn.qty);
				output_str += temp; // ใช้ += ต่อข้อความได้อย่างปลอดภัย
			}
			
			if (output_str.length() > 0) {
				// สร้าง Buffer สำหรับ Escape String (ขนาดที่ปลอดภัยคือ 2 เท่าของความยาวข้อความ + 1)
				std::vector<char> esc_output(output_str.length() * 2 + 1);
				char esc_name[128];
				
				Sql_EscapeString(mmysql_handle, esc_output.data(), output_str.c_str());
				Sql_EscapeString(mmysql_handle, esc_name, mob->jname.c_str());
				
				// ใช้ esc_output.data() ในการดึงค่ามา Query
				Sql_Query(mmysql_handle, "INSERT INTO `mob_spawn_data` (`mob_id`, `mob_name`, `spawn_string`) VALUES (%d, '%s', '%s')", i, esc_name, esc_output.data());
				count++;
			}
		}
	}
	clif_displaymessage(fd, "--- ^00FF00[ระบบ]^000000 ส่งออกข้อมูลจุดเกิดมอนสเตอร์สำเร็จ! ---");
	return 0;
}

// ==========================================
// [ระบบหน้าเว็บ] ส่งออกข้อมูล Drop มอนสเตอร์ (กรองอุปกรณ์ออกให้แล้ว)
// ==========================================
ACMD_FUNC(exportmobdrop)
{
	// สร้างตารางเก็บข้อมูล Drop
	if( SQL_ERROR == Sql_Query(mmysql_handle, "CREATE TABLE IF NOT EXISTS `server_mob_drops` (`mob_id` INT NOT NULL, `mob_name` VARCHAR(50), `item_id` INT, `item_name` VARCHAR(50), `drop_rate` INT, `is_mvp` INT)") ) {
		Sql_ShowDebug(mmysql_handle);
		return -1;
	}
	Sql_Query(mmysql_handle, "TRUNCATE TABLE `server_mob_drops`");

	int count = 0;
	// วนลูปเช็คมอนสเตอร์ (ปกติรหัส 1000 - 4000)
	for (int i = 1000; i <= 4000; i++) {
		std::shared_ptr<s_mob_db> mob = mob_db.find(i);
		if (mob == nullptr) continue;

		bool is_boss = (mob->get_bosstype() == BOSSTYPE_MVP);

		for (int j = 0; j < MAX_MOB_DROP_TOTAL; j++) {
			if (mob->dropitem[j].nameid == 0 || mob->dropitem[j].rate < 1) continue;

			std::shared_ptr<item_data> id = item_db.find(mob->dropitem[j].nameid);
			if (id == nullptr) continue;

			// กรองอาวุธและชุดเกราะออกจากมอนสเตอร์ทั่วไป (แบบเดียวกับที่เราทำใน @mi)
			if (!is_boss) {
			...
			char esc_mob[128], esc_item[128];
			Sql_EscapeString(mmysql_handle, esc_mob, mob->jname.c_str());
			Sql_EscapeString(mmysql_handle, esc_item, id->name.c_str());

			Sql_Query(mmysql_handle, "INSERT INTO `server_mob_drops` (`mob_id`, `mob_name`, `item_id`, `item_name`, `drop_rate`, `is_mvp`) VALUES (%d, '%s', %d, '%s', %d, %d)",
				mob->id, esc_mob, mob->dropitem[j].nameid, esc_item, mob->dropitem[j].rate, (is_boss ? 1 : 0));
			count++;
		}
	}
	clif_displaymessage(fd, "--- ^00FF00[ระบบ]^000000 ส่งออกข้อมูล Monster Drops ลง SQL สำเร็จ! ---");
	return 0;
}
...
ACMD_FUNC(afk) {
	...
	if (sd->state.autotrade) {
		clif_displaymessage(fd, "You are already in AFK mode.");
		return -1;
	}
	
	// เน€เธเธดเธ”เนเธเนเธเธฒเธ AFK Mode
	trade_tradeack(sd, 4);  // เธเนเธญเธเธเธฑเธเนเธกเนเนเธซเนเธชเธฒเธกเธฒเธฃเธ–เธเธทเนเธญเธเธฒเธขเนเธ”เน
	sd->state.autotrade = 1;  // เธ•เธฑเนเธเธเนเธฒเธชเธ–เธฒเธเธฐ AFK
	
	// เธ—เธณเนเธซเนเธเธนเนเน€เธฅเนเธเธญเธญเธเธเธฒเธเน€เธเธก เนเธ•เนเธขเธฑเธเธเธเธญเธญเธเนเธฅเธเนเนเธเน€เธเธดเธฃเนเธเน€เธงเธญเธฃเน
	clif_authfail_fd(fd, 15);  
	
	clif_displaymessage(fd, "You are now in AFK mode.");
	return 0;
}
...
* Compatible with latest rAthena
*------------------------------------------*/
ACMD_FUNC(whosell)
{
    char item_name[100];
    ...
    std::shared_ptr<item_data> item_ptr = nullptr;
    nullpo_retr(-1, sd);
    ...
    if (sscanf(message, "+%d %d[%d]", &refine, &item_id, &card_id) == 3) s_type = 1+2+4;
    else if (sscanf(message, "+%d %d", &refine, &item_id) == 2) s_type = 1+4;
    else if (sscanf(message, "+%d [%d]", &refine, &card_id) == 2) s_type = 2+4;
    else if (sscanf(message, "%d[%d]", &item_id, &card_id) == 2) s_type = 1+2;
    else if (sscanf(message, "[%d]", &card_id) == 1) s_type = 2;
    else if (sscanf(message, "+%d", &refine) == 1) s_type = 4;
    else if (sscanf(message, "%d", &item_id) == 1 && item_id == atoi(message)) s_type = 1;
    else if (sscanf(message, "%99[^\n]", item_name) == 1) {
        s_type = 1;
        item_ptr = item_db.searchname(item_name);
        if (!item_ptr) {
            clif_displaymessage(fd, "Not found item with this name");
            return -1;
        }
        item_id = item_ptr->nameid;
    }
    else {
        clif_displaymessage(fd, "Use: @whosell (<+refine> )(<item_id>)(<[card_id]>) or @whosell <name>");
        return -1;
    ...
    // check card
    if (s_type & 2) {
        auto card_ptr = item_db.find(card_id);
        if (!card_ptr || card_ptr->type != IT_CARD) {
    ...
    // check item
    if (s_type & 1) {
        auto it_ptr = item_db.find(item_id);
        if (!it_ptr) {
    ...
    for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter)) {
        ...
        for (j = 0; j < pl_sd->vend_num; j++) {
            auto vend_item_ptr = item_db.find(pl_sd->cart.u.items_cart[pl_sd->vending[j].index].nameid);
            if (!vend_item_ptr) continue;

            if (s_type & 1 && pl_sd->cart.u.items_cart[pl_sd->vending[j].index].nameid != item_id) continue;
            if (s_type & 2 && ((vend_item_ptr->type != IT_ARMOR && vend_item_ptr->type != IT_WEAPON) ||
                ...
                continue;
            if (s_type & 4 && ((vend_item_ptr->type != IT_ARMOR && vend_item_ptr->type != IT_WEAPON) ||
                pl_sd->cart.u.items_cart[pl_sd->vending[j].index].refine != refine))
                continue;

            char atcmd_output[CHAT_SIZE_MAX];
            if (vend_item_ptr->type == IT_ARMOR)
                snprintf(atcmd_output, CHAT_SIZE_MAX, "+%d %d[%d] | Price %d | Amount %d | Map %s (%d,%d) | Seller %s",
                    ...
                    pl_sd->status.name);
            else if (vend_item_ptr->type == IT_WEAPON)
                snprintf(atcmd_output, CHAT_SIZE_MAX, "+%d %d[%d,%d,%d,%d] | Price %d | Amount %d | Map %s (%d,%d) | Seller %s",
                    pl_sd->cart.u.items_cart[pl_sd->vending[j].index].refine,
                    pl_sd->cart.u.items_cart[pl_sd->vending[j].index].nameid,
                    pl_sd->cart.u.items_cart[pl_sd->vending[j].index].card[0],
                ...
                snprintf(atcmd_output, CHAT_SIZE_MAX, "ID %d | Price %d | Amount %d | Map %s (%d,%d) | Seller %s",
                    pl_sd->cart.u.items_cart[pl_sd->vending[j].index].nameid,
                    pl_sd->vending[j].value,
                    pl_sd->vending[j].amount,
                    mapindex_id2name(pl_sd->mapindex),
    ...
    if (count > 0) {
        char atcmd_output[CHAT_SIZE_MAX];
        snprintf(atcmd_output, CHAT_SIZE_MAX, "Found %d ea.", count);
        clif_displaymessage(fd, atcmd_output);
    } else
...
ACMD_FUNC(voiceblockers) {
	...
	int rc;
	if (pl_sd) {
		rc = Sql_Query(mmysql_handle, "SELECT COUNT(*) FROM `voice_blocks` WHERE `blocked_account_id`=%d", pl_sd->status.account_id);
	} else {
		char esc[NAME_LENGTH * 2 + 1];
		Sql_EscapeString(mmysql_handle, esc, player_name);
		rc = Sql_Query(mmysql_handle, "SELECT COUNT(*) FROM `voice_blocks` WHERE `blocked_name`='%s'", esc);
	}
	if (rc == SQL_ERROR) {
...
ACMD_FUNC(voiceblocks) {
	...
	if (Sql_Query(mmysql_handle, "SELECT `blocked_name` FROM `voice_blocks` WHERE `blocker_account_id`=%d ORDER BY `created_at`", pl_sd->status.account_id) == SQL_ERROR) {
		clif_displaymessage(fd, "Voice block table unavailable (voice server not initialised?).");
		return -1;
	}
	char output[160];
	snprintf(output, sizeof(output), "'%s' has voice-blocked %llu player(s):", pl_sd->status.name, (unsigned long long)Sql_NumRows(mmysql_handle));
	clif_displaymessage(fd, output);
	int shown = 0;
