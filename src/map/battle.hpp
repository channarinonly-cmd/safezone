...
// ==========================================
struct s_class_penalty_yaml {
	int job_id;
	int sc_id;
	int skill_id; // เพิ่มคอลัมน์สกิล
	float rate;  // <--- เปลี่ยนตรงนี้เป็น float
	int type;
};
extern std::vector<s_class_penalty_yaml> class_penalty_db;
...
// eAthena additions
	...
	// ===================================
	// Advanced Class Penalty System
	// ===================================
	int penalty_mob_type;
	int penalty_drop_rate;
	int minion_hate_range;
	bool penalty_job_list[6000];
	...
	//Block IP AND Prevent Change IP
	int ip_block_limit;
	int prevent_change_ip;
	char roplay_ip_whitelist[512];

	//[KIRA] PK PASS
	int pk_pass;
	int	pk_pass_attack_time;
	int	pk_pass_protect_time;
	int	pk_pass_attack_effect;
	int	pk_pass_protect_effect;
	...
	/* HNOMKENG Item Type */
	int itemtype_healing;
	int itemtype_usable;
	int itemtype_etc;
	int itemtype_armor;
	int itemtype_weapon;
	int itemtype_card;
	// Card insertion (CZ_REQ_ITEMCOMPOSITION) per equip slot
	int card_insert_allow_armor;
	int card_insert_allow_head_top;
	int card_insert_allow_hand_l;
	int card_insert_allow_shoes;
	int card_insert_allow_garment;
	int card_insert_allow_hand_r;
	int card_insert_allow_acc_l;
	int card_insert_allow_acc_r;


	int itemtype_petegg;
	int itemtype_petarmor;
	int itemtype_ammo;
	int itemtype_delayconsume;
	int itemtype_shadowgear;
	int itemtype_cash;

	int cashpoint_nameid;
	int buy_vat;
	int stall_cash_to_zeny_tax_rate; // <--- เพิ่มบรรทัดนี้ลงไปครับ
	...
	int disable_drop_type;
	int disable_mob_type;
	...
	int autoattack_max_moblist;
	int autoattack_max_itemlist;
	int autoattack_steal_reduce_rate;
	int autoattack_max_time_per_mob;
	int autoattack_mob_struck;
	int autoattack_quiver_check;
	int autoattack_max_flee_mob;
	int autoattack_gunslinger_conflip_fix;
	int autoattack_coinflip_coin;
	int char_bonus_debug;
	int magic_critical;         	
 	int magic_critical_option; 
	int magic_critical_flag; 
	
	int sm_per_limit;    // โควตาต่อคน
	int sm_server_limit; // โควตารวมทั้งเซิร์ฟเวอร์
	int mob_max_level_cap; // Custom: Limit Max Monster Level
	// [Custom] ปิดดรอปไอเทมสัตว์เลี้ยง
	int disable_pet_drops;
