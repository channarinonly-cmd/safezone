...
// ==========================================
	...
	if (!aa_unit_autoattack && speed > 0 && ud->target_to && !ud->stepaction) {
		...
		if (tbl && tbl->m == bl->m && check_distance_bl(bl, tbl, ud->chaserange)) {
			speed = -1; // บังคับให้ speed เป็น -1 เพื่อเข้าสู่เงื่อนไขการโจมตีด้านล่างทันที
			ud->walkpath.path_len = ud->walkpath.path_pos; // ตัดจบเส้นทางการเดินที่เหลือทิ้ง
		}
	}
	// ==========================================
...
int unit_walktoxy( struct block_list *bl, short x, short y, unsigned char flag)
{
	...
	if(!(flag&2) && (!status_bl_has_mode(bl,MD_CANMOVE) || !unit_can_move(bl)))
		return 0;
// Addon Cell PVP (Fixed Cooldown Bug)
	if (bl->type == BL_PC)
	{
		...
		if (sd && sd->pvpcan_walkout_tick && !map_getcell(sd->bl.m, x, y, CELL_CHKPVP)) {
			int32 diff = DIFF_TICK(tick, sd->pvpcan_walkout_tick);

			// [ดักบัค] ถ้าเวลาห่างกันเกิน 10 นาที หรือติดลบแปลกๆ แปลว่าตัวแปรเอ๋อ ให้ล้างค่าทิ้งเลย
			if (diff < -5000 || diff > 600000) {
				sd->pvpcan_walkout_tick = 0;
			}
			// ถ้ายังอยู่ในช่วงคูลดาวน์จริงๆ (ไม่อนุญาตให้เดิน)
			else if (diff < battle_config.cellpvp_walkout_delay) {
				int e_tick = (battle_config.cellpvp_walkout_delay - diff) / 1000;
				char e_msg[150];
				
				if (e_tick < 0) e_tick = 0; // กันตัวเลขติดลบ

				if (e_tick > 99)
					sprintf(e_msg, msg_txt(sd, 1711), sd->status.name, (double)e_tick / 60.0);
				else
					sprintf(e_msg, msg_txt(sd, 1712), sd->status.name, e_tick + 1);

				clif_displaymessage(sd->fd, e_msg);
				return 0;
			} 
			// ถ้าหมดเวลาคูลดาวน์แล้ว ให้ล้างค่าทิ้ง
			else {
				sd->pvpcan_walkout_tick = 0;
			}
		}
...
bool unit_movepos(struct block_list *bl, short dst_x, short dst_y, int easy, bool checkpath)
{
	...
	if(sd) {
		...
		// Addon Cell PVP: จับการเคลื่อนที่แบบพุ่ง/สไลด์เข้าโซน
		if( !sd->state.pvp && map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) )
			map_pvp_area(sd, 1);
		else if( sd->state.pvp && !map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) )
			map_pvp_area(sd, 0);
		...
		case BL_PC: {
			...
			if(sd->shadowform_id) { // If shadow target has leave the map
				...
				if(battle_config.autoattack_clean_logout)
					status_change_end(d_bl,SC_AUTOATTACK);
			}
			...
			// --- แก้ไขจุดนี้: ย้ายโค้ดมาไว้เหนือ break; ---
			if( sd->state.pvp )
				map_pvp_area(sd, 0);

			break; 
		} // ปิดปีกกาของ case BL_PC
		case BL_MOB: {
// ...
