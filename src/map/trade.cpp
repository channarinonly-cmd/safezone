...
void trade_tradeadditem(map_session_data *sd, short index, short amount)
{
	...
	struct item_data* item_type = sd->inventory_data[index];

	if(item_type->type == IT_HEALING && battle_config.itemtype_healing) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_USABLE && battle_config.itemtype_usable) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_ETC && battle_config.itemtype_etc) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_ARMOR && battle_config.itemtype_armor) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_WEAPON && battle_config.itemtype_weapon) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_CARD && battle_config.itemtype_card) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_PETEGG && battle_config.itemtype_petegg) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_PETARMOR && battle_config.itemtype_petarmor) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_AMMO && battle_config.itemtype_ammo) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_DELAYCONSUME && battle_config.itemtype_delayconsume) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_SHADOWGEAR && battle_config.itemtype_shadowgear) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	} else if(item_type->type == IT_CASH && battle_config.itemtype_cash) {
		clif_displaymessage (sd->fd, msg_txt(sd,260));
		clif_tradeitemok(*sd, index, EXITEM_ADD_FAILED_CLOSED);
		return;
	}
...
void trade_tradecommit(map_session_data *sd)
{
	...
	int is_town = 0;
	if (Sql_Query(mmysql_handle, "SELECT `tax_rate` FROM `mayor_towns` WHERE `town_map` = '%s' AND `is_active` = 1", map_mapid2mapname(map_id)) == SQL_SUCCESS) {
...
// ==========================================
    // ผู้เล่นคนที่ 1 (sd) จ่ายเงินให้ tsd
    // ==========================================
    if (sd->deal.zeny) {
        int tax = 0;
        int original_zeny = sd->deal.zeny; // เก็บค่ายอดเงินตั้งต้นไว้ก่อน
        char output[256];
        
        pc_payzeny(sd, original_zeny, LOG_TYPE_TRADE, tsd->status.char_id);
        
        if (battle_config.trade_tax && original_zeny >= 100) {
            tax = (int)((original_zeny * total_tax_rate) / 100);
            sd->deal.zeny = original_zeny - tax;
            
            if (tax > 0) {
                // แก้ไข: เอา Single Quote ออกจาก %d 
                if (Sql_Query(mmysql_handle, "UPDATE `castle_money` SET `money` = `money` + %d WHERE `castle` = 0", tax) != SQL_SUCCESS) 
                    Sql_ShowDebug(mmysql_handle);
                    
                if (is_town) {
                    if (Sql_Query(mmysql_handle, "UPDATE `mayor_towns` SET `treasury` = `treasury` + %d WHERE `town_map` = '%s'", tax, map_mapid2mapname(map_id)) != SQL_SUCCESS) 
                        Sql_ShowDebug(mmysql_handle);
                }
            }
        }
        ...
        if (tax > 0) {
            // แจ้งเตือนคนรับ (tsd) -> ได้รับเงิน (1592)
            sprintf(output, msg_txt(tsd, 1592), sd->deal.zeny, total_tax_rate, tax);
            clif_displaymessage(tsd->fd, output);
            
            // แจ้งเตือนคนจ่าย (sd) -> จ่ายเงิน (1591)
            sprintf(output, msg_txt(sd, 1591), original_zeny, total_tax_rate, tax); 
    ...
    // ==========================================
    // ผู้เล่นคนที่ 2 (tsd) จ่ายเงินให้ sd
    // ==========================================
    if (tsd->deal.zeny) {
        int tax = 0;
        int original_zeny = tsd->deal.zeny; // เก็บค่ายอดเงินตั้งต้นไว้ก่อน
        char output[256];
        
        pc_payzeny(tsd, original_zeny, LOG_TYPE_TRADE, sd->status.char_id);
        
        if (battle_config.trade_tax && original_zeny >= 100) {
            tax = (int)((original_zeny * total_tax_rate) / 100);
            tsd->deal.zeny = original_zeny - tax;
        ...
        if (tax > 0) {
            // แจ้งเตือนคนรับ (sd) -> ได้รับเงิน (1592)
            sprintf(output, msg_txt(sd, 1592), tsd->deal.zeny, total_tax_rate, tax);
            clif_displaymessage(sd->fd, output);
            
            // แจ้งเตือนคนจ่าย (tsd) -> จ่ายเงิน (1591)
            sprintf(output, msg_txt(tsd, 1591), original_zeny, total_tax_rate, tax);
            clif_displaymessage(tsd->fd, output);
        }
        tsd->deal.zeny = 0;
    }
