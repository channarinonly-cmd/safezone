...
static TIMER_FUNC(pet_hungry){
	...
	if (pd->pet.intimate <= PET_INTIMATE_NONE)
		return 1; // ถ้าสัตว์เลี้ยงหนีไปแล้ว ไม่ต้องทำอะไรต่อ
	...
	// 🔴 **เพิ่มเงื่อนไขให้สัตว์เลี้ยงกลับเป็นไข่แทนที่จะหายไป**
	if( pd->pet.hungry < PET_HUNGRY_NONE ) {
		int food_index = pc_search_inventory(sd, pet_db_ptr->FoodID);

		if (food_index < 0) { 
			// ❌ ไม่มีอาหาร → ให้สัตว์เลี้ยงกลับเป็นไข่
			if (pet_return_egg(sd, pd)) {
				clif_displaymessage(sd->fd, "Your pet is so hungry that it has to return to its egg!"); 
				return 1; 
			}
		} else {
			// ✅ มีอาหาร → หิวจัดแต่ยังอยู่
			pet_stop_attack(pd);
			pd->pet.hungry = PET_HUNGRY_NONE;
			...
			interval = 20000; // หิวจัด ตรวจสอบทุก 20 วินาที
		}
	}
	...
	// 🔵 **เพิ่มเงื่อนไขให้อาหารอัตโนมัติ**
	if( pet_db_ptr->allow_autofeed && pd->pet.hungry <= PET_HUNGRY_HUNGRY ) {  
		pet_food(sd, pd); // ✅ ให้สัตว์เลี้ยงกินอาหารอัตโนมัติ
	}
	// 🔵 จบส่วนที่เพิ่มเข้ามา
...
#define PET_LOOT_RANGE 100
	if (!target && pd->loot && pd->loot->count < pd->loot->max) {
    // ตรวจสอบน้ำหนักของผู้เล่น
    if (sd->weight >= (sd->max_weight * 0.9)) {
        // หากน้ำหนักเกินหรือเท่ากับ 90% ของขีดจำกัดของผู้เล่น
        // จะไม่ทำการค้นหาไอเทมหรือวาร์ป
        pet_unlocktarget(pd);
        return 0;
    }

    // เมื่อน้ำหนักน้อยกว่า 90% จะทำการค้นหาไอเทมและวาร์ปไปยังไอเทม
    map_foreachinallrange(pet_ai_sub_hard_lootsearch, &pd->bl, PET_LOOT_RANGE, BL_ITEM, pd, &target);

    if (target) {
        // วาร์ปไปตำแหน่งของไอเทมทันที
        unit_warp(&pd->bl, target->m, target->x, target->y, CLR_TELEPORT);
        return 0;
    }
}
