...
bool pc_authok(map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	pc_group_pc_load(sd);
 	...
 	if (sd->group_id != 99) {
 		char ipstr[16];
 		sprintf(ipstr, "%d.%d.%d.%d", CONVIP(ip));
 	
 		if (battle_config.ip_block_limit) {
 			if (is_ip_exceed_online_limit(ipstr)) {
 				clif_authfail_fd(sd->fd, 0);
 				return false;
 			}
 		}
 	
 		if (battle_config.prevent_change_ip) {
 			char old_ip[64];
 			if (!is_whitelisted(ipstr) && compare_2ip(sd->status.account_id, ipstr, old_ip, sizeof(old_ip))) {
 				disconnect_old_ip_session(sd->status.account_id, old_ip);
 			}
 		}
 	}
...
void pc_aa_load(map_session_data* sd)
{
	...
	if (Sql_Query(mmysql_handle,"SELECT `stopmelee`,`pickup_item_config`,`aggressive_behavior`,`autositregen_conf`,"
		...
		"`tp_use_flywing`,`tp_min_hp`,`tp_delay_nomobmeet`,`skill_rate`,`teleport_boss`,`tp_swarm_enable`,`tp_swarm_count`,`focus_mob`,`stay_mode`,`revive_auto`,`party_auto`,`heal_party`,`heal_hp` FROM `aa_common_config` WHERE `char_id` = %d",
	...
	if(Sql_NumRows(mmysql_handle)){
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			...
			Sql_GetData(mmysql_handle, 14, &data, NULL); sd->aa.teleport.swarm_enable		= atoi(data) != 0;
			Sql_GetData(mmysql_handle, 15, &data, NULL); sd->aa.teleport.swarm_count		= (uint16)cap_value(atoi(data), 0, 50);
			Sql_GetData(mmysql_handle, 16, &data, NULL); sd->aa.focus_mob					= atoi(data);
			Sql_GetData(mmysql_handle, 17, &data, NULL); sd->aa.stay_mode				= atoi(data);
			Sql_GetData(mmysql_handle, 18, &data, NULL); sd->aa.revive_auto					= atoi(data);
			Sql_GetData(mmysql_handle, 19, &data, NULL); sd->aa.party_auto					= atoi(data);
			Sql_GetData(mmysql_handle, 20, &data, NULL); sd->aa.heal_party					= atoi(data);
			Sql_GetData(mmysql_handle, 21, &data, NULL); sd->aa.heal_hp					    = atoi(data);
...
void load_char_bonus_data(map_session_data &sd) {
	...
	if (Sql_Query(mmysql_handle,"SELECT `char_id`,`class`,`base_level` FROM `char` WHERE `account_id`= %d", sd.status.account_id ) != SQL_SUCCESS ){
		Sql_ShowDebug(mmysql_handle);
		return;
	}
	...
	if(Sql_NumRows(mmysql_handle)){
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			...
			Sql_GetData(mmysql_handle, 0,  &data, NULL); char_data.charid	= atoi(data);
			Sql_GetData(mmysql_handle, 1,  &data, NULL); char_data.jobid	= atoi(data);
			Sql_GetData(mmysql_handle, 2,  &data, NULL); char_data.level	= atoi(data);			
...
void pc_bonus(map_session_data *sd,int type,int val)
{
	...
	switch(type){
		...
		case SP_ADD_ITEM_SPHEAL_RATE:
			...
			break;
		case SP_AID_PERIOD_ADDSTOREITEMCOUNT:
			sd->bonus.addstoreitemcount += val;
			break;
		default:
...
void pc_bonus2(map_session_data *sd,int type,int type2,int val)
{
	...
	case SP_MAGIC_HP_DRAIN_RATE: // bonus2 bMagicHPDrainRate,x,n;
		if(sd->state.lr_flag != 2) {
			sd->bonus.magic_hp_drain_rate += type2;
			sd->bonus.magic_hp_drain_per += val;
		}
		break;
	case SP_MAGIC_SP_DRAIN_RATE: // bonus2 bMagicSPDrainRate,x,n;
		if(sd->state.lr_flag != 2) {
			sd->bonus.magic_sp_drain_rate += type2;
			sd->bonus.magic_sp_drain_per += val;
		}
		break;
...
int pc_insert_card(map_session_data* sd, int idx_card, int idx_equip)
{
	...
	// Card insertion blocker (configurable in conf/battle/misc.conf)
	{
		uint32 equipmask = item_eq->equip;
		bool allowed = true;

		// Priority: weapon -> shield -> armor -> head_top -> shoes -> garment
		if( (equipmask & EQP_WEAPON) || (equipmask & EQP_HAND_R) )
			allowed = battle_config.card_insert_allow_hand_r != 0;
		else if( (equipmask & EQP_SHIELD) || (equipmask & EQP_HAND_L) )
			allowed = battle_config.card_insert_allow_hand_l != 0;
		else if( equipmask & EQP_ACC_L )
			allowed = battle_config.card_insert_allow_acc_l != 0;
		else if( equipmask & EQP_ACC_R )
			allowed = battle_config.card_insert_allow_acc_r != 0;
		else if( equipmask & EQP_ACC )
			allowed = (battle_config.card_insert_allow_acc_l != 0 || battle_config.card_insert_allow_acc_r != 0);
		else if( equipmask & EQP_ARMOR )
			allowed = battle_config.card_insert_allow_armor != 0;
		else if( equipmask & EQP_HEAD_TOP )
			allowed = battle_config.card_insert_allow_head_top != 0;
		else if( equipmask & EQP_SHOES )
			allowed = battle_config.card_insert_allow_shoes != 0;
		else if( equipmask & EQP_GARMENT )
			allowed = battle_config.card_insert_allow_garment != 0;
...
pc_setreg(sd, add_str("@pickup_item_id"), fitem->item.nameid);
pc_setreg(sd, add_str("@pickup_mob_id"), fitem->mob_id); // 👈 เพิ่มบรรทัดนี้ เพื่อบอกว่าตกจากมอนสเตอร์ตัวไหน
npc_event(sd, "Item_Daily_Core::OnPCItemPickupEvent", 0);
...
//    struct mob_data *md;
//
//    // ตรวจสอบตัวชี้วัด null
//    if (!sd || !bl || bl->type != BL_MOB)
//        return false;
//
//    // ** เพิ่มบรรทัดนี้เพื่อเช็คสถานะ SC_STAMINA **
//    // ถ้าไม่ได้อยู่ในสถานะ SC_STAMINA ให้ยกเลิกการ Steal ทันที
...
//        map_getmapflag(bl->m, MF_NOMOBLOOT) ||
...
//
//    // คำนวณโอกาสสำเร็จ
//    rate = (sd_status->dex - md_status->dex) / 2 + skill_lv * 2 + 4;
//    rate += sd->bonus.add_steal_rate;
//
//    // ปรับลดเรทเมื่ออยู่ในสถานะ AUTOATTACK
//    if (sd->sc.getSCE(SC_AUTOATTACK) && battle_config.autoattack_steal_reduce_rate) {
//        rate = rate * (100 - battle_config.autoattack_steal_reduce_rate) / 100;
//    }
//
//    if (rate < 1 || rnd() % 100 >= rate)
//        return false;
//
//    // กำหนดไอเทมตามระดับมอนสเตอร์
//    if (md->level <= 20)
//        itemid = 3402;
//    else if (md->level <= 40)
//        itemid = 3403;
...
//    else if (md->level <= 99)
//        itemid = 3406;
//    else if (md->level <= 120)
//        itemid = 3409;
//    else if (md->level <= 140)
//        itemid = 3410;
//    else if (md->level <= 160)
//        itemid = 3411;
//    else if (md->level <= 180)
//        itemid = 3412;
//    else if (md->level <= 199)
//        itemid = 3413;
//    else
//        itemid = 3402;
//
...
//        memset(&tmp_item.option, 0, sizeof(tmp_item.option));
//    }
//
...
//
//    if (md->level <= 20 && itemid == 3402) {
//        struct item_data *i_data = itemdb_search(itemid);
//        char message[128];
//        sprintf(message, msg_txt(sd, 542), (sd->status.name[0]) ? sd->status.name : "GM", 
//               md->db->jname.c_str(), i_data->ename.c_str(), 5.0f);
...
bool pc_steal_item(map_session_data *sd,struct block_list *bl, uint16 skill_lv)
{
	...
	// ========================================================
	// 🛡️ [C++ Anti-Exploit] ระบบป้องกันบอท/มาโคร ปั๊มเงินด้วยสกิล Steal
	// บังคับว่าผู้เล่นต้องอยู่ในสถานะมี Stamina (เปิดใช้งานระบบพลังงาน) เท่านั้นถึงจะขโมยของได้
	// หากปล่อยบอททิ้งไว้จน Stamina หมด ระบบจะบล็อกการทำงานของสกิลนี้ทันที ป้องกัน Zeny เฟ้อ!
	// ========================================================
	if (!sd->sc.getSCE(SC_STAMINA)) {
		clif_displaymessage(sd->fd, "[ระบบความปลอดภัย] Stamina ของคุณหมดแล้ว ไม่สามารถใช้งานสกิล Steal ได้ค่ะ!");
		return false;
	}
	// ========================================================
	...
	for( i = 0; i < MAX_MOB_DROP; i++ ){
		uint32 new_rate = md->db->dropitem[i].rate;

		if(sd->sc.getSCE(SC_AUTOATTACK) && battle_config.autoattack_steal_reduce_rate){
			new_rate = (new_rate * (100 - battle_config.autoattack_steal_reduce_rate)) / 100;
		}

		if( item_db.exists(md->db->dropitem[i].nameid) && !md->db->dropitem[i].steal_protected && rnd() % 10000 < new_rate
#ifndef RENEWAL
		* rate/100.
#endif
		)
...
int pc_steal_coin(map_session_data *sd,struct block_list *target)
{
	...
	// --- Custom: Spoof Monster Level for Steal Coin ---
	// ป้องกันปัญหา Zeny เฟ้อจากมอนสเตอร์เลเวลสูงจัด
	if (battle_config.mob_max_level_cap > 0 && target_lv > battle_config.mob_max_level_cap) {
		target_lv = battle_config.mob_max_level_cap;
	}
	// --------------------------------------------------
...
//		} else {
//			if(!pc_issit(sd)){
//				int i = pc_search_inventory(sd, 600); 
//				int i2 = pc_search_inventory(sd, 601);
//				if (i >= 0) {
//					pc_setpos(sd, map_id2index(tbl->m), tbl->x, tbl->y, CLR_TELEPORT);
//					sd->aa.last_warp_time = gettick();	
//				} else if (pc_checkskill(sd, AL_TELEPORT) > 0) {
//					pc_setpos(sd, map_id2index(tbl->m), tbl->x, tbl->y, CLR_TELEPORT);
//					status_heal(&sd->bl, 0, -20, 0);
//					sd->aa.last_warp_time = gettick();						
//				} else if (i2 >= 0) {
//					pc_delitem(sd, i2, 1, 0, 0, LOG_TYPE_CONSUME);
//					pc_setpos(sd, map_id2index(tbl->m), tbl->x, tbl->y, CLR_TELEPORT);
//					sd->aa.last_warp_time = gettick();						
//				}
//			}	
...
TIMER_FUNC(pc_follow_timer) {
	...
	// ทั้งผู้ตามและเป้าหมายต้องถูก attach อยู่กับ map blocks และไม่ติดการกระทำอื่น
	if (sd->bl.prev != nullptr && tbl->prev != nullptr &&
		sd->ud.skilltimer  == INVALID_TIMER &&
		sd->ud.attacktimer == INVALID_TIMER &&
		sd->ud.walktimer   == INVALID_TIMER) {

		// แมพเดียวกันและเดินไปถึงได้
		if ((sd->bl.m == tbl->m) && unit_can_reach_bl(&sd->bl, tbl, AREA_SIZE, 0, nullptr, nullptr)) {
			// ดึงค่าระยะการโจมตีของอาวุธที่ผู้เล่นใส่อยู่ (เช่น ธนู=10, หอก=3, มีด=1)
			int atk_range = sd->battle_status.rhw.range;
			if (atk_range < 1) atk_range = 1; // กันเหนียวไว้อย่างน้อย 1 ช่อง

			// ถ้าตัวละครอยู่ไกลกว่าระยะโจมตี ค่อยสั่งให้เดินเข้าหา
			if (!check_distance_bl(&sd->bl, tbl, atk_range))
				unit_walktobl(&sd->bl, tbl, atk_range, 0);
		} else {
			// ข้ามแมพ/ไปไม่ถึง
			if (!pc_issit(sd)) {
				bool warped = false;

// 1) เช็คสกิล Teleport ก่อน (สำหรับตัวเดินตามที่มีสกิลหรือใส่คลิปวิง)
				if (!warped) {
					if (pc_checkskill(sd, AL_TELEPORT) > 0) {
						int sp_cost = skill_get_sp(AL_TELEPORT, 1); // ใช้ SP ตามเลเวล 1
						
						if (sd->status.sp >= sp_cost) {
							// หัก SP และทำการวาร์ป
							status_heal(&sd->bl, 0, -sp_cost, 0); 
							pc_setpos(sd, map_id2index(tbl->m), tbl->x, tbl->y, CLR_TELEPORT);
							sd->aa.last_warp_time = gettick();
							warped = true;
						}
				...
				// 2) ถ้าไม่มีสกิลหรือ SP ไม่พอ ให้ลองใช้ไอเท็มตามลำดับ 12323, 601, 12887
				if (!warped) {
					int items[] = {12323, 601, 12887};
					for (int i = 0; i < (int)(sizeof(items)/sizeof(items[0])); ++i) {
						int item = items[i];
						int index = pc_search_inventory(sd, item);
						if (index >= 0) {
							
							// --- [กฎการหักไอเทม: 12323 และ 601 หักออก 1 ชิ้น | 12887 ไม่หัก] ---
							if (item != 12887) {
				...
				// 3) อุดช่องโหว่วาร์ปฟรี: ถ้าไม่มีทั้งสกิลและไอเทมวิงเลย (warped ยังคงเป็น false)
				if (!warped) {
					// แจ้งเตือนเพื่อให้เจ้าของรู้ว่าตัวเดินตามค้างเพราะอะไร
					clif_displaymessage(sd->fd, "ระบบเดินตาม: ไม่สามารถวาร์ปได้ (SP ไม่พอ หรือ Fly Wing หมด)");
					
					// ไม่ใส่ pc_setpos() ตรงนี้ เพื่อป้องกันไม่ให้วาร์ปฟรีโดยไม่มีต้นทุน
				}
			}
			// ถ้านั่งอยู่ จะไม่วาร์ป (คงพฤติกรรมเดิม)
		}
	}

	// ตั้ง timer ใหม่ (250ms)
	sd->followtimer = add_timer(tick + 250, pc_follow_timer, sd->bl.id, 0);
	return 0;
}
...
static void pc_calcexp(map_session_data *sd, t_exp *base_exp, t_exp *job_exp, struct block_list *src)
{
	...
	if (!sd->sc.getSCE(SC_STAMINA) && battle_config.stamina_enable && !map_getmapflag(sd->bl.m, MF_NOSTAMINA)) {
		*base_exp = 0;
		*job_exp = 0;
	}
...
TIMER_FUNC(pc_deathmatch_timer){
...
void pc_damage(map_session_data *sd,struct block_list *src,unsigned int hp, unsigned int sp, unsigned int ap)
{
	...
	// ดักไว้เลยว่า ต้องอยู่ในสถานะเปิด Auto Attack เท่านั้น โค้ดส่วนนี้ถึงจะทำงาน
	if (sd->sc.getSCE(SC_AUTOATTACK)) {
		...
		// ===================================================
		// 🔥 [Custom] ยันต์กันผี! ป้องกัน Auto Attack ล็อคเป้าร่างโคลนตอนโดนตี (สวนกลับ)
		// ===================================================
		if(!sd->aa.target_id && !sd->aa.mobs.aggressive_behavior && src->type == BL_MOB){
			struct mob_data *md = (struct mob_data *)src;
			// เช็คก่อนว่าคนที่มาสะกิด "ไม่ใช่มอนสเตอร์โคลน" ใช่ไหม?
			if (!(md->npc_event[0] != '\0' && (strstr(md->npc_event, "OnFieldDead") || strstr(md->npc_event, "FPCtrl")))) {
				...
				sd->aa.target_id = src->id; // ถ้าเป็นมอนสเตอร์ป่าปกติ ให้ล็อคเป้าหวดกลับได้เลย!
			}
		}
		// ===================================================
...
//get itemid
int pc_get_itemid(struct map_session_data *sd, short equip_index) {

    if (equip_index < 0 || equip_index >= MAX_INVENTORY) {
        return -1;  
    }


    if (!sd->inventory_data[equip_index]) {
        return -1; 
    }


    return sd->inventory_data[equip_index]->nameid;
}
...
bool pc_equipitem(map_session_data *sd,short n,int req_pos,bool equipswitch)
{
	...
	if(pos & EQP_HAND_R) {
		if(id)
			...
			sd->weapontype1 = W_FIST;
		pc_calcweapontype(sd);
		clif_changelook(&sd->bl, LOOK_WEAPON, sd->status.costume_weapon ? sd->status.costume_weapon : sd->status.weapon);
	}
	if(pos & EQP_HAND_L) {
		if(id) {
	...
	npc_script_event(sd, NPCE_EQUIPITEM);
...
bool pc_unequipitem(map_session_data *sd, int n, int flag) {
	...
	pc_setreg(sd, add_str("@unequip_pos"), pos); // <--- เพิ่มบรรทัดนี้ เพื่อส่งตำแหน่งที่ถอดให้สคริปต์

	npc_script_event(sd, NPCE_UNEQUIPITEM);
...
bool is_whitelisted(const char* ip) {
	if (!battle_config.roplay_ip_whitelist[0]) return false;

	char temp[512];
	safestrncpy(temp, battle_config.roplay_ip_whitelist, sizeof(temp));

	char* token = strtok(temp, " ,");
	while (token != NULL) {
		if (strcmp(ip, token) == 0) return true;
		token = strtok(NULL, " ,");
	}
	return false;
}

bool is_ip_exceed_online_limit(const char* ip) {
	const int limit = battle_config.ip_block_limit;
	int count = 0;
	...
	for (int i = 0; i < fd_max; i++) {
		...
		char current_ip[64];
		ip2str(session[i]->client_addr, current_ip);
...
//Compare New IP and Old IP
bool compare_2ip(int account_id, const char* new_ip, char* old_ip_out, size_t out_size) {
	char old_ip[64] = "";

	if (Sql_Query(mmysql_handle, "SELECT `old_ip` FROM `login` WHERE `account_id` = %d", account_id) != SQL_SUCCESS) {
		Sql_ShowDebug(mmysql_handle);
		safestrncpy(old_ip_out, "", out_size);
		return false;
	}
	...
	if (Sql_NumRows(mmysql_handle)) {
		...
		if (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			...
			if (data)
				safestrncpy(old_ip, data, sizeof(old_ip));
		}
	}

	Sql_FreeResult(mmysql_handle);

	safestrncpy(old_ip_out, old_ip, out_size);

	return strcmp(new_ip, old_ip) != 0;
}

//Disconnect Old IP
void disconnect_old_ip_session(int account_id, const char* old_ip) {
	for (int i = 0; i < fd_max; i++) {
		if (!session_isActive(i)) continue;

		struct map_session_data* tsd = (struct map_session_data*)session[i]->session_data;
		if (!tsd) continue;

		char current_ip[64];
		ip2str(session[i]->client_addr, current_ip);

		if (strcmp(current_ip, old_ip) == 0 && !tsd->state.autotrade) {
			ShowInfo("Disconnecting session for account %d (char_id: %d) with old IP: %s\n",
				tsd->status.account_id, tsd->status.char_id, old_ip);
...
bool pc_setstand(map_session_data *sd, bool force){
	...
	// ==========================================
	// --- Addon: [แนวคิด PK PASS] บังคับ Sync สถานะ PVP ---
	// ==========================================
	if (sd) {
		// ไม่ต้องเช็ค map_getcell ใหม่ เพราะ sd->state.pvp ถูกดักไว้ตอนเดินเข้า/เกิดในโซนแล้ว
		// เราแค่สั่ง "ตบหน้า" Client ให้ตื่นมาอ่านค่า Property ล่าสุด
		if (sd->state.pvp == 1) {
...
uint16 pc_level_penalty_mod( map_session_data* sd, e_penalty_type type, std::shared_ptr<s_mob_db> mob, mob_data* md ){
	...
	// --- Custom: Spoof Monster Level for Renewal Penalty ---
	// หลอกระบบว่ามอนสเตอร์เลเวลตันแค่นี้ เพื่อไม่ให้ผู้เล่นโดนหัก EXP และ Drop
	if (battle_config.mob_max_level_cap > 0 && monster_level > battle_config.mob_max_level_cap) {
		monster_level = battle_config.mob_max_level_cap;
	}
	// -------------------------------------------------------
			...
			for(const auto &sd_data : sd->char_bonus) {
				if(bonus_db.second->jobid == JOB_ALL && sd->status.base_level >= bonus_db.second->level) {
					...
					if(bonus_db.second->icon != EFST_BLANK)
						clif_status_load(&sd->bl, bonus_db.second->icon, 1);

					if(bonus_db.second->script)
						run_script(bonus_db.second->script, 0, sd->bl.id,0);
				}
...
bool pc_check_skill_allow(map_session_data *sd, int skill_id)
{
	if(sd == nullptr)
		return false;

	std::vector<t_itemid> skills = {};

	for(auto &entry : no_skill_db){
		if(util::vector_exists(entry.second->maps,sd->bl.m)){
			skills = entry.second->skills;
			break;
		}
	}

	if(util::vector_exists(skills,skill_id))
		return true;
...
// ==========================================
// Custom Fake Player (Dummy PC)
// ==========================================
struct map_session_data* pc_create_dummy(int char_id, const char* map_name, int x, int y, const char* fake_name) {
    int16 m = map_mapname2mapid(map_name);
    if (m < 0) return NULL; // หาแมพไม่เจอ

    // 1. สร้าง Session ผู้เล่นแบบไม่มีหน้าจอ (fd = 0)
    struct map_session_data* sd = (struct map_session_data*)aCalloc(1, sizeof(struct map_session_data));
    sd->bl.type = BL_PC;
    sd->fd = 0; // บอกเซิร์ฟเวอร์ว่านี่คือบอท!

    // 2. ป้องกัน ID ชนกัน
    sd->bl.id = npc_get_new_npc_id(); 
    sd->status.account_id = -sd->bl.id; // ใช้ ID ติดลบกัน Auth Mismatch
    sd->status.char_id = char_id;
    safestrncpy(sd->status.name, fake_name, NAME_LENGTH);
    ...
    // 3. ดึงรูปร่างจาก SQL
    if (SQL_SUCCESS == Sql_Query(mmysql_handle, "SELECT `class`, `base_level`, `hair`, `hair_color`, `clothes_color`, `head_top`, `head_mid`, `head_bottom`, `sex` FROM `char` WHERE `char_id` = %d", char_id) && Sql_NumRows(mmysql_handle) > 0) {
        Sql_NextRow(mmysql_handle);
        char* data;
        Sql_GetData(mmysql_handle, 0, &data, NULL); sd->status.class_ = atoi(data);
        Sql_GetData(mmysql_handle, 1, &data, NULL); sd->status.base_level = atoi(data);
        Sql_GetData(mmysql_handle, 2, &data, NULL); sd->status.hair = atoi(data);
        Sql_GetData(mmysql_handle, 3, &data, NULL); sd->status.hair_color = atoi(data);
        Sql_GetData(mmysql_handle, 4, &data, NULL); sd->status.clothes_color = atoi(data);
        Sql_GetData(mmysql_handle, 5, &data, NULL); sd->status.head_top = atoi(data);
        Sql_GetData(mmysql_handle, 6, &data, NULL); sd->status.head_mid = atoi(data);
        Sql_GetData(mmysql_handle, 7, &data, NULL); sd->status.head_bottom = atoi(data);
        Sql_GetData(mmysql_handle, 8, &data, NULL); sd->status.sex = (data[0] == 'F' ? 0 : 1);
        Sql_FreeResult(mmysql_handle);
    }

    // 4. อัปเดต View Data (ให้คนมองเห็นชุด)
    ...
    // 5. ดึงของสวมใส่จาก SQL (ส่องของได้เพราะตรงนี้)
    ...
    clif_spawn(&sd->bl);
    map_getmapdata(m)->users++; // ดันยอดคนออนไลน์
